...
Proof with simpl_env;
  ...
  Case "typing_tabs".
    ...
    lapply (H0 X); [intros Q | auto].
    rewrite <- app_assoc.
    apply H0...
      rewrite_env (nil ++ [(X, bind_kn)] ++ G ++ F ++ E0).
      apply wf_lenv_weakening; auto. 
        simpl_env. apply wf_env_kn; auto.
  Case "typing_let".
    apply typing_let with (L:=L `union` dom (G++F++E0) `union` dom D2)(D1:=D1)(D2:=D2)(T1:=T1).
      apply IHTyp; auto.
        eapply wf_lenv_split_left.
          apply lenv_split_weakening; eauto. 
            apply wf_lenv_disjoint in WFL. solve_uniq.
      ...
      rewrite <- app_assoc.
      apply (H0 x); auto.
      rewrite_env (nil ++ [(x, bind_typ T1)] ++ G ++ F ++ E0).
...
Lemma lenv_split_subst_tb : forall G1 G2 D1 D2 D3 X P,
  lenv_split (G1 ++ [(X, bind_kn)] ++ G2) D1 D2 D3 ->
  wf_typ G2 P ->
...
Proof.
  intros G1 G2 D1 D2 D3 X P S.
  remember (G1 ++ [(X, bind_kn)] ++ G2) as G.
...
Proof.
  ...
  Case "value_abs".
    assert (expr (subst_te Z P (exp_abs T e1))) as J.
      apply subst_te_expr; auto.
    apply value_abs; auto.
  Case "value_tabs".
    assert (expr (subst_te Z P (exp_tabs e1))) as J.
      apply subst_te_expr; auto.
    apply value_tabs; auto.
Qed.

Lemma typing_through_subst_te : forall E F D Z e T P,
  typing (F ++ [(Z, bind_kn)] ++ E) D e T ->
  wf_typ E P ->
  typing (map (subst_tb Z P) F ++ E) (map (subst_tlb Z P) D) (subst_te Z P e) (subst_tt Z P T).
Proof with simpl_env;
           eauto 6 using wf_env_subst_tb,
                         wf_typ_subst_tb.
  intros E F D Z e T P Typ PsubK.
  remember (F ++ [(Z, bind_kn)] ++ E) as G.
  ...
  Case "typing_var".
    ...
    rewrite (map_subst_tb_id E0 Z P).
      ...
      assert (wf_env ([(Z, bind_kn)] ++ E0)) as J.
        eapply wf_env_strengthening_tail; eauto.
      inversion J; auto.
  Case "typing_lvar".
  ...
  Case "typing_tabs".
    pick fresh Y and apply typing_tabs.
      ...
      rewrite_env (map (subst_tb Z P) ([(Y, bind_kn)] ++ F) ++ E0); eauto.
  Case "typing_tapp".
    rewrite subst_tt_open_tt... 
      eapply type_from_wf_typ; eauto.
  Case "typing_let".
    apply typing_let with (L:=L)(D1:=map (subst_tlb Z P) D1)(D2:=map (subst_tlb Z P) D2)(T1:=subst_tt Z P T1); auto.
      intros x xnL.
      rewrite subst_te_open_ee_var...
      rewrite_env (map (subst_tb Z P) ([(x, bind_typ T1)] ++ F) ++ E0).
      simpl in H0.
      apply H0...
...
Proof.
  ...
  Case "typing_app".
    ...
    fsetdec.
  Case "typing_tabs".
    pick fresh y. lapply (H0 y); intros; auto.
  ...
  Case "typing_let".
    ...
    assert (x `notin` fv_ee e2) as J2.
      unfold not. intros.
      pick fresh y. 
      apply fv_in_open_ee with (k:=0)(e2:=y) in H3; auto.
...
Proof.
  ...
  Case "typing_tabs".
    pick fresh X and apply typing_tabs; auto.
      rewrite_env (([(X, bind_kn)] ++ G2) ++ G1).
      rewrite subst_ee_open_te_var; eauto.
  Case "typing_tapp".
    eapply typing_tapp. eapply IHTyp; auto.
    eapply wf_typ_strengthening; eauto.
  Case "typing_let".
    apply typing_let with (L:=L `union` {{x}})(D1:=D1)(D2:=D2)(T1:=T1); auto.
      intros x0 x0nL.
      rewrite_env (([(x0, bind_typ T1)] ++ G2) ++ G1).
      rewrite subst_ee_open_ee_var; auto.
...
Proof.
  ...
  generalize dependent G2. generalize dependent G1.
  (typing_cases (induction Typ) Case);   
    intros G1 G2 EQ; simpl_env in *; simpl; subst; simpl_env; eauto.
  Case "typing_var".
    destruct (x0 == y); subst.
    SCase "x0 == y".
      apply typing_var.
  ...
  Case "typing_tabs".
    pick fresh X and apply typing_tabs; auto.
      rewrite_env (([(X, bind_kn)] ++ G2) ++ [(x, bind_typ U0)] ++ G1).
      rewrite subst_ee_open_te_var; eauto.
  Case "typing_tapp".
    eapply typing_tapp. eapply IHTyp; auto.
    eapply wf_typ_strengthening; eauto.
  Case "typing_let".
    apply typing_let with (L:=L `union` {{y}})(D1:=D1)(D2:=D2)(T1:=T1); auto.
      intros x0 x0nL.
      rewrite_env (([(x0, bind_typ T1)] ++ G2) ++ [(x, bind_typ U0)] ++ G1).
      rewrite subst_ee_open_ee_var; auto.
  ...
  Case "typing_app".
    ...
    SCase "left".
      ...
      rewrite <- (non_subst E (D2L++D2R) e2 T1 x u); auto.
        apply (typing_app T1 E (D1L ++ F ++ D1R) (D2L ++ D2R)); auto.
    ...
    SCase "right".
      ...
      rewrite <- (non_subst E (D1L ++ D1R) e1 (typ_arrow T1 T2) x u); auto.
        apply (typing_app T1 E (D1L ++ D1R) (D2L ++ F ++ D2R)); auto.
      ...
      apply H0; auto. 
        ...
        apply typing_weakening; auto.
          ...
          apply wf_lenv_weakening; auto.
            simpl_env. apply wf_env_kn; auto.
  Case "typing_bang".
    contradict EQ. apply nil_neq_one_mid.
  Case "typing_let".
    lapply (lenv_split_cases_mid E D1 D2 DL x U DR); auto.
    intros C.
    destruct C as [LEFT | RIGHT].
    SCase "left".
      destruct LEFT as [D1L [D1R [D2L [D2R [Q1 [Q2 [S1 S2]]]]]]]; subst.
      lapply (IHTYP D1R D1L); auto.
      intros I. 
      assert (x `notin` (dom (D2L++D2R) `union` dom E)) as J.
        eapply lenv_split_not_in_left; eauto.
          simpl_env. fsetdec.
      lapply I; auto. intros TYPE1. 
      apply typing_let with (L:=L `union` {{x}})(D1:=D1L ++ F ++ D1R)(D2:=D2L ++ D2R)(T1:=T1); auto.
        assert (wf_lenv E (D1L ++ F ++ D1R)) as JJ.
          eapply wf_lenv_split_left. 
            eapply lenv_split_sub_left; eauto.
        apply TYPE1; auto.
        ...
        intros x0 x0nL.
        rewrite subst_ee_open_ee_var; auto.
        rewrite <- (non_subst ([(x0, bind_typ T1)] ++ E) (D2L++D2R) (open_ee e2 x0) T2 x u); auto.
    ...
    SCase "right".
      ...
      assert (x `notin` (dom (D1L++D1R) `union` dom E)) as J.
        eapply lenv_split_not_in_right; eauto.
          simpl_env. fsetdec.
      rewrite <- (non_subst E (D1L ++ D1R) e1 (typ_bang T1) x u); auto.
      assert (wf_typ E T1) as WFT1.
        apply typing_regular in TYP.
        destruct TYP as [_ [_ [_ WFT]]]. inversion WFT; subst; auto.
      assert (wf_lenv E (D2L ++ F ++ D2R)) as JJ.
        eapply wf_lenv_split_right. 
          eapply lenv_split_sub_right; eauto.
      apply typing_let with (L:=L `union` {{x}} `union` dom E `union` dom F `union` dom (D2L ++ F ++ D2R))(D1:=D1L ++ D1R)(D2:=D2L ++ F ++ D2R)(T1:=T1); auto.
        intros x0 x0nL.
        rewrite subst_ee_open_ee_var; auto.
        apply H0; auto.
          rewrite_env (nil ++ [(x0, bind_typ T1)] ++ E).
          apply typing_weakening; auto.
...
Proof.
  ...
  Case "typing_app".
    ...
    apply (typing_app T1 E D1P D2P DX); auto.
  Case "typing_tabs".
  ...
  Case "typing_let".
    assert (exists D1P, exists D2P,
      ...
      eapply lenv_split_exists_permute; eauto.
    destruct J as [D1P [D2P [PERM2 [PERM3 S]]]].
    apply typing_let with (L:=L)(D1:=D1P)(D2:=D2P)(T1:=T1); auto.
Qed.
...
Proof with simpl_env; eauto.
  ...
  Case "typing_let".
    ...
    SCase "red_beta".
      ...
      rewrite (subst_ee_intro x); auto.
      assert (D2 = D3) as J.
        apply (lenv_split_empty_left E D2 D3); auto. 
      subst D2.
      lapply (H x); auto.
        intros J.
        rewrite_env (nil ++ E).
...
Lemma canonical_form_abs : forall e U1 U2,
  value e ->
  typing nil lempty e (typ_arrow U1 U2) ->
  exists V, exists e1, (e = exp_abs V e1).
Proof.
  intros e U1 U2 Val Typ.
  remember Bang_Definitions.empty as E.
...
Lemma canonical_form_tabs : forall e U1,
  value e ->
  typing nil lempty e (typ_all U1) ->
  exists e1, e = exp_tabs e1.
Proof.
  intros e U1 Val Typ.
  remember Bang_Definitions.empty as E.
  remember (typ_all U1) as T.
  revert U1 HeqT HeqE.
  (typing_cases (induction Typ) Case); 
    intros U1 EQT EQE; subst;
    try solve [ inversion Val | inversion EQT | eauto ].
Qed.

Lemma canonical_form_bang : forall e U,
  value e ->
  typing nil lempty e (typ_bang U) ->
  exists e1, (e = exp_bang e1).
Proof.
  intros e U Val Typ.
  remember Bang_Definitions.empty as E.
  remember lempty as D.
  remember (typ_bang U) as T.
  revert U HeqT HeqE HeqD.
  (typing_cases (induction Typ) Case); 
    intros U EQT EQE EQD; subst;
    try solve [ inversion Val | inversion EQT | eauto ].
Qed.
...
Proof with eauto.
  ...
  Case "typing_app".
    ...
    SCase "Val1".
      destruct (canonical_form_abs _ _ _ Val1 Typ1) as [S [e3 EQE]]; subst.
      exists (open_ee e3 e2)...
  Case "typing_tapp".
    right.
    destruct IHTyp as [Val1 | [e1' Rede1']]...
    SCase "Val1".
      destruct (canonical_form_tabs _ _ Val1 Typ) as [e3 EQ]; subst.
      exists (open_te e3 T)...
  ...
  Case "typing_let".
    ...
    destruct IHTyp as [Val1 | [e1' Rede1']]...
    SCase "Val1".
      destruct (canonical_form_bang _ _ Val1 Typ) as [e3 EQE]; subst.
      exists (open_ee e2 e3)...
  Case "typing_fst".
    right.
