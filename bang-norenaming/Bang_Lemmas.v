(** Administrative lemmas for Bang.
...
Require Export Bang_Infrastructure.
  ...
  Case "wf_typ_var".
    destruct (X == Z).  subst...
      analyze_binds_uniq H0... 
        apply wf_typ_var.
          assert ((subst_tb Z P bind_kn) = bind_kn) as J; auto.
            solve_uniq.
          apply binds_weaken_at_tail.

          assert (bind_kn = (subst_tb Z P bind_kn)) as J; auto.
          rewrite J.
          apply binds_map; auto.
  Case "wf_typ_all".
    pick fresh Y and apply wf_typ_all...
    rewrite subst_tt_open_tt_var...
    rewrite_env (map (subst_tb Z P) ([(Y, bind_kn)] ++ F) ++ E).
    apply H0...
Qed.
...
Lemma wf_typ_strengthening_typ : forall E F X T,
 wf_typ (F ++ [(X, bind_kn)] ++ E) T ->
 X `notin` (fv_tt T) ->
 wf_typ (F ++ E) T.
Proof with simpl_env; eauto.
  intros E F X T H.
  remember (F ++ [(X, bind_kn)] ++ E) as G.
  generalize dependent F.
...
Lemma wf_lenv_strengthening_typ: forall G D X, 
  wf_lenv ([(X, bind_kn)] ++ G) D ->
  X `notin` fv_lenv D ->
  wf_lenv G D.
Proof.
  induction D; intros X WFL NIN.
  Case "nil".
    apply wf_lenv_empty.
      inversion WFL; subst. 
      inversion H; subst.
...
Lemma wf_env_subst_tb : forall Z P E F,
  wf_env (F ++ [(Z, bind_kn)] ++ E) ->
  wf_typ E P ->
...
Lemma wf_lenv_subst_tb: forall Z P E F D,
  wf_lenv (F ++ [(Z, bind_kn)] ++ E) D ->
  wf_typ E P ->
  wf_lenv (map (subst_tb Z P) F ++ E) (map (subst_tlb Z P) D).
Proof.
  intros Z P E F D WFL.
  remember (F ++[(Z, bind_kn)] ++ E) as E0.
  generalize dependent F.
...
Lemma typing_regular : forall E D e T,
  typing E D e T ->
  wf_env E /\ wf_lenv E D /\ expr e /\ wf_typ E T.
Proof with simpl_env; try solve [intuition].
  ...
  Case "typing_abs".
    pick fresh y.
    destruct (H1 y) as [WF [WFL [EXP WFT]]]...
    repeat split...
    SCase "wf_lenv".
      inversion WFL; auto.
    SCase "expr".
  ...
  Case "typing_tabs".
    pick fresh Y.
    destruct (H0 Y) as [HWF [WFL [EX WT]]]...
    inversion HWF; subst.
    repeat split...
    SCase "wf_lenv".
  ...
  Case "typing_let".
    pick fresh y.
    destruct (H1 y) as [WF2 [WFL2 [EXP2 WFT2]]]...
    destruct IHtyping as [WF1 [WFL1 [EXP1 WFT1]]].
    repeat split...
    SCase "expr".
...
Proof with try solve [solve_notin | intuition].
  ...
  Case "red_let_cong".
    destruct IHred as [EXP1 EXP1'].
    apply expr_let with (L:={}) ; auto.
      intros x xnL.
      inversion H. pick fresh y. rewrite (subst_ee_intro y)...
  Case "red_let_beta".
    inversion H; subst. inversion H2; subst.
    pick fresh y. rewrite (subst_ee_intro y)...
Qed.
