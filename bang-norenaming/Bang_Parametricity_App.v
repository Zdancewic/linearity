...
*)
Lemma Identification : forall Id A A', 
...
Proof.
  ...
  assert (F_related_terms (typ_all (typ_arrow (typ_bvar 0) (typ_bvar 0))) rho_nil delta_nil delta_nil Id Id) as Frel_All.
  ...
  destruct (@Harrow x y) as [u [u' [Hn_wxu [Hn_w'x'u' Hrel_wft2]]]]; auto.
    simpl in Htypingv0.
...
Corollary  Rearrangement_Identification : forall Id a A A', 
  typing nil nil Id (typ_all (typ_arrow (typ_bvar 0) (typ_bvar 0))) ->
...
Proof.
  intros Id a A A' Htypingid Htypinga x Htypingx Hn_xx0 Hn_axx'0.
  ...
  destruct (@Identification Id A A' Htypingid x (exp_app a x) (Rfun A A' a)) as [X JJ].
  ...
  repeat(split; auto).
    ...
    apply preservation_normalization with (e:=exp_app (exp_tapp Id A) x); auto.
      apply typing_app with (T1:=A)(D1:=nil) (D2:=nil); auto.
        assert (open_tt (typ_arrow (typ_bvar 0) (typ_bvar 0)) A = typ_arrow A A).
          unfold open_tt. auto.
        rewrite <- H.
...
Corollary  EQ_Identification : forall Id, 
  typing nil nil Id (typ_all (typ_arrow (typ_bvar 0) (typ_bvar 0))) ->
  (forall x y A, 
    Rid A x y -> 
    (exists x0, normalize x x0) ->
...
Proof.
  intros Id Htypingid x y A HRid Hn_xx0 Hn_yy0.
  unfold Rid in *. destruct HRid as [Hvx [Hvy [Htypingx [Htypingy Heq_xy]]]].
  destruct Hn_xx0 as [x0 [Hbrc_xx0 Hx0]].
  destruct Hn_yy0 as [y0 [Hbrc_yy0 Hy0]].
  destruct (@Identification Id A A Htypingid x y (Rid A)) as [X JJ].
...
*)
Lemma CBoolean : forall CBool A A',
  typing nil nil CBool (typ_all (typ_arrow (typ_bvar 0) (typ_arrow (typ_bvar 0) (typ_bvar 0)))) ->
...
Proof.
  intros CBool A A' Htyping_bool t1 t2 f1 f2 R.
  ...
  assert (F_related_terms (typ_all (typ_arrow (typ_bvar 0) (typ_arrow (typ_bvar 0) (typ_bvar 0)))) rho_nil delta_nil delta_nil CBool CBool) as Frel_All.
    apply fundamental_parametricity; auto.
  destruct Frel_All as [v [v' [Htypingid [Htypingid' [Hn [Hn' Frel_All]]]]]].
  ...
  destruct (@Harrow t1 t2) as [u [u' [Hn_wxu [Hn_w'x'u' Hrel_wft2]]]]; auto.
    simpl in Htypingv0.
    assert ((if X==X then A else typ_fvar X) = A).
      destruct (X==X); auto. contradict n; auto.
    rewrite H in Htypingv0. 
  ...
  destruct (@Harrow' f1 f2) as [x [x' [Hn_uxu0 [Hn_u'x'u'0 Hrel_wft22]]]]; auto.
    simpl in Htypingv1.
    assert ((if X==X then A else typ_fvar X) = A).
      destruct (X==X); auto. contradict n; auto.
...
Corollary  Rearrangement_CBoolean : forall CBool a A A', 
  typing nil nil CBool (typ_all (typ_arrow (typ_bvar 0) (typ_arrow (typ_bvar 0) (typ_bvar 0)))) ->
  typing nil nil a (typ_arrow A A') ->
  (forall t f, 
    typing nil nil t A -> 
...
Proof.
  intros CBool a A A' Htypingid Htypinga t f 
    Htypingt Htypingf Hn_tt0 Hn_att'0 Hn_ff0 Hn_aff'0.
  assert (wf_typ nil A) as HwftA. auto.
  assert (wf_typ nil A') as HwftA'. 
    apply typing_regular in Htypinga. decompose [and] Htypinga.
    apply wft_arrow_inv in H3. destruct H3; auto.
  destruct (@CBoolean CBool A A' Htypingid t (exp_app a t) f (exp_app a f) (Rfun A A' a)) as [X JJ].
...
Corollary  CBool_Identification : forall CBool, 
  typing nil nil CBool (typ_all (typ_arrow (typ_bvar 0) (typ_arrow (typ_bvar 0) (typ_bvar 0)))) ->
...
Proof.
  intros CBool Htyping_bool t1 t2 f1 f2 A HRidt HRidf Hn_t1vt1 Hn_t2vt2 Hn_f1vf1 Hn_f2vf2.
  unfold Rid in *. 
  ...
  destruct (@CBoolean CBool A A Htyping_bool t1 t2 f1 f2 (Rid A)) as [X JJ]. 
  
  assert (wfr (Rid A) A A) as WFR.
    split; auto.
