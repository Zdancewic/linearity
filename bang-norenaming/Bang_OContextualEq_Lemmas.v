...
Require Import Bang_ContextualEq_Lemmas.

Export OParametricity.

Lemma in_dE__notin_lE : forall X E (lE:lenv),
  X `in` ddom_env E ->
  X `notin` dom lE.
Proof.
  intros.
  destruct (@in_dec X (dom lE)); auto.
    assert (X <> X) as J.
      apply typvars_neq_termvars with (E:=E)(lE:=lE); auto.  
    contradict J; auto.
Qed.
...
Lemma F_Related_osubst_lcons_inv : forall E x T D gsubst gsubst' lgsubst lgsubst' rsubst dsubst dsubst' Env lEnv,
  ...
  exists e, exists lgsubst0,
    ...
    exists lEnv1, exists lEnv',
      ...
      typing Env lEnv1 e (apply_delta_subst_typ dsubst T) /\
      typing Env lEnv1 e' (apply_delta_subst_typ dsubst' T) /\
      F_Related_oterms T rsubst dsubst dsubst' e e' Env lEnv1 /\
...
Proof.
  ...
  induction Hrel_subst; intros; subst.
    ...
    destruct EQ as [e [lgsubst0 [e' [lgsubst0' [lEnv1 [lEnv' [EQ1 [EQ2 [EQ3 [EQ4 [Wft [EQ5 [Typ [Typ' [Hrel Sub]]]]]]]]]]]]]]]; subst.
    exists e. exists lgsubst0.
    exists e'. exists lgsubst0'.
    exists lEnv1. exists lEnv'.
    simpl_env.
    ...
    split.
      ...
      rewrite <- subst_tt_fresh; auto.
        apply notin_fv_wf with (X:=X) in Wft; auto.
    split.
    destruct Hrel as [v [v' [Htv [Htv' [Henv [Henv' Hrel]]]]]].
    exists v. exists v'.
    split.
      simpl.
      rewrite <- subst_tt_fresh; auto.
    ...
    split; auto.
      ...
      apply Forel_weaken_head with (E:=E) ; auto.
    ...
    apply IHHrel_subst in EQ.
    destruct EQ as [e0 [lgsubst0 [e'0 [lgsubst0' [lEnv1 [lEnv' [EQ1 [EQ2 [EQ3 [EQ4 [Wft [EQ5 [Typ [Typ' [Hrel Sub]]]]]]]]]]]]]]]; subst.
...
Lemma Forel_stronger_head:  forall t E v v' rsubst dsubst dsubst' X R t2 t2' Env lEnv,
  ...
  wf_delta_osubst ([(X, bind_kn)]++E) ([(X, t2)]++dsubst) Env ->
  wf_delta_osubst ([(X, bind_kn)]++E) ([(X, t2')]++dsubst') Env ->
  F_Related_ovalues t rsubst dsubst dsubst' v v' Env lEnv
  .
Proof.
  intros t E v v' rsubst dsubst dsubst' X R t2 t2' Env lEnv Hrel HX Hwft2 Hwft2' Rsubst Dsubst Dsubst'.
  rewrite_env (nil ++ [(X, R)] ++ rsubst) in Hrel.
  rewrite_env (nil ++ [(X, t2)] ++ dsubst) in Hrel.
  rewrite_env (nil ++ [(X, t2')] ++ dsubst') in Hrel.
  apply Forel_stronger with (E:=E) (E':=nil) in Hrel; auto.
    inversion Dsubst; subst.
    apply dom_delta_osubst in H3. auto.
      ...
      apply one_eq_app in H2. destruct H2 as [[dsubst1 [dEQ1 dEQ2]] | [dEQ1 dEQ2]]; subst.
        apply one_eq_app in H5. destruct H5 as [[dsubst1' [dEQ1' dEQ2']] | [dEQ1' dEQ2']]; subst.
          apply one_eq_app in HeqEE. destruct HeqEE as [[E0' [eEQ1' eEQ2']] | [eEQ1' eEQ2']]; subst.
            ...
            apply Forel_stronger_head with (E:=E0'++E) in Hrel; auto.
               apply IHHwfr with (dsubst'0:=dsubst1') (dsubst0:=dsubst1) (rsubst':=rS0) (E':=E0'); auto.
                 ...
                 assert (X `notin` fv_env Env) as XnEnv.
                   apply wf_delta_osubst__uniq in H3. decompose [and] H3.
                   apply wfe_dom_fv_env; auto.
  ...
  Case "wf_rho_subst_srel".
    ...
    apply one_eq_app in HeqrS. destruct HeqrS as [[rS0 [rEQ1 rEQ2]] | [rEQ1 rEQ2]]; subst.
        ...
        apply one_eq_app in H5. destruct H5 as [[dsubst1' [dEQ1' dEQ2']] | [dEQ1' dEQ2']]; subst.
          apply one_eq_app in HeqEE. destruct HeqEE as [[E0' [eEQ1' eEQ2']] | [eEQ1' eEQ2']]; subst.
            assert (X `notin` fv_env (E0'++E)) as XnE.
              ...
              apply wf_rho_subst__uniq in Hwfr.
              decompose [and] Hwfr; auto.
            apply Forel_weaken_head with (E:=E0'++E); auto.
               apply IHHwfr with (dsubst'0:=dsubst1') (dsubst0:=dsubst1) (rsubst':=rS0) (E':=E0'); auto.
                 clear - Hdisj. 
                 simpl in Hdisj.
                 apply disjdom_cons_2 in Hdisj; auto.
               ...
               apply dom_delta_osubst in H7. auto.
      ...
      apply one_eq_app in HeqEE. destruct HeqEE as [[E0' [eEQ1' eEQ2']] | [eEQ1' eEQ2']]; subst.
        ...
        simpl in Hdomd0'. symmetry in Hdomd0'.  apply dom_empty_inv in Hdomd0'.
        subst. 
        simpl_env in H5. simpl_env in H2.
        subst.
        simpl_env. auto.
...
Lemma delta_osubst_opt' :  forall E E' dsubst dsubst' X t e Env,
...
Proof.
  intros E E' dsubst dsubst' X t e Env Hwf_delta_subst Heq Heq' Typ.
...
Definition P_Forel_typ_permute_renaming_one (n:nat) := 
  ((forall t v v' E1 E2 E3 rsubst1 rsubst2 rsubst3 dsubst1 dsubst2 dsubst3 dsubst1' dsubst2' dsubst3' X R t2 t2' Y Env lEnv,
  typ_size t = n ->
  ...
  wf_delta_osubst (E1++E2++[(X, bind_kn)]++E3) (dsubst1++dsubst2++[(X, t2)]++dsubst3) Env ->
  wf_delta_osubst (E1++E2++[(X, bind_kn)]++E3) (dsubst1'++dsubst2'++[(X, t2')]++dsubst3') Env  ->
  wf_delta_osubst (E1++[(Y, bind_kn)]++E2++E3) (dsubst1++[(Y, t2)]++dsubst2++dsubst3) Env  ->
  wf_delta_osubst (E1++[(Y, bind_kn)]++E2++E3) (dsubst1'++[(Y, t2')]++dsubst2'++dsubst3') Env  ->
  ...
  *
  (forall t v v' E1 E2 E3 (rsubst1 rsubst2 rsubst3:rho_subst) dsubst1 dsubst2 dsubst3 dsubst1' dsubst2' dsubst3' (X:atom) (R:rel) t2 t2' (Y:atom) Env lEnv,
  typ_size t = n ->
  (F_Related_ovalues (subst_tt X Y t) (rsubst1++[(Y, R)]++rsubst2++rsubst3) (dsubst1++[(Y, t2)]++dsubst2++dsubst3) (dsubst1'++[(Y, t2')]++dsubst2'++dsubst3') v v' Env lEnv ->
  ddom_env E1 [=] dom rsubst1 ->
  ddom_env E1 [=] dom dsubst1 ->
...
Proof.
  ...
  split.
  (* -> *)
  intros t v v' E1 E2 E3 rsubst1 rsubst2 rsubst3 dsubst1 dsubst2 dsubst3 dsubst1' dsubst2' dsubst3' X R t2 t2' Y Env lEnv
  ...
  repeat(split; simpl_env; auto).
     SCase "arrow".
     intros lEnv1 x x' Htypingx Htypingx' Wfle Harrow'.
     ...
     assert (Y `notin` dom lEnv1).
       apply in_dE__notin_lE with (E:=[(Y,bind_kn)]).
         simpl. auto.
     apply Hrel_wft1'_wft1 with (E1:=E1) (E2:=E2) (E3:=E3) in Hrel_wft1'; auto; simpl_env.
       ...
       destruct (@Harrow lEnv1 x x') as [u [u' [Hnorm_vxu [Hnorm_v'x'u' Hrel_wft1]]]]; auto.
         assert (ddom_env (E2++E3) [=] dom (dsubst2++dsubst3)) as EQ1.
           ...
           clear. fsetdec.
         assert (wf_typ Env t2) as Wft2.
           apply wfor_left_inv in WfR; auto.
         rewrite delta_osubst_opt with (Env:=Env) (E:=E2++E3) (E':=E1) in Htypingx; auto.
         ...
         rewrite delta_osubst_opt with (Env:=Env) (E:=E3) (E':=E1++E2); auto.
           simpl_env.
           rewrite subst_tt_tt' with (Env:=Env) in Htypingx; auto.
  ...
  repeat(split; simpl_env in *; auto).
     ...
     apply Hrel_wft1_wft1' with (X:=X)(Y:=Y)(R:=R)(t2:=t2)(t2':=t2')(E1:=E1)(E2:=E2)(E3:=E3) in Hrel_wft1; auto; simpl_env.
       ...
       destruct (@Harrow lEnv1 x x') as [u [u' [Hnorm_vxu [Hnorm_v'x'u' Hrel_wft1']]]]; auto.
         ...
         assert (wf_typ Env t2) as Wft2.
           apply wfor_left_inv in WfR; auto.
         rewrite delta_osubst_opt with (E:=E2++E3) (E':=E1) (Env:=Env); auto.
         assert (ddom_env (E1++E2) [=] dom (dsubst1++dsubst2)) as EQ2.
           simpl_env. rewrite Heqd1. rewrite Heqd2. 
           clear. fsetdec.
         ...
         assert (X `notin` dom (E1++E2) `union` dom E3 `union` dom Env) as Xn.
           ...
           clear - H3 H2.
           simpl_env. solve_uniq.
         rewrite delta_osubst_opt with (E:=E3) (E':=E1++E2) (Env:=Env) in Htypingx; auto.
           simpl_env  in Htypingx.
           rewrite subst_tt_tt' with (Env:=Env); auto.
...
Lemma Forel_typ_permute_renaming_one': forall t v v' E1 E2 E3 rsubst1 rsubst2 rsubst3 dsubst1 dsubst2 dsubst3 dsubst1' dsubst2' dsubst3' X R t2 t2' Y Env lEnv,
  F_Related_ovalues t (rsubst1++rsubst2++[(X, R)]++rsubst3) (dsubst1++dsubst2++[(X, t2)]++dsubst3) (dsubst1'++dsubst2'++[(X, t2')]++dsubst3') v v' Env lEnv ->
  ddom_env E1 [=] dom rsubst1 ->
  ddom_env E1 [=] dom dsubst1 ->
  ddom_env E1 [=] dom dsubst1' ->
...
Lemma Forel_typ_permute_renaming_one: forall t v v' E1 E2 rsubst1 rsubst2 dsubst1 dsubst2 dsubst1' dsubst2' X R t2 t2' Y Env lEnv,
  ...
  wf_delta_osubst (E1++[(X, bind_kn)]++E2) (dsubst1++[(X, t2)]++dsubst2) Env ->
  wf_delta_osubst (E1++[(X, bind_kn)]++E2) (dsubst1'++[(X, t2')]++dsubst2') Env ->
  wf_delta_osubst ([(Y, bind_kn)]++E1++E2) ([(Y, t2)]++dsubst1++dsubst2) Env ->
  wf_delta_osubst ([(Y, bind_kn)]++E1++E2) ([(Y, t2')]++dsubst1'++dsubst2') Env ->
...
Proof.
  ...
  rewrite_env (nil++[(Y, t2')]++dsubst1'++dsubst2').
  apply Forel_typ_permute_renaming_one' with (E1:=nil) (E2:=E1) (E3:=E2); auto.
Qed.
...
Lemma wf_olgsubst_kcons_inv : forall E X D dsubst gsubst lgsubst Env lEnv,
  wf_lgamma_osubst ([(X, bind_kn)]++E) D dsubst gsubst lgsubst Env lEnv ->
  X `notin` fv_lenv D ->
  exists T, exists dsubst', 
    ...
    wf_lgamma_osubst E D dsubst' gsubst lgsubst Env lEnv.
Proof.
  intros E X D dsubst gsubst lgsubst Env lEnv Wflg XnD.
  ...
  induction Wflg; intros; subst.
    ...
    split; auto.
      apply wf_lgamma_osubst_slval; auto.
        ...
        rewrite_env (nil ++ [(X, bind_kn)] ++E0) in H3.
        simpl_env in XnD.
        simpl in XnD.
...
Proof.
  ...
  induction Hrel_subst; intros; subst.
    ...
    destruct HeqDD as [[D1' [EQ1 EQ2]] | [EQ1 EQ2]]; subst.
      ...
      exists ([(x,e)]++lgsubst1). exists lgsubst2. exists ([(x,e')]++lgsubst1'). exists lgsubst2'. exists (lEnv2++lEnv3). exists lEnv4.
      simpl_env.
      split; auto.
      split; auto.
      split; auto.
      ...
      exists nil. exists ([(x,e)]++lgsubst). exists nil. exists ([(x,e')]++lgsubst'). exists nil. exists (lEnv2++lEnv1).
      simpl_env.
      split; auto.
      split; auto.
      split; auto.
...
Lemma F_Related_osubst_lgweaken : forall D1 E D2 gsubst gsubst' lgsubst1 lgsubst2 lgsubst1' lgsubst2' rsubst dsubst dsubst' x T e e' Env lEnv1 lEnv2 lEnv,
  ...
  typing Env lEnv e' (apply_delta_subst_typ dsubst' T) ->
  F_Related_oterms T rsubst dsubst dsubst' e e' Env lEnv ->
...
Lemma F_Related_osubst_dweaken : forall E1 E2 D gsubst gsubst' lgsubst lgsubst' rsubst1 rsubst2 dsubst1 dsubst2 dsubst1' dsubst2' X t t' R Env lEnv,
  ...
  wfor Env R t t' ->
  F_Related_osubst (E1++[(X, bind_kn)]++E2) D gsubst gsubst' lgsubst lgsubst' (rsubst1++[(X, R)]++rsubst2) (dsubst1++[(X, t)]++dsubst2) (dsubst1'++[(X, t')]++dsubst2') Env lEnv.
Proof.
  intros E1 E2 D gsubst gsubst' lgsubst lgsubst' rsubst1 rsubst2 dsubst1 dsubst2 dsubst1' dsubst2' X t t' R Env lEnv
  ...
  Case "F_Related_osubst_typ".
    ...
    apply one_eq_app in HeqE. destruct HeqE as [[E0' [eEQ1' eEQ2']] | [eEQ1' eEQ2']]; subst.
      ...
      simpl_env.
      apply F_Related_osubst_typ; auto.
        rewrite delta_osubst_opt with (E:=E2) (E':=E0') (Env:=Env); eauto.
          rewrite <- subst_tt_fresh; auto.
        ...
        split; auto.
          apply F_Related_osubst__inversion in Hrel_subst.
          decompose [prod] Hrel_subst.
          apply Forel_weaken with (E:=E2) (E':=E0'); auto.
            assert (X `notin` fv_env (E0'++E2)) as XnE.
              apply wfe_dom_fv_env; auto.
  ...
  Case "F_Related_osubst_ltyp".
    simpl_env in *.
    apply F_Related_osubst_ltyp; auto.
      rewrite delta_osubst_opt with (E:=E2) (E':=E1) (Env:=Env); eauto.
        rewrite <- subst_tt_fresh; auto.
      ...
      destruct H3 as [v [v' [Htv [Htv' [Henv [He'nv' H3]]]]]].
      exists v. exists v'.
      split.
        rewrite delta_osubst_opt with (E:=E2) (E':=E1) (Env:=Env); eauto.
          rewrite <- subst_tt_fresh; auto.
      ...
      split; auto.
        apply F_Related_osubst__inversion in Hrel_subst.
        decompose [prod] Hrel_subst.
        apply Forel_weaken with (E:=E2) (E':=E1); auto.
...
Lemma F_Related_osubst_gweaken : forall E1 E2 D gsubst1 gsubst2 gsubst1' gsubst2' lgsubst lgsubst' rsubst1 rsubst2 dsubst1 dsubst2 dsubst1' dsubst2' x T e e' Env lEnv,
  ...
  typing Env nil e' (apply_delta_subst_typ dsubst2' T) ->
  F_Related_oterms T rsubst2 dsubst2 dsubst2' e e' Env nil ->
...
Lemma F_Rosubst_dweaken : forall E1 E2 rsubst1 rsubst2 dsubst1 dsubst2 dsubst1' dsubst2' X R t2 t2' Env,
  ...
  X `notin` (dom E1 `union` dom E2 `union` fv_env Env)->
  wfor Env R t2 t2' ->
  F_Rosubst (E1++[(X, bind_kn)]++E2) (rsubst1++[(X, R)]++rsubst2) (dsubst1++[(X, t2)]++dsubst2) (dsubst1'++[(X, t2')]++dsubst2') Env.
Proof.
  intros E1 E2 rsubst1 rsubst2 dsubst1 dsubst2 dsubst1' dsubst2' X R t2 t2' Env HRsubst 
...
Lemma commut_lgamma_osubst_close_ee: forall E D dsubst gsubst lgsubst Env lEnv y e,
   wf_lgamma_osubst E D dsubst gsubst lgsubst Env lEnv ->
   y `notin` dom lgsubst `union` dom Env `union` dom lEnv ->
   apply_gamma_subst lgsubst (close_ee e y)
= close_ee (apply_gamma_subst lgsubst e) y.
Proof.
  intros E D dsubst gsubst lgsubst Env lEnv y e Hwfg Hfv.
  generalize dependent y.
  generalize dependent e.
  induction Hwfg; intros; simpl; auto.
    rewrite subst_ee_close_ee; auto.
      apply notin_fv_ee_typing with (y:=y) in H2; auto.
Qed.

Lemma commut_gamma_osubst_close_ee: forall E D dsubst gsubst lgsubst Env lEnv y e,
   wf_lgamma_osubst E D dsubst gsubst lgsubst Env lEnv ->
   y `notin` dom gsubst `union` dom Env ->
...
Proof.
  ...
  induction Hwfg; intros; simpl; auto.
    rewrite subst_ee_close_ee; auto.
      apply notin_fv_ee_typing with (y:=y) in H0; auto.
Qed.

Lemma commut_delta_osubst_close_ee: forall dE dsubst Env y e,
   wf_delta_osubst dE dsubst Env ->
   y `notin` dom dsubst `union` dom Env ->
   apply_delta_subst dsubst (close_ee e y)
= close_ee (apply_delta_subst dsubst e) y.
Proof.
  intros dE dsubst Env y e Hwfd.
  generalize dependent e.
  generalize dependent y.
...
Lemma odsubst__preserves__in_fv_ee : forall E dsubst e x Env,
  wf_delta_osubst E dsubst Env ->
  x `in` fv_ee e ->
  x `notin` dom dsubst ->
  x `in` fv_ee (apply_delta_subst dsubst e).
Proof.
  intros E dsubst e x Env H.
  induction H; intros; simpl; auto.
    rewrite <- swap_subst_te_odsubst with (E:=E) (Env:=Env); auto.
      assert (J:=@subst_te_fv_ee_eq (apply_delta_subst SE e) X T).
      apply IHwf_delta_osubst in H2;auto.
      fsetdec.
...
Lemma ogsubst__preserves__in_fv_ee : forall E D dsubst gsubst lgsubst e x Env lEnv,
  wf_lgamma_osubst E D dsubst gsubst lgsubst Env lEnv ->
  x `in` fv_ee e ->
  x `notin` dom gsubst ->
  x `in` fv_ee (apply_gamma_subst gsubst e).
Proof.
  intros E D dsubst gsubst lgsubst e x Env lEnv H.
  generalize dependent x.
  induction H; intros; simpl; auto.
    rewrite <- swap_subst_ee_ogsubst with (E:=E)(D:=lE)(lgsubst:=lgsE)(dsubst:=dsE)(t:=apply_delta_subst_typ dsE T)(Env:=Env)(lEnv:=lEnv)(lEnv':=lempty); auto.
      destruct (@in_dec x (fv_ee (apply_gamma_subst gsE e))) as [J | J].
        assert (J':=@subst_ee_fv_ee_in (apply_gamma_subst gsE e) x e0 J).
        apply IHwf_lgamma_osubst in H3; auto.
        apply typing_fv_ee_upper in H1. simpl in H1.
        rewrite J'. simpl_env in H4. fsetdec.
...
Lemma olgsubst__preserves__in_fv_ee : forall E D dsubst gsubst lgsubst e x Env lEnv,
  wf_lgamma_osubst E D dsubst gsubst lgsubst Env lEnv ->
  x `in` fv_ee e ->
  x `notin` dom lgsubst ->
...
Proof.
  ...
  induction H; intros; simpl; auto.
    ...
    rewrite <- swap_subst_ee_olgsubst with (E:=E)(D:=lE)(gsubst:=gsE)(dsubst:=dsE)(t:=apply_delta_subst_typ dsE T)(Env:=Env)(lEnv:=lEnv1)(lEnv':=lEnv2); auto.
      destruct (@in_dec x (fv_ee (apply_gamma_subst lgsE e))) as [J | J].
        assert (J':=@subst_ee_fv_ee_in (apply_gamma_subst lgsE e) x e0 J).
        apply IHwf_lgamma_osubst in H5; auto.
        apply typing_fv_ee_upper in H3. simpl in H3.
        rewrite J'. simpl_env in H6. fsetdec.
...
Lemma odsubst__preserves__notin_fv_ee : forall E dsubst e x Env,
  wf_delta_osubst E dsubst Env ->
  x `notin` fv_ee e ->
...
Proof.
  ...
  induction H; intros; simpl; auto.
    rewrite <- swap_subst_te_odsubst with (E:=E)(Env:=Env); auto.
      assert (J:=@subst_te_fv_ee_eq (apply_delta_subst SE e) X T).
      apply IHwf_delta_osubst in H2. fsetdec.
...
Lemma ogsubst__preserves__notin_fv_ee : forall E D dsubst gsubst lgsubst e x Env lEnv,
  wf_lgamma_osubst E D dsubst gsubst lgsubst Env lEnv ->
  x `notin` fv_ee e ->
  x`notin` dom Env `union` dom lEnv ->
  x `notin` fv_ee (apply_gamma_subst gsubst e).
Proof.
  intros E D dsubst gsubst lgsubst e x Env lEnv H H'.
  induction H; intros; simpl; auto.
    rewrite <- swap_subst_ee_ogsubst with (E:=E)(D:=lE)(lgsubst:=lgsE)(dsubst:=dsE)(t:=apply_delta_subst_typ dsE T)(Env:=Env)(lEnv:=lEnv)(lEnv':=lempty); auto.
      assert (J:=@subst_ee_fv_ee_sub (apply_gamma_subst gsE e) x0 e0).
      assert (J':=H3).
      apply IHwf_lgamma_osubst in H3; auto.
      apply typing_fv_ee_upper in H1. simpl in H3.
      fsetdec.

      apply wf_lgamma_osubst__nfv with (x:=x0) in H; auto.
Qed.

Lemma olgsubst__preserves__notin_fv_ee : forall E D dsubst gsubst lgsubst e x Env lEnv,
  wf_lgamma_osubst E D dsubst gsubst lgsubst Env lEnv ->
  x `notin` fv_ee e ->
  x `notin` dom Env `union` dom lEnv ->
...
Proof.
  ...
  induction H; intros; simpl; auto.
    ...
    rewrite <- swap_subst_ee_olgsubst with (E:=E)(D:=lE)(gsubst:=gsE)(dsubst:=dsE)(t:=apply_delta_subst_typ dsE T)(Env:=Env)(lEnv:=lEnv1)(lEnv':=lEnv2); auto.
      ...
      assert (x `notin` dom Env `union` dom lEnv1) as J'. auto.
      apply IHwf_lgamma_osubst in J'.
      apply typing_fv_ee_upper in H3. simpl_env in *.
      clear - J' J H5 H3. fsetdec.
