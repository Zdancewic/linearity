...
Lemma m_typing_osubst_kind_closed : forall E lE dsubst gsubst lgsubst Env lEnv e1 (X:atom) T1,
  wf_delta_osubst E dsubst Env ->
  wf_lgamma_osubst E lE dsubst gsubst lgsubst Env lEnv ->
  X `notin` dom dsubst `union` dom Env `union` dom lEnv ->
  wf_env E ->
  typing ([(X, bind_kn)] ++ E) lE (open_te e1 X) (open_tt T1 X) ->
  typing ([(X, bind_kn)] ++ Env) lEnv
             (open_te (apply_delta_subst dsubst (apply_gamma_subst gsubst (apply_gamma_subst lgsubst e1))) X) 
             (open_tt (apply_delta_subst_typ dsubst T1) X).
Proof.
  intros E lE dsubst gsubst lgsubst Env lEnv e1 X T1 Hwfd Hwfg Hfv Henv Htyping.
  rewrite_env (nil++lE) in Htyping.
  assert (wf_lenv Env lEnv) as Wfle.
    apply wf_lgamma_osubst__wf_lenv in Hwfg. decompose [and] Hwfg; auto.
  assert (wf_lenv (apply_delta_subst_env dsubst [(X, bind_kn)]++Env) lEnv) as wfle.
     simpl. simpl_env. 
     rewrite_env(nil ++ [(X, bind_kn)]++Env). 
     apply wf_lenv_weakening; auto.
       simpl_env. apply wf_env_kn; auto.
  apply typing_osubst_closed with (E:=E) (E':=[(X, bind_kn)]) (dsubst:=dsubst) (gsubst:=gsubst) (lgsubst:=lgsubst)(Env:=Env)(lEnv:=lEnv)in Htyping; auto.
...
Lemma m_red_abs_osubst : forall E lE dsubst gsubst lgsubst Env lEnv V e1 x (y:atom) L T1,
  wf_lgamma_osubst E lE dsubst gsubst lgsubst Env lEnv ->
  wf_typ E V ->
  ...
  red (exp_app
                  (apply_delta_subst dsubst (apply_gamma_subst gsubst (apply_gamma_subst lgsubst (exp_abs V e1)))) x)
                  (apply_delta_subst dsubst (apply_gamma_subst gsubst (apply_gamma_subst lgsubst (subst_ee y x (open_ee e1 y))))).
Proof.
  intros E lE dsubst gsubst lgsubst Env lEnv V e1 x y L T1 Hwfg Hwft Htyping Hfvy Hfr.
...
Lemma m_red_labs_osubst : forall E lE dsubst gsubst lgsubst Env lEnv lEnv' V e1 x (y:atom) L T1,
  wf_lgamma_osubst E lE dsubst gsubst lgsubst Env lEnv ->
  wf_typ E V ->
  ...
  red (exp_app
                  ...
                  (apply_delta_subst dsubst (apply_gamma_subst gsubst (apply_gamma_subst lgsubst (subst_ee y x (open_ee e1 y))))).
Proof.
  intros E lE dsubst gsubst lgsubst Env lEnv lEnv' V e1 x y L T1 Hwfg Hwft [Disj1 Disj2] Htyping Hfvy Hfr.
  rewrite <- subst_ee_intro; auto.
  assert (apply_delta_subst dsubst (apply_gamma_subst gsubst (apply_gamma_subst lgsubst x)) =x) as Heq1.
    apply disjoint_lgamma_osubst in Hwfg. destruct Hwfg as [J1 [J2 [J3 [J4 [J5 [J6 [J7 [J8 [J9 J10]]]]]]]]].
     rewrite gamma_osubst_closed_exp; auto.
...
Lemma m_red_tabs_osubst : forall E lE dsubst gsubst lgsubst Env lEnv e1 (X:atom) t2 L T1,
  wf_lgamma_osubst E lE dsubst gsubst lgsubst Env lEnv ->
  wf_typ Env t2 ->
  X `notin` fv_te e1 ->
  (forall X : atom,  X `notin` L -> typing ([(X, bind_kn)] ++ E) lE (open_te e1 X) (open_tt T1 X)) ->
  red (exp_tapp
                 (apply_delta_subst dsubst (apply_gamma_subst gsubst (apply_gamma_subst lgsubst (exp_tabs e1)))) t2)
                 (apply_delta_subst dsubst (apply_gamma_subst gsubst (apply_gamma_subst lgsubst (subst_te X t2 (open_te e1 X))))). 
Proof.
  intros E lE dsubst gsubst lgsubst Env lEnv e1 X t2 L T1 Hwfg Hwfor Hfv Hfr.
...
Lemma m_red_bang_osubst : forall E D2 dsubst gsubst lgsubst2 Env lEnv2 T1 e2 x1 (y:atom) L T2,
  wf_lgamma_osubst E D2 dsubst gsubst lgsubst2 Env lEnv2 ->
  wf_typ E T1 ->
  typing Env nil x1 (apply_delta_subst_typ dsubst T1) ->
  y `notin` fv_ee e2 ->
  (forall x : atom, x `notin` L -> typing ([(x, bind_typ T1)] ++ E) D2 (open_ee e2 x) T2) ->
  red (exp_let (exp_bang x1) (apply_delta_subst dsubst (apply_gamma_subst gsubst (apply_gamma_subst lgsubst2 e2))))
                  (apply_delta_subst dsubst (apply_gamma_subst gsubst (apply_gamma_subst lgsubst2 (subst_ee y x1 (open_ee e2 y))))).
Proof.
  intros E D2 dsubst gsubst lgsubst2 Env lEnv2 T1 e2 x1 y L T2 Hwfg2 Hwft1 Htyping2 Hfvy Hfr.
  rewrite <- subst_ee_intro; auto.
  assert (apply_delta_subst dsubst (apply_gamma_subst gsubst (apply_gamma_subst lgsubst2 x1)) =x1) as Heq1.
     apply disjoint_lgamma_osubst in Hwfg2. destruct Hwfg2 as [J1 [J2 [J3 [J4 [J5 [J6 [J7 [J8 [J9 J10]]]]]]]]].
     rewrite gamma_osubst_closed_exp; auto.
       rewrite gamma_osubst_closed_exp; auto.
         rewrite delta_osubst_closed_exp; auto.
           split;intros x0 x0in.
             apply in_fv_te_typing with (X:=x0) in Htyping2; auto.
             clear - J3 Htyping2. solve_uniq.

             apply notin_fv_te_typing with (X:=x0) in Htyping2; auto.
             clear - J3 x0in. solve_uniq.
         split; intros x0 x0in.
           apply in_fv_ee_typing with (x:=x0) in Htyping2; auto.
           clear - J5 Htyping2. solve_uniq.

           apply notin_fv_ee_typing with (y:=x0) in Htyping2; auto.
           clear - x0in J5. solve_uniq.
       rewrite gamma_osubst_closed_exp; auto.
         split; intros x0 x0in.
           apply in_fv_ee_typing with (x:=x0) in Htyping2; auto.
           clear - Htyping2 J4. solve_uniq.

           apply notin_fv_ee_typing with (y:=x0) in Htyping2; auto.
           clear - x0in J4. solve_uniq.

          split; intros x0 x0in.
            apply in_fv_ee_typing with (x:=x0) in Htyping2; auto.
            clear - Htyping2 J5 . solve_uniq.
  ...
  assert (open_ee e2 (apply_delta_subst dsubst (apply_gamma_subst gsubst  (apply_gamma_subst lgsubst2 x1))) = open_ee e2 x1) as Heq2. 
     rewrite Heq1. auto. 
  rewrite Heq2.
  rewrite commut_lgamma_osubst_open_ee with (E:=E)(D:=D2)(dsubst:=dsubst)(gsubst:=gsubst)(Env:=Env)(lEnv:=lEnv2); auto.
  rewrite commut_gamma_osubst_open_ee with (E:=E) (dsubst:=dsubst) (D:=D2) (lgsubst:=lgsubst2)(Env:=Env)(lEnv:=lEnv2); auto.
  apply red_bang_preserved_under_delta_osubst with (dE:=E)(Env:=Env); auto.
    apply wf_lgamma_osubst__wf_osubst in Hwfg2. destruct Hwfg2; auto.
  rewrite <- commut_gamma_subst_bang; auto.
  rewrite <- commut_gamma_osubst_open_ee with (E:=E) (dsubst:=dsubst) (D:=D2) (lgsubst:=lgsubst2)(Env:=Env)(lEnv:=lEnv2); auto.
  apply red_bang_preserved_under_gamma_osubst with (E:=E) (dsubst:=dsubst) (D:=D2) (lgsubst:=lgsubst2)(Env:=Env)(lEnv:=lEnv2); auto. 

  rewrite <- commut_gamma_subst_bang; auto.
  rewrite <- commut_lgamma_osubst_open_ee with (E:=E)(D:=D2)(dsubst:=dsubst)(gsubst:=gsubst)(Env:=Env)(lEnv:=lEnv2); auto.
  apply red_bang_preserved_under_lgamma_osubst with (E:=E)(D:=D2)(dsubst:=dsubst)(gsubst:=gsubst)(Env:=Env)(lEnv:=lEnv2); auto. 
...
Lemma m_typing_ovar_stronger : 
  forall (E E' : env) (dsubst : delta_subst) (X : atom) x k t1 t2 (dsubst' : delta_subst) Env lEnv,
  wf_delta_osubst (E' ++ [(X, bind_kn)] ++ E) (dsubst' ++ [(X, (apply_delta_subst_typ (dsubst' ++ dsubst) t2))]  ++ dsubst) Env ->
  X `notin` (fv_env E) `union` (fv_env E') `union` (fv_env Env)->
  X `notin` fv_tt t1 ->
  ddom_env E[=]dom dsubst ->
  ddom_env E'[=]dom dsubst' ->
  wf_typ Env (apply_delta_subst_typ (dsubst' ++ dsubst) t2) ->
  wf_typ (E'++E) t2 ->
  ...
  typing Env lEnv x
         (apply_delta_subst_typ
            ...
            (open_tt_rec k X t1)).
Proof.
  intros E E' dsubst X e k t1 t2 dsubst' Env lEnv Hwfd Hfv Hfvt1 Heq Heq' Hwft2 Hwft Htyping.
  rewrite delta_osubst_opt with (E:=E) (E':=E')(Env:=Env); auto.
  rewrite swap_subst_tt_odsubst with (E:=E'++E)(Env:=Env); simpl_env; auto.
...
Proof.
  intros E E' dsubst X e k t1 t2 dsubst' Env lEnv Hwfd Hfv Hfvt1 Heq Heq' Hwft2 Hwft Htyping.
  rewrite delta_osubst_opt with (E:=E) (E':=E') (Env:=Env) in Htyping; eauto.
  rewrite swap_subst_tt_odsubst with (E:=E'++E) (Env:=Env) in Htyping; simpl_env; eauto.
...
Lemma m_ocongr_tapp : forall E lE dsubst gsubst lgsubst Env lEnv T2 e1 v u, 
   wf_delta_osubst E dsubst Env ->
   wf_lgamma_osubst E lE dsubst gsubst lgsubst Env lEnv ->
   wf_typ E T2 ->
...
Proof.
   intros E lE dsubst gsubst lgsubst Env lEnv T2 e1 v u Hwfd Hwfg Hwft He1_v Hvt1_u.
...
Lemma m_tapp_ofv : forall E dsubst dsubst' Env T2 e e' v v' u u' T1 y lEnv,
   wf_env E ->
   wf_delta_osubst E dsubst Env ->
   wf_delta_osubst E dsubst' Env ->
   wf_typ E T2 ->
...
Proof.
  intros E dsubst dsubst' Env T2 e e' v v' u u' T1 y lEnv Hwfe Hwfd Hwfd' Hwft He_v He'_v' Hvt2_u Hv't2_u' Hfv.
...
Lemma m_all_ofv : forall t3 t2' e e' X t v v' (X4:atom) E E' Env lEnv, 
  wf_typ Env t3 ->
  wf_typ Env t2' ->
...
Proof.
  intros t3 t2' e e' X t v v' X4 E E' Env lEnv Hwft3 Hwft2' Het3_v He't2'_v' Hfv.
...
Lemma m_delta_osubst_opt : forall (E E' : env) (dsubst : delta_subst) (X : atom)
         (t : delta_binding) (t' : typ) (dsubst' : delta_subst) Env lEnv e t2,
  wf_delta_osubst (E' ++ E) (dsubst' ++ dsubst) Env ->
...
Proof.
  intros.
  rewrite delta_osubst_opt with (E:=E) (E':=E')(Env:=Env); eauto.
     simpl. destruct (X==X).
