...
Lemma notin_lE__in_dE : forall X E (lE:lenv),
  X `in` dom lE ->
  X `notin` ddom_env E.
Proof.
  intros.
...
Lemma dE_lE_disjoint : forall E (lE:lenv),
  disjdom (ddom_env E) (dom lE).
Proof.
  intros.
  split; intros.
    apply in_dE__notin_lE with (E:=E); auto.
    apply notin_lE__in_dE with (lE:=lE); auto.
Qed.

Lemma lEnv_E_disjoint : forall (lEnv:lenv) (E:env) (gsubst:gamma_subst),
  gdom_env E [=] dom gsubst ->
  disjoint lEnv E.
Proof.
  intros lEnv E gsubst EQ.
  apply disjdom__disjoint.
    apply disjdom_sym_1.
    apply disjdom_eq with (D1:=gdom_env E `union` ddom_env E).
      apply disjdom_app_3.
        apply disjdom_eq with (D1:=dom gsubst).
          apply disjoint__disjdom.
          assert (J:=@disjoint_termvars lEnv gsubst); auto.
          fsetdec.
        apply dE_lE_disjoint.
      rewrite dom__ddom_gdom. fsetdec.
Qed.

Lemma lEnv_lE_disjoint : forall (lEnv lE:lenv) (lgsubst:gamma_subst),
  dom lE [=] dom lgsubst ->
  disjoint lEnv lE.
Proof.
  intros lEnv lE lgsubst EQ.
  apply disjdom__disjoint.
    apply disjdom_sym_1.
    apply disjdom_eq with (D1:=dom lgsubst).
      apply disjoint__disjdom.
        assert (J:=@disjoint_termvars lEnv lgsubst); auto.
        fsetdec.
Qed.
...
Proof.
  ...
  repeat(split; auto).
     SCase "arrow".
     intros lEnv1 x x' Htypingx Htypingx' Hwfle Harrow'.
     ...
     assert (X `notin` fv_lenv lEnv1) as XnotinlEnv1.
       assert (X `notin` dom lEnv1) as J. 
         apply in_dE__notin_lE with (E:=[(X,bind_kn)]).
           simpl. auto.
...
Proof.
  ...
  split. apply wft_osubst with (E:=E); auto.
         apply wft_osubst with (E:=E); auto.
Qed.
             ...
             assert (X4 `notin` fv_lenv lEnv') as X4notinlEnv'.             
               assert (X4 `notin` dom Env `union` dom lEnv') as J.
                 assert (X4 `notin` dom lEnv').
                   apply in_dE__notin_lE with (E:=[(X4, bind_kn)]); simpl; auto.
                 auto.
             ...
             assert (X2 `notin` fv_lenv lEnv') as X2notinlEnv'.
               assert (X2 `notin` dom Env `union` dom lEnv') as J.
                 assert (X2 `notin` dom lEnv').
                   apply in_dE__notin_lE with (E:=[(X2, bind_kn)]); simpl; auto.
                 auto.
               apply typing_regular in Htypingv0.
               decompose [and] Htypingv0.
               apply wf_lenv_fv_lenv_sub in H2.
...
Proof.
  ...
  Case "typing_abs".
    ...
    split; try solve [split; try solve [apply delta_gamma_lgamma_osubst_value with (E:=E) (D:=D) (Env:=Env) (lEnv:=lEnv); eauto using FrTyping__labsvalue | auto]].
      ...
      split; try solve [apply delta_gamma_lgamma_osubst_value with (E:=E) (D:=D) (Env:=Env) (lEnv:=lEnv); eauto using FrTyping__labsvalue].
      SSCase "arrow".
        intros lEnv1 x x' Htyping Htyping' Hwfle Harrow_left.
        ...
        assert (disjoint lEnv1 gsubst /\ disjoint lEnv1 lgsubst /\ disjoint lEnv1 gsubst' /\ disjoint lEnv1 lgsubst' /\ disjoint lEnv1 E /\ disjoint lEnv1 D) as Disj.
          split. apply disjoint_termvars.
          split. apply disjoint_termvars.
          split. apply disjoint_termvars.
          split. apply disjoint_termvars.
          split. apply lEnv_E_disjoint with (gsubst:=gsubst).
                   apply dom_lgamma_osubst in Hwflg.
                   decompose [and] Hwflg; auto.
                 apply lEnv_lE_disjoint with (lgsubst:=lgsubst).
                   apply dom_lgamma_osubst in Hwflg.
                   decompose [and] Hwflg; auto.
        pick fresh y.
  ...
  Case "typing_app".
   ...
   destruct (@Harrow lEnv2 (apply_delta_subst dsubst (apply_gamma_subst gsubst (apply_gamma_subst lgsubst2 e2)))
                           (apply_delta_subst dsubst' (apply_gamma_subst gsubst'(apply_gamma_subst lgsubst2' e2)))) 
