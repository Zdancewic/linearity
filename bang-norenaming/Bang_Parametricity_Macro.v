...
Require Export Bang_Parametricity_Pre.
...
Lemma m_type_preserved_under_delta_gamma_subst : 
  forall (E : env) lE (dsubst : delta_subst) (gsubst lgsubst: gamma_subst) (t : typ),
  wf_lgamma_subst E lE dsubst gsubst lgsubst ->
  wf_typ E t ->
  type (apply_delta_subst_typ dsubst (apply_gamma_subst_typ gsubst (apply_gamma_subst_typ lgsubst t))).
Proof.
  intros E lE dsubst gsubst lgsubst t Hwfg Hwft.
...
Lemma m_red_abs_subst : forall E lE dsubst gsubst lgsubst V e1 x (y:atom) L T1,
  wf_lgamma_subst E lE dsubst gsubst lgsubst ->
  wf_typ E V ->
...
Proof.
  intros E lE dsubst gsubst lgsubst V e1 x y L T1 Hwfg Hwft Htyping Hfvy Hfr.
...
Lemma m_red_labs_subst : forall E lE dsubst gsubst lgsubst V e1 x (y:atom) L T1,
  wf_lgamma_subst E lE dsubst gsubst lgsubst ->
  wf_typ E V ->
  typing nil nil x (apply_delta_subst_typ dsubst V) ->
  y `notin` fv_ee e1 ->
...
Lemma m_red_tabs_subst : forall E lE dsubst gsubst lgsubst e1 (X:atom) t2 L T1,
  wf_lgamma_subst E lE dsubst gsubst lgsubst ->
  wf_typ nil t2 ->
  X `notin` fv_te e1 ->
...
Proof.
  intros E lE dsubst gsubst lgsubst e1 X t2 L T1 Hwfg Hwfr Hfv Hfr.
...
Lemma m_red_bang_subst : forall E D2 dsubst gsubst lgsubst2 T1 e2 x1 (y:atom) L T2,
  wf_lgamma_subst E D2 dsubst gsubst lgsubst2 ->
  wf_typ E T1 ->
  typing nil nil x1 (apply_delta_subst_typ dsubst T1) ->
  y `notin` fv_ee e2 ->
...
Proof.
  intros E D2 dsubst gsubst lgsubst2 T1 e2 x1 y L T2 Hwfg2 Hwft1 Htyping2 Hfvy Hfr.
  rewrite <- subst_ee_intro; auto.
  ...
  assert (open_ee e2 (apply_delta_subst dsubst (apply_gamma_subst gsubst  (apply_gamma_subst lgsubst2 x1))) = open_ee e2 x1) as Heq2. 
     rewrite Heq1. auto. 
  rewrite Heq2.
  rewrite commut_lgamma_subst_open_ee with (E:=E)(D:=D2)(dsubst:=dsubst)(gsubst:=gsubst); auto.
  rewrite commut_gamma_subst_open_ee with (E:=E) (dsubst:=dsubst) (D:=D2) (lgsubst:=lgsubst2); auto.
  apply red_bang_preserved_under_delta_subst with (dE:=E); auto.
    apply wf_lgamma_subst__wf_subst in Hwfg2. destruct Hwfg2; auto.
  rewrite <- commut_gamma_subst_bang; auto.
  rewrite <- commut_gamma_subst_open_ee with (E:=E) (dsubst:=dsubst) (D:=D2) (lgsubst:=lgsubst2); auto.
  apply red_bang_preserved_under_gamma_subst with (E:=E) (dsubst:=dsubst) (D:=D2) (lgsubst:=lgsubst2); auto. 

  rewrite <- commut_gamma_subst_bang; auto.
  rewrite <- commut_lgamma_subst_open_ee with (E:=E)(D:=D2)(dsubst:=dsubst)(gsubst:=gsubst); auto.
  apply red_bang_preserved_under_lgamma_subst with (E:=E)(D:=D2)(dsubst:=dsubst)(gsubst:=gsubst); auto. 
...
Lemma m_typing_arrow_stronger : 
  forall (E E' : env) (dsubst : delta_subst) (X : atom) e k t1 t2 t3 (dsubst' : delta_subst),
  wf_delta_subst (E' ++ [(X, bind_kn)] ++ E) (dsubst' ++ [(X, (apply_delta_subst_typ (dsubst' ++ dsubst) t2))]  ++ dsubst) ->
  X `notin` union (fv_env E) (fv_env E') ->
  X `notin` fv_tt t1 ->
  X `notin` fv_tt t3 ->
  ddom_env E[=]dom dsubst ->
  ddom_env E'[=]dom dsubst' ->
  wf_typ nil (apply_delta_subst_typ (dsubst' ++ dsubst) t2) ->
  wf_typ (E'++E) t2 ->
  typing nil nil e
         (apply_delta_subst_typ
            (dsubst' ++
             [(X, apply_delta_subst_typ (dsubst' ++ dsubst) t2)] ++ dsubst)
            (typ_arrow (open_tt_rec k X t1) (open_tt_rec k X t3))) ->
  typing nil nil e (apply_delta_subst_typ (dsubst'++dsubst) (typ_arrow (open_tt_rec k t2 t1) (open_tt_rec k t2 t3))).
Proof. 
  intros E E' dsubst X e k t1 t2 t3 dsubst' Hwfd Hfv Hfvt1 Hfvt3 Heq Heq' Hwft2 Hwft Htyping.
...
Lemma m_typing_arrow_weaken : 
  ...
  typing nil nil e
         (apply_delta_subst_typ
            (dsubst' ++ dsubst)
            (typ_arrow (open_tt_rec k t2 t1) (open_tt_rec k t2 t3))) ->
  typing nil nil e (apply_delta_subst_typ (dsubst'++[(X, apply_delta_subst_typ (dsubst' ++ dsubst) t2)]++dsubst) (typ_arrow (open_tt_rec k X t1) (open_tt_rec k X t3))).
Proof.
  intros E E' dsubst X e k t1 t2 t3 dsubst' Hwfd Hfv Hfvt1 Hfvt3 Heq Heq' Hwft2 Hwft Htyping.
...
Lemma m_typing_var_stronger : 
  forall (E E' : env) (dsubst : delta_subst) (X : atom) x k t1 t2 (dsubst' : delta_subst),
  wf_delta_subst (E' ++ [(X, bind_kn)] ++ E) (dsubst' ++ [(X, (apply_delta_subst_typ (dsubst' ++ dsubst) t2))]  ++ dsubst) ->
  X `notin` union (fv_env E) (fv_env E') ->
  X `notin` fv_tt t1 ->
  ddom_env E[=]dom dsubst ->
  ...
  typing nil nil x
         (apply_delta_subst_typ
            ...
            (open_tt_rec k X t1)).
Proof.
  intros E E' dsubst X e k t1 t2 dsubst' Hwfd Hfv Hfvt1 Heq Heq' Hwft2 Hwft Htyping.
  erewrite delta_subst_opt with (E:=E) (E':=E'); eauto.
  erewrite swap_subst_tt_dsubst with (E:=E'++E); simpl_env; eauto.
...
Lemma m_typing_all_stronger : 
  forall (E E' : env) (dsubst : delta_subst) (X : atom) e k t t2 (dsubst' : delta_subst),
  wf_delta_subst (E' ++ [(X, bind_kn)] ++ E) (dsubst' ++ [(X, (apply_delta_subst_typ (dsubst' ++ dsubst) t2))]  ++ dsubst) ->
  X `notin` union (fv_env E) (fv_env E') ->
  X `notin` fv_tt t ->
  ddom_env E[=]dom dsubst ->
  ...
  typing nil nil e
         (apply_delta_subst_typ
            (dsubst' ++
             [(X, apply_delta_subst_typ (dsubst' ++ dsubst) t2)] ++ dsubst)
            (typ_all (open_tt_rec (S k) X t))) ->
  typing nil nil e (apply_delta_subst_typ (dsubst'++dsubst) (typ_all (open_tt_rec (S k) t2 t))).
Proof.
  intros E E' dsubst X e k t t2 dsubst' Hwfd Hfv Hfvt Heq Heq' Hwft2 Hwft Htyping.
...
Lemma m_typing_subst_kind_closed : forall E lE dsubst gsubst lgsubst e1 (X:atom) T1,
  wf_delta_subst E dsubst ->
  wf_lgamma_subst E lE dsubst gsubst lgsubst ->
  X `notin` dom dsubst ->
  wf_env E ->
  typing ([(X, bind_kn)] ++ E) lE (open_te e1 X) (open_tt T1 X) ->
  typing ([(X, bind_kn)] ++ nil) nil
             (open_te (apply_delta_subst dsubst (apply_gamma_subst gsubst (apply_gamma_subst lgsubst e1))) X) 
             (open_tt (apply_delta_subst_typ dsubst T1) X).
Proof.
  intros E lE dsubst gsubst lgsubst e1 X T1 Hwfd Hwfg Hfv Henv Htyping.
  rewrite_env (nil++lE) in Htyping.
  apply typing_subst_closed with (E:=E) (E':=[(X, bind_kn)]) (dsubst:=dsubst) (gsubst:=gsubst) (lgsubst:=lgsubst)in Htyping; auto.
...
Lemma m_congr_tapp : forall E lE dsubst gsubst lgsubst T2 e1 v u, 
   wf_delta_subst E dsubst ->
   wf_lgamma_subst E lE dsubst gsubst lgsubst ->
   wf_typ E T2 ->
...
Proof.
   intros E lE dsubst gsubst lgsubst T2 e1 v u Hwfd Hwfg Hwft He1_v Hvt1_u.
...
Lemma m_tapp_fv : forall E dsubst dsubst' T2 e e' v v' u u' T1 y,
   wf_env E ->
   wf_delta_subst E dsubst ->
   wf_delta_subst E dsubst' ->
   wf_typ E T2 ->
...
Proof.
  intros E dsubst dsubst' T2 e e' v v' u u' T1 y Hwfe Hwfd Hwfd' Hwft He_v He'_v' Hvt2_u Hv't2_u' Hfv.
...
Lemma m_all_left_fv : forall t3 t2' e e' X t v v' (X4:atom) E E',
  wf_typ nil t3 ->
  wf_typ nil t2' ->
...
Proof.
  intros t3 t2' e e' X t v v' X4 E E' Hwft3 Hwft2' Het3_v He't2'_v' Hfv.
...
Lemma m_all_right_fv : forall t3 t2' e e' X t v v' (X2:atom) E E',
  wf_typ nil t3 ->
  wf_typ nil t2' ->
  normalize (exp_tapp e t3) v ->
  normalize (exp_tapp e' t2') v' ->
...
Proof.
  intros t3 t2' e e' X t v v' X2 E E' Hwft3 Hwft2' Het3_v He't2'_v' Hfv.
  assert (X `notin` fv_te v).
    apply notin_fv_te_mred with (e:=exp_tapp e t3).
      destruct Het3_v; auto.
      simpl. eauto using notin_fv_wf.
