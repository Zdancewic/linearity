...
Inductive wf_delta_osubst : env -> delta_subst -> env -> Prop :=
  ...
  | wf_delta_osubst_styp : forall E SE Env X T,
      wf_delta_osubst E SE Env -> 
      X `notin` dom E  `union`  dom Env -> 
      wf_typ Env T ->
      wf_delta_osubst ([(X, bind_kn)] ++ E) ([(X, T)] ++ SE) Env 
...
Inductive wf_gamma_osubst : env -> delta_subst -> gamma_subst -> env -> Prop :=
  ...
  | wf_gamma_osubst_skind : forall X T E dsE gsE Env,
      wf_gamma_osubst E dsE gsE Env -> 
      X `notin` dom E `union` dom Env ->
      wf_typ Env T ->
      wf_gamma_osubst ([(X, bind_kn)]++E) ([(X, T)]++dsE) gsE Env
  .
...
Inductive wf_lgamma_osubst : env -> lenv -> delta_subst -> gamma_subst -> gamma_subst -> env -> lenv -> Prop :=
  ...
  | wf_lgamma_osubst_skind : forall X T E lE dsE gsE lgsE Env lEnv,
      wf_lgamma_osubst E lE dsE gsE lgsE Env lEnv -> 
      X `notin` dom E `union` dom lE `union` dom Env `union` dom lEnv ->
      wf_typ Env T ->
      wf_lgamma_osubst ([(X, bind_kn)]++E) lE ([(X, T)]++dsE) gsE lgsE Env lEnv
  .
...
Lemma swap_subst_te_odsubst: forall t X E dsubst e Env,
  wf_delta_osubst E dsubst Env ->
  wf_typ Env t ->
...
Proof.
  intros t X E dsubst e Env Hwfd Hwft xndsubst.
  generalize dependent e.
  generalize dependent t.
...
Lemma delta_osubst_opt :  forall E E' dsubst X t t' dsubst' Env,
   wf_delta_osubst (E'++[(X, bind_kn)]++E) (dsubst'++[(X, t)]++dsubst) Env ->
...
Proof.
  intros E E' dsubst X t t' dsubst' Env Hwf_delta_osubst Fr Heq He1' Hwft.
...
Lemma swap_subst_te_ogsubst: forall t X E D dsubst gsubst lgsubst Env lEnv e,
  wf_lgamma_osubst E D dsubst gsubst lgsubst Env lEnv ->
  X `notin` dom Env ->
  wf_typ Env t ->
...
Proof.
  intros t X E D dsubst gsubst lgsubst Env lEnv e Hwflg FrX Hwft.
  generalize dependent e.
  generalize dependent t.
...
Lemma swap_subst_te_olgsubst: forall t X E D dsubst gsubst lgsubst Env lEnv e,
  wf_lgamma_osubst E D dsubst gsubst lgsubst Env lEnv  ->
  X `notin` dom Env  ->
  wf_typ Env t ->
...
Proof.
  ...
  induction Hwfd; intros v Hv; simpl; auto.
    ...
    destruct Hv; simpl; auto.
      ...
      apply value_bang; eauto using subst_te_expr, type_from_wf_typ.
...
Proof.
  ...
  induction Wdf; intros; simpl; auto.
    rewrite delta_osubst_permut with (dE:=E) (Env:=Env); auto.      
...
Lemma commut_osubst_tt_dsubst: forall t X E dsubst Env T,
  wf_delta_osubst E dsubst Env ->
  wf_typ Env t ->
  X `notin` dom E `union` dom Env ->
...
Proof.
  intros t X E dsubst Env T Hwfd Hwft Fr.
  generalize dependent t.
  generalize dependent T.
  induction Hwfd; intros; simpl; auto.
    simpl_env in*. destruct_notin.
...
Lemma swap_subst_tt_odsubst: forall t X E dsubst Env T,
  wf_delta_osubst E dsubst Env ->
  wf_typ E t ->
...
Lemma red_abs_preserved_under_delta_osubst: forall dE dsubst Env T e1 e2,
   wf_delta_osubst dE dsubst Env ->
   red (exp_app (exp_abs T e1) e2) (open_ee e1 e2) ->
   red (exp_app (apply_delta_subst dsubst (exp_abs T e1)) (apply_delta_subst dsubst e2))
                  (apply_delta_subst dsubst (open_ee e1 e2)).
Proof.
  intros dE dsubst Env T e1 e2 Hwfd Hrc.
  generalize dependent T.
  ...
  induction Hwfd; intros; simpl; auto.
    ...
    apply IHHwfd; auto.
      rewrite <- subst_te_open_ee.
      assert (exp_app (exp_abs (subst_tt X T T0) (subst_te X T e1)) (subst_te X T e2)
                  = subst_te X T (exp_app (exp_abs T0 e1) e2)). auto.
      rewrite H1.
      apply red_preserved_under_typsubst; eauto using type_from_wf_typ.
Qed.

Lemma red_tabs_preserved_under_delta_osubst: forall dE dsubst Env e1 t2,
   wf_delta_osubst dE dsubst Env ->
   red (exp_tapp (exp_tabs e1) t2) (open_te e1 t2) ->
   red (exp_tapp (apply_delta_subst dsubst (exp_tabs e1)) (apply_delta_subst_typ dsubst t2))
...
intros.
  ...
  induction H; intros; simpl; auto.
    ...
    apply IHwf_delta_osubst; auto.
      rewrite <- subst_te_open_te; eauto using type_from_wf_typ.
      assert (exp_tapp (exp_tabs (subst_te X T e1)) (subst_tt X T t2)
                  = subst_te X T (exp_tapp (exp_tabs e1) t2)). auto.
      rewrite H3.
      apply red_preserved_under_typsubst; eauto using type_from_wf_typ.
Qed.

Lemma red_abs_preserved_under_gamma_osubst: forall E D dsubst gsubst lgsubst Env lEnv T e1 e2,
   wf_lgamma_osubst E D dsubst gsubst lgsubst Env lEnv ->
   red (exp_app (exp_abs T e1) e2) (open_ee e1 e2) ->
   red (exp_app (apply_gamma_subst gsubst (exp_abs T e1)) (apply_gamma_subst gsubst e2))
...
intros.
  ...
  induction H; intros; simpl; auto.
    ...
    apply IHwf_lgamma_osubst; auto.
      rewrite <- subst_ee_open_ee; auto.
      assert (exp_app (exp_abs T0 (subst_ee x e e1)) (subst_ee x e e2)
                  = subst_ee x e (exp_app (exp_abs T0 e1) e2)) as J. auto.
      rewrite J.
      apply red_preserved_under_expsubst; auto.
Qed.

Lemma red_tabs_preserved_under_gamma_osubst: forall E D dsubst gsubst lgsubst Env lEnv e1 t2,
   wf_lgamma_osubst E D dsubst gsubst lgsubst Env lEnv ->
   red (exp_tapp (exp_tabs e1) t2) (open_te e1 t2) ->
   red (exp_tapp (apply_gamma_subst gsubst (exp_tabs e1)) (apply_gamma_subst_typ gsubst t2))
...
intros.
  ...
  induction H; intros; simpl; auto.
    ...
    apply IHwf_lgamma_osubst; auto.
      rewrite <- subst_ee_open_te; auto.
      assert (exp_tapp (exp_tabs (subst_ee x e e1)) t2
                  = subst_ee x e (exp_tapp (exp_tabs e1) t2)) as J. auto.
      rewrite J.
      apply red_preserved_under_expsubst; auto.
Qed.

Lemma red_abs_preserved_under_lgamma_osubst: forall E D dsubst gsubst lgsubst Env lEnv T e1 e2,
   wf_lgamma_osubst E D dsubst gsubst lgsubst Env lEnv ->
   red (exp_app (exp_abs T e1) e2) (open_ee e1 e2) ->
   red (exp_app (apply_gamma_subst lgsubst (exp_abs T e1)) (apply_gamma_subst lgsubst e2))
...
Lemma red_tabs_preserved_under_lgamma_osubst: forall E D dsubst gsubst lgsubst Env lEnv e1 t2,
   wf_lgamma_osubst E D dsubst gsubst lgsubst Env lEnv ->
   red (exp_tapp (exp_tabs e1) t2) (open_te e1 t2) ->
   red (exp_tapp (apply_gamma_subst lgsubst (exp_tabs e1)) (apply_gamma_subst_typ lgsubst t2))
...
Lemma red_bang_preserved_under_delta_osubst: forall dE dsubst e1 e2 Env,
   wf_delta_osubst dE dsubst Env ->
   red (exp_let (exp_bang e1) e2) (open_ee e2 e1) ->
   red (exp_let (apply_delta_subst dsubst (exp_bang e1)) (apply_delta_subst dsubst e2))
                  (apply_delta_subst dsubst (open_ee e2 e1)).
Proof.
  intros dE dsubst e1 e2 Env Hwfd Hrc.
  generalize dependent e1.
  generalize dependent e2.
  induction Hwfd; intros; simpl; auto.
    rewrite subst_te_open_ee.
    apply IHHwfd; auto.
      rewrite <- subst_te_open_ee.
      assert (exp_let (exp_bang (subst_te X T e1)) (subst_te X T e2)
                  = subst_te X T (exp_let (exp_bang e1) e2)). auto.
      rewrite H1.
      apply red_preserved_under_typsubst; eauto using type_from_wf_typ.
Qed.

Lemma red_bang_preserved_under_gamma_osubst: forall E D dsubst gsubst lgsubst e1 e2 Env lEnv,
   wf_lgamma_osubst E D dsubst gsubst lgsubst Env lEnv ->
   red (exp_let (exp_bang e1) e2) (open_ee e2 e1) ->
   red (exp_let (apply_gamma_subst gsubst (exp_bang e1)) (apply_gamma_subst gsubst e2))
                  (apply_gamma_subst gsubst (open_ee e2 e1)).
intros.
  generalize dependent e1.
  generalize dependent e2.
  induction H; intros; simpl; auto.
    ...
    apply IHwf_lgamma_osubst; auto.
      rewrite <- subst_ee_open_ee; auto.
      assert (exp_let (exp_bang (subst_ee x e e1)) (subst_ee x e e2)
                  = subst_ee x e (exp_let (exp_bang e1) e2)) as J. auto.
      rewrite J.
      apply red_preserved_under_expsubst; auto.
Qed.

Lemma red_bang_preserved_under_lgamma_osubst: forall E D dsubst gsubst lgsubst e1 e2 Env lEnv,
   wf_lgamma_osubst E D dsubst gsubst lgsubst Env lEnv ->
   red (exp_let (exp_bang e1) e2) (open_ee e2 e1) ->
   red (exp_let (apply_gamma_subst lgsubst (exp_bang e1)) (apply_gamma_subst lgsubst e2))
                  (apply_gamma_subst lgsubst (open_ee e2 e1)).
intros.
  generalize dependent e1.
  generalize dependent e2.
  induction H; intros; simpl; auto.
...
(* ********************************************************************** *)
(* weaken stronger *)
Lemma odsubst_stronger : forall E E' dsubst dsubst' X t Env,
  wf_delta_osubst (E'++[(X,bind_kn)]++E) (dsubst'++[(X,t)]++dsubst) Env ->
  wf_typ Env t ->
  ddom_env E [=] dom dsubst ->
  ddom_env E' [=] dom dsubst' ->
...
Proof.
  intros E E' dsubst dsubst' X t Env Hwf_dsubst HT HdomE HdomE' HX.
  remember (E'++[(X,bind_kn)]++E) as G.
...
Lemma odsubst_weaken : forall E E' dsubst dsubst' X t Env,
  wf_delta_osubst (E'++E) (dsubst'++dsubst) Env ->
  wf_typ Env t ->
  ddom_env E [=] dom dsubst ->
  ddom_env E' [=] dom dsubst' ->
...
Lemma odsubst_weaken_head : forall E dsubst X t Env,
  wf_delta_osubst E dsubst Env ->
  wf_typ Env t ->
  ddom_env E [=] dom dsubst ->
  X `notin` (dom E `union` dom Env)->
  wf_delta_osubst ([(X,bind_kn)]++E) ([(X,t)]++dsubst) Env.
Proof.
  intros E dsubst X t Env Hwf_dsubst Ht HdomE HX.
    apply odsubst_weaken with (t:=t) (E:=E) (E':=(@nil (atom*binding))) (dsubst:=dsubst) (dsubst':=delta_nil) (X:=X); simpl_env; eauto.
Qed.
      ...
      apply one_eq_app in HeqdsG. destruct HeqdsG as [[dsubst'' [dsEQ1 dsEQ2]] | [dsEQ1 dsEQ2]]; subst.
        ...
        apply IHwf_delta_osubst with (dsubst':=dsubst''); auto.
          assert (X `notin` fv_tt T).
            apply notin_fv_wf with (E:=Env); auto.
...
intros E D dsubst gsubst lgsubst Env lEnv x t Hwfg Hbinds.
  ...
  induction Hwfg; intros; analyze_binds Hbinds.
    ...
    destruct BindsTac as [J [e' [Hb [Ht [Disj1 [Disj2 Disj3]]]]]].
    split; auto.
      simpl. rewrite -> delta_osubst_permut with (dE:=E) (Env:=Env); auto.
        rewrite <-subst_tt_fresh; auto.
          exists (e'). split;auto. split;auto.
          assert (JJ := @disjoint_lgamma_osubst E lE dsE gsE lgsE Env lEnv Hwfg).
          destruct JJ as [J1 [J2 [J3 [J4 [J5 [J6 [J7 [J8 [J9 J10]]]]]]]]].
...
Lemma bindosdE__bindsT : forall E dsubst X Env,
  wf_delta_osubst E dsubst Env ->
  binds X (bind_kn) E ->
  exists T, binds X (T) dsubst /\ wf_typ Env T /\ disjdom (fv_tt T) (dom dsubst).
intros.
  induction H; intros; analyze_binds H0; subst.
      ...
      split; intros x xnotin.
        destruct (x == X0); subst.
          apply in_fv_wf with (E:=Env) in xnotin; auto.
...
Lemma lgamma_osubst_split_nonlin_weakening_typ : forall E G lE dsubst dsubst' gsubst lgsubst1 lgsubst2 lgsubst X t Env lEnv1 lEnv2 lEnv, 
  ...
  wf_lgamma_osubst (E++[(X, bind_kn)]++G) lE (dsubst'++[(X,t)]++dsubst) gsubst lgsubst Env lEnv ->
  lgamma_osubst_split (E++[(X, bind_kn)]++G) lE (dsubst'++[(X,t)]++dsubst) gsubst lgsubst1 lgsubst2 lgsubst Env lEnv1 lEnv2 lEnv.
Proof.
  intros E G lE dsubst dsubst' gsubst lgsubst1 lgsubst2 lgsubst X t Env lEnv1 lEnv2 lEnv H1 H2 H3.
  remember (E++G) as GG.
...
Lemma lgamma_osubst_split_nonlin_weakening_typ_tail : forall G lE dsubst gsubst lgsubst1 lgsubst2 lgsubst X t Env lEnv1 lEnv2 lEnv, 
  lgamma_osubst_split (G) lE dsubst gsubst lgsubst1 lgsubst2 lgsubst Env lEnv1 lEnv2 lEnv ->
  X `notin` fv_lenv lE `union` dom Env `union` dom lEnv ->
  wf_lgamma_osubst ([(X, bind_kn)]++G) lE ([(X,t)]++dsubst) gsubst lgsubst Env lEnv ->
  lgamma_osubst_split ([(X, bind_kn)]++G) lE ([(X,t)]++dsubst) gsubst lgsubst1 lgsubst2 lgsubst Env lEnv1 lEnv2 lEnv.
Proof.
  intros G lE dsubst gsubst lgsubst1 lgsubst2 lgsubst X t Env lEnv1 lEnv2 lEnv H1 H2 H3.
  rewrite_env (nil ++ [(X, bind_kn)] ++ G).
...
Lemma wft_osubst_closed : forall E E' t dsubst Env,
  wf_delta_osubst E dsubst Env->
  wf_typ (E'++E) t ->
  wf_env (E'++E) ->
  wf_env (apply_delta_subst_env dsubst E' ++ Env) ->
  wf_typ (apply_delta_subst_env dsubst E' ++ Env) (apply_delta_subst_typ dsubst t).
Proof.
  intros E E' t dsubst Env Hwfd Hwft Hwfe Hwfe'.
  remember (E'++E) as EE.
  generalize dependent E'.
  generalize dependent E.
  generalize dependent dsubst.
  ...
  Case "wf_typ_var".
     analyze_binds H0.
       ...
       apply bindosdE__bindsT with (X:=X) in Hwfd; auto.
  ...
  Case "wf_typ_all".
     ...
     apply wf_typ_all with (L:=L `union` dom dsubst  `union` dom (E'++E0) `union` dom (apply_delta_subst_env dsubst E') `union` dom Env).
       ...
       apply H0 with (dsubst:=dsubst) (E'0:=[(X,bind_kn)]++E') (E:=E0) in H2; simpl; simpl_env; eauto.
  ...
  Case "wf_typ_bang".
     rewrite commut_delta_subst_tbang.
     eapply wf_typ_bang; eauto.
  Case "wf_typ_with".
...
Proof.
  ...
  Case "typing_lvar".
     ...
     destruct HeqDD as [[D'' [EQ1 EQ2]] | [EQ1 EQ2]]; subst.
       ...
       apply typing_weakening with (F:=apply_delta_subst_env dsubst E') (E:=Env) (G:=@nil (atom*binding)) in J2; simpl_env in *; auto.
  Case "typing_abs".
     assert (wf_typ (apply_delta_subst_env dsubst E' ++ Env) (apply_delta_subst_typ dsubst T1)) as Wft.
  ...
  Case "typing_app".
     ...
     assert (wf_delta_osubst E0 dsubst Env) as Wfd.
       apply wf_lgamma_osubst__wf_osubst in J.
       destruct J; auto.
     apply typing_app with (T1:=apply_delta_subst_typ dsubst T1)
  ...
  Case "typing_tabs".
     ...
     apply typing_tabs with (L:=L `union` dom gsubst `union` dom lgsubst  
       ...
       intros.
       assert (X `notin` L) as FrxL. auto.
       apply H0 with (D:=D0) (D'0:=D')(E:=E0)(gsubst:=gsubst)(lgsubst:=lgsubst)(dsubst:=dsubst)(E'0:=[(X,bind_kn)]++E')(Env:=Env)(lEnv:=lEnv) in FrxL; clear H0; auto.
         ...
         rewrite_env (nil++[(X, bind_kn)]++apply_delta_subst_env dsubst E' ++ Env).
         apply wf_lenv_weakening; auto.
  ...
  Case "typing_bang".
    symmetry in HeqDD. apply app_eq_nil in HeqDD.
    destruct HeqDD; subst. simpl. 
    assert (J:=Hwfg).
    apply wf_lgamma_osubst_empty_linctx in J; auto.
    apply empty_dom__empty_ctx in J; subst.
    apply typing_bang; auto.
      rewrite_env (apply_delta_subst_lenv dsubst lempty++lempty).
      apply IHHtyp with (D:=lempty)(E:=E0); auto.
  Case "typing_let".
     assert (HH:=H1).
     apply lenv_split_cases_app in H1.
     destruct H1 as [D1L [D1R [D2L [D2R [Hsplit' [Hsplit [J1 J2]]]]]]]; subst.
     assert (J:=Hwfg).
     apply wf_lgamma_osubst_split with (lE1:=D1R) (lE2:=D2R) (E':=E') in Hwfg; auto.
     destruct Hwfg as [lgsubst1 [lgsubst2 [lEnv1 [lEnv2 [Hgsplit [Hwflg1 Hwflg2]]]]]].
     assert (wf_lenv (E'++E0) D1L) as Wfe1'.
     ...
     assert (exp_let
       ...
       assert (forall x, x `in` dom lgsubst2 -> x `notin` fv_ee e1) as FV1.
         ...
         assert (x `notin` dom D1L `union` dom D2L) as xnotinD12L.
           apply dom_lenv_split in Hsplit'.
           rewrite Hsplit' in xnotinD'. auto.
         apply typing_fv with (x:=x) in Htyp; auto.
       rewrite gamma_subst_disjoint_exp with (gsubst:=lgsubst2)(e:=e1); auto.
       assert (forall x, x `in` dom lgsubst1 -> x `notin` fv_ee e2) as FV2.
         intros x FV.
         apply wf_lgamma_osubst__fv with (x:=x) in Hwflg1; auto.
         ...
         apply H in ynL. 
         apply typing_fv with (x:=x) in ynL; auto.
         apply notin_fv_ee_open_ee_inv in ynL; auto.
       rewrite gamma_subst_disjoint_exp with (gsubst:=lgsubst1)(e:=e2); auto.
     ...
     assert (wf_delta_osubst E0 dsubst Env) as Wfd.
       apply wf_lgamma_osubst__wf_osubst in J.
       destruct J; auto.
     apply typing_let with (L:=L `union` dom gsubst `union` dom lgsubst2 `union` dom (E'++E0) `union` dom (apply_delta_subst_env dsubst E') `union` dom (apply_delta_subst_lenv dsubst D2L) `union` dom Env `union` dom lEnv2)(T1:=apply_delta_subst_typ dsubst T1)
                           (D1:=apply_delta_subst_lenv dsubst D1L ++ lEnv1)
                           (D2:=apply_delta_subst_lenv dsubst D2L ++ lEnv2).     
     SCase "typing1".
       rewrite <- commut_delta_subst_tbang.
       apply IHHtyp with (E:=E0)(D:=D1R)(D':=D1L); auto.
         apply dom_lenv_split in Hsplit'. solve_uniq.
     ...
     SCase "typing2".
       intros x xn.
       assert (wf_typ (E'++E0) T1) as WFT1.
         apply typing_regular in Htyp. destruct Htyp as [_ [_ [_ Hwft1]]].
         inversion Hwft1; subst; auto.
       assert (x `notin` L) as FrxL. auto.
       apply H0 with (E:=E0) (gsubst:=gsubst) (dsubst:=dsubst) (E'0:=[(x, bind_typ T1)]++E') (D':=D2L) (D:=D2R) (lgsubst:=lgsubst2)(Env:=Env)(lEnv:=lEnv2) in FrxL; clear H0;auto.
       SSCase "typing".
         rewrite commut_lgamma_osubst_open_ee with (E:=E0) (dsubst:=dsubst) (D:=D2R) (gsubst:=gsubst)(Env:=Env)(lEnv:=lEnv2) in FrxL; auto.
         rewrite commut_gamma_osubst_open_ee with (E:=E0) (dsubst:=dsubst) (D:=D) (lgsubst:=lgsubst)(Env:=Env)(lEnv:=lEnv) in FrxL; auto.
         rewrite commut_delta_osubst_open_ee with (dE:=E0)(Env:=Env) in FrxL; auto.
         simpl in FrxL. simpl_env in *.
         rewrite apply_gamma_subst_nfv in FrxL; auto.
         rewrite apply_gamma_subst_nfv in FrxL; auto.
       ...
       SSCase "disjoint".
         apply dom_lenv_split in Hsplit'. 
         clear - Hsplit' Disj1. solve_uniq.

       SSCase "disjoint".
         apply dom_lenv_split in Hsplit'. 
         apply lgamma_osubst_split__lenv_split in Hgsplit.
         apply dom_lenv_split in Hgsplit. 
         clear - Hsplit' Disj2 Hgsplit. solve_uniq.

       SSCase "wf_lenv".
         simpl. rewrite_env (nil++[(x, bind_typ (apply_delta_subst_typ dsubst T1))]++apply_delta_subst_env dsubst E' ++ Env).
         apply wf_lenv_weakening; simpl; simpl_env; auto.
         SSSCase "wf_lenv".
          apply wf_lenv_merge; auto.
           eapply wfle_osubst_closed; eauto.
             assert (disjoint (D'++D) Env) as Disj.
       ...
       SSCase "split".
       eapply lenv_split_osubst_closed; eauto.
         apply dom_lenv_split in Hsplit. fsetdec.
...
Lemma wft_osubst: forall t dsubst E Env,
  wf_delta_osubst E dsubst Env ->
  wf_typ E t ->
  wf_typ Env (apply_delta_subst_typ dsubst t).
intros.
