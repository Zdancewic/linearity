...
(* Path lemmas for multigraphs. *)

(* The following notions are defined used :                                  *)
(*      - StrictTrail : walk without repetition of edge,                      *)
(*              constructors : ST_null, ST_step;                          *)
(*      - StrictPath : trail without repetition of inner vertex,              *)
(*              constructors : SP_null, SP_step;                          *)
(*      - StrictlyClosed_trail, StrictCycle.                             *)

Require Export Multidegrees.
Require Export GraphBasics.Paths.
...
Inductive StrictTrail : Multigraph v a -> Vertex -> Vertex -> V_list -> E_list -> Set :=
  | ST_null : forall (g : Multigraph v a) (x : Vertex), v x ->
      StrictTrail g x x V_nil E_nil
  | ST_step :
      forall (g : Multigraph v a) (x y z : Vertex) (vl : V_list) (el : E_list),
      StrictTrail () y z vl el ->
      v x ->
      a (A_ends x y) ->
      (forall u : Edge, In u el -> ~ E_eq u (E_ends x y)) ->
      StrictTrail g x z (y :: vl) (E_ends x y :: el).
...
Section STRICT_PATH_AND_DEGREE.

Variable v : V_set.
Variable a : A_set.
Variable g : Multigraph v a.

Lemma Path_mdegree_one :
 forall (x y : Vertex) (vl : V_list) (el : E_list),
 Path v a x y vl el -> forall z : Vertex, In z vl -> mdegree z v a g > 0.
Proof.
        intros x y vl el p; elim p; intros.
        inversion H.

        inversion H0.
        rewrite <- H1; apply mdegree_not_isolated.
        split with x0; apply (M_non_directed v a g); trivial.

        apply H; auto.
Qed.

Lemma Path_consx_mdegree_one :
 forall (x y : Vertex) (vl : V_list) (el : E_list),
 Path v a x y vl el ->
 vl <> V_nil -> forall z : Vertex, In z (x :: vl) -> mdegree z v a g > 0.
Proof.
        simple destruct vl; intros.
        absurd (nil = V_nil); auto.

        inversion H1.
        rewrite <- H2; apply mdegree_not_isolated.
        inversion H; split with v0; trivial.

        apply (Path_mdegree_one x y (v0 :: l) el H); auto.
Qed.

Lemma Path_mdegree_two :
 forall (x y : Vertex) (vl : V_list) (el : E_list),
 Path v a x y vl el ->
 forall z : Vertex, In z vl -> z <> y -> mdegree z v a g > 1.
Proof.
        intros x y vl el p; elim p; intros.
        inversion H.
        ...
        rewrite <- H2; apply mdegree_not_mpendant.
        split with x0.
        apply (M_non_directed v a g); trivial.
...
Lemma Cycle_mdegree_two :
 forall (x y : Vertex) (vl : V_list) (el : E_list) (p : Path v a x y vl el),
 Cycle v a x y vl el p -> forall z : Vertex, In z vl -> mdegree z v a g > 1.
Proof.
        intros; case (V_eq_dec z y); intros.
        rewrite e; apply mdegree_not_mpendant.
        inversion p.
        ...
        elim (Path_last_step v a y0 y vl0 el0 H1); intros.
        split with x1.
        apply (M_non_directed v a g); apply (P_inel_ina v a y0 y vl0 el0 H1);
         trivial.
        ...
        apply (Path_mdegree_two x y vl el p); trivial.
Qed.
...
Lemma Cycle_consx_mdegree_two :
 forall (x y : Vertex) (vl : V_list) (el : E_list) (p : Path v a x y vl el),
 vl <> V_nil ->
 Cycle v a x y vl el p ->
 forall z : Vertex, In z (x :: vl) -> mdegree z v a g > 1.
Proof.
        intros; inversion H1.
        rewrite <- H2; inversion H0.
        apply (Cycle_mdegree_two x y vl el p H0 y).
        apply (P_iny_vl v a x y vl el p); trivial.

        apply (Cycle_mdegree_two x y vl el p H0); trivial.
Qed.

Lemma Path_mdegree_zero_nil :
 forall (x y : Vertex) (vl : V_list) (el : E_list),
 Path v a x y vl el ->
 (exists2 z : Vertex, In z (x :: vl) & mdegree z v a g = 0) -> vl = V_nil.
Proof.
        simple destruct vl; intros.
        trivial.

        elim H0; intros.
        absurd (mdegree x0 v a g > 0).
        omega.

        apply (Path_consx_mdegree_one _ _ _ _ H).
        discriminate.

        trivial.
Qed.

Lemma Cycle_mdegree_one_nil :
 forall (x y : Vertex) (vl : V_list) (el : E_list) (p : Path v a x y vl el),
 Cycle v a x y vl el p ->
 (exists2 z : Vertex, In z (x :: vl) & mdegree z v a g <= 1) -> vl = V_nil.
Proof.
        simple destruct vl; intros.
        trivial.

        elim H0; intros.
        absurd (mdegree x0 v a g > 1).
        omega.

        apply (Cycle_consx_mdegree_two _ _ _ _ p).
        discriminate.
...
End STRICT_PATH_AND_DEGREE.

Section STRICT_PATH_EQ.
...
Section STRICT_PATH_IN_A_SUBGRAPH.
...
End STRICT_PATH_IN_A_SUBGRAPH.

Section MULTI_REVERSE_WALK.
