...
Proof.
  ...
  Case "typing_abs".
    apply typing_abs with (L:=dom E `union` dom lE' `union` dom lE `union` {{x}} `union` {{y}} `union` L); auto.
      ...
      apply H1 with (b0:=b)(lE0:=lE)(lE'0:=lE')(x1:=x) in x0nL; auto.
      ...
      apply app_cons_not_nil.
  Case "typing_labs".
    apply typing_labs with (L:=dom E `union` dom lE' `union` dom lE `union` {{x}} `union` {{y}} `union` L); auto.
      intros x0 x0notin.
      simpl_env.
      assert (x0 `notin` L) as x0nL. auto.
  ...
  Case "typing_app".
    ...
    SCase "left".
      ...
      apply (typing_app T1 K E (D1L ++ [(y, lbind_typ t)] ++ D1R) (D2L ++ D2R)); auto.
        simpl_env.
    ...
    SCase "right".
      ...
      apply (typing_app T1 K E (D1L ++ D1R) (D2L ++ [(y, lbind_typ t)] ++ D2R)); auto.
        simpl_env.
  ...
  Case "typing_tabs".
    apply typing_tabs with (L:=dom E `union` dom lE' `union` dom lE `union` {{x}} `union` {{y}} `union` L); auto.
      ...
      apply H1 with (b0:=b)(lE0:=lE)(lE'0:=lE')(x0:=x) in XnL; auto.
