...
Require Export LinF_ExtraLemmas.
...
Fixpoint typ_size (T : typ) {struct T} : nat :=
  match T with
    | typ_bvar i => 1
    | typ_fvar x => 1
    | typ_arrow k T1 T2 => 1 + (typ_size T1) + (typ_size T2)
    | typ_all k T1 => 1 + (typ_size T1)
...
Fixpoint ddom_env (E : env) : atoms :=
  match E with
    | nil => {}
    | (X, bind_kn _) :: E' => (singleton X) `union` (ddom_env E')
...
Fixpoint apply_delta_subst_env (dsubst:delta_subst) (E:env) {struct E} :=
  ...
  | pair X (bind_kn k)::E' =>
      pair X (bind_kn k)::(apply_delta_subst_env dsubst E')
  end.
...
Lemma ddom_single : forall x K,
  ddom_env [(x,bind_kn K)] [=] singleton x.
Proof.
  simpl. intros. fsetdec.
Qed.

Lemma ddom_push : forall x K E,
  ddom_env ([(x,bind_kn K)] ++ E) = (singleton x) `union` (ddom_env E).
Proof.
...
Lemma free_env_kind_push : forall X K E,
  fv_env ([(X,bind_kn K)] ++ E) [=] (singleton X) `union` (fv_env E).
Proof.
  simpl. intros. fsetdec.
Qed.

Lemma free_env_kind_cons : forall X K E,
  fv_env ((X,bind_kn K) :: E) [=] (singleton X)  `union` (fv_env E).
Proof.
  simpl. intros. fsetdec.
Qed.
...
Lemma ddom_dom__inv : forall A (sE : list (atom * A)) E X b K,
  ddom_env ([(X, bind_kn K)]++E) [=] dom ([(X, b)]++sE) ->
  X `notin` ddom_env E ->
  X `notin` dom sE ->
  ddom_env E [=] dom sE.
Proof.
...
Lemma delta_subst_closed_typ : forall dsubst t K,
   wf_typ nil t K ->
...
Lemma gamma_subst_closed_typ : forall gsubst t K,
   wf_typ nil t K ->
...
Lemma delta_subst_binds_kind: forall E X K dsubst,
  binds X (bind_kn K) E ->
  binds X (bind_kn K) (apply_delta_subst_env dsubst E).
Proof.
  induction E; intros X K dsubst Hbinds; simpl.
    inversion Hbinds.
    destruct a; destruct b;
      simpl_env in *; analyze_binds Hbinds.
Qed.

Lemma delta_subst_var : forall dsubst X T K,
   uniq dsubst ->
   binds X (T) dsubst -> wf_typ nil T K ->
   apply_delta_subst_typ dsubst (typ_fvar X) = T.
Proof.
  induction dsubst; intros X T K Huniq Hbinds Hwft.
    inversion Hbinds.
...
Lemma commut_gamma_subst_abs: forall gsubst K V e,
   apply_gamma_subst gsubst (exp_abs K V e)
= exp_abs K (apply_gamma_subst_typ gsubst V) (apply_gamma_subst gsubst e).
Proof.
  induction gsubst; intros; simpl; auto.
    unfold apply_gamma_subst_typ in *.
    destruct  a. destruct  g; auto.
...
Lemma commut_gamma_subst_tabs: forall gsubst K e,
   apply_gamma_subst gsubst (exp_tabs K e)
= exp_tabs K (apply_gamma_subst gsubst e).
Proof.
  induction gsubst; intros; simpl; auto.
    destruct  a. destruct  g; auto.
Qed.
...
Lemma commut_delta_subst_abs: forall dsubst e V K,
   apply_delta_subst dsubst (exp_abs K V e)
= exp_abs K (apply_delta_subst_typ dsubst V) (apply_delta_subst dsubst e).
Proof.
  induction dsubst; intros; simpl; auto.
    destruct  a. destruct  d; auto.
Qed.

Lemma commut_delta_subst_tabs: forall dsubst e K,
   apply_delta_subst dsubst (exp_tabs K e)
= exp_tabs K (apply_delta_subst dsubst e).
Proof.
  induction dsubst; intros; simpl; auto.
    destruct  a. destruct  d; auto.
Qed.
...
Lemma commut_delta_subst_arrow: forall dsubst K T1 T2,
   apply_delta_subst_typ dsubst (typ_arrow K T1 T2)
= typ_arrow K (apply_delta_subst_typ dsubst T1) (apply_delta_subst_typ dsubst T2).
Proof.
  induction dsubst; intros; simpl; auto.
    destruct  a. destruct  d; auto.
Qed.

Lemma commut_delta_subst_all: forall dsubst K T,
   apply_delta_subst_typ dsubst (typ_all K T)
= typ_all K (apply_delta_subst_typ dsubst T).
Proof.
  induction dsubst; intros; simpl; auto.
    destruct  a. destruct  d; auto.
Qed.
...
Lemma wft_strengthen_ex : forall E F T K,
 wf_typ (F++E) T K -> ddom_env F [=] {} -> wf_typ E T K.
Proof.
  intros E F T K Hwft Heq.
  induction F; auto.
...
intros.
  ...
  inversion H1; subst; auto.
    apply IHred in H6; subst; auto.
    apply value_unred in H4. 
      assert (False) as F.
        apply H4. exists e1'. auto.
      tauto.
    inversion H0.

  inversion H1; subst; auto.
    apply value_unred in H. 
      assert (False) as F.
        apply H. exists e1'. auto.
      tauto.
    apply IHred in H6; subst; auto.
    apply value_unred in H6. 
      assert (False) as F.
        apply H6. exists e2'. auto.
      tauto.

  inversion H1; subst; auto.
    inversion H6.
    apply value_unred in H0. 
      assert (False) as F.
        apply H0. exists e2'. auto.
      tauto.
...
Lemma _congr_app_arg : forall v1 e2 v2 u,
  value v1 ->
  bigstep_red e2 v2 ->
  bigstep_red (exp_app v1 v2) u ->
  bigstep_red (exp_app v1 e2) u.
Proof.
  intros v1 e2 v2 u He Hbrc Hbrc'.
  generalize dependent v1.
  generalize dependent u.
  induction Hbrc; intros; eauto.
Qed.

Lemma _congr_app : forall e1 e2 v1 v2 u,
  value v1 ->
  value v2 ->
  expr e1 ->
  expr e2 ->
  ...
  bigstep_red (exp_app e1 e2) u.
Proof.
  intros e1 e2 v1 v2 u Hv1 Hv2 He1 He2 Hbrc1 Hbrc2 Hbrc.
  generalize dependent e2.
  generalize dependent v2.
  generalize dependent u.
  induction Hbrc1; intros.
    eapply _congr_app_arg; eauto.

    apply bigstep_red__trans with (e':=exp_app e'' e2).
...
Lemma congr_app_arg : forall v1 e2 v2 u,
  value v1 ->
  normalize e2 v2 ->
  normalize (exp_app v1 v2) u ->
  normalize (exp_app v1 e2) u.
Proof.
  intros v1 e2 v2 u He Hnorm Hnorm'.
  destruct Hnorm. destruct Hnorm'.
  split; auto.
    apply _congr_app_arg with (v2:=v2); auto.
Qed.

Lemma congr_app : forall e1 e2 v1 v2 u,
  expr e1 ->
  expr e2 ->
  normalize e1 v1 ->
  normalize e2 v2 ->
  ...
  normalize (exp_app e1 e2) u.
Proof.
  intros e1 e2 v1 v2 u He1 He2 Hnorm1 Hnorm2 Hnorm.
  destruct Hnorm1. destruct Hnorm2. destruct Hnorm.
  split; auto.
    apply _congr_app with (v1:=v1) (v2:=v2); auto.
Qed.
...
Proof.
  ...
  induction Hrc; intros; simpl; auto.
    apply red_app_2; auto.
      apply subst_te_value; auto.
    rewrite subst_te_open_ee. apply red_abs; auto.
      assert (exp_abs K (subst_tt X T0 T) (subst_te X T0 e1) = (subst_te X T0 (exp_abs K T e1))). auto.
      rewrite H1.
      apply subst_te_expr; auto.
      apply subst_te_value; auto.
    rewrite subst_te_open_te; auto. apply red_tabs; auto.
      assert (exp_tabs K (subst_te X T0 e1) = (subst_te X T0 (exp_tabs K e1))). auto.
      rewrite H1.
      apply subst_te_expr; auto.
Qed.
...
Proof.
  ...
  induction Hrc; intros; simpl; auto.
    apply red_app_2; auto.
      apply subst_ee_value; auto.
    rewrite subst_ee_open_ee; auto. apply red_abs; auto.
      assert (exp_abs K T (subst_ee x e0 e1) = (subst_ee x e0 (exp_abs K T e1))). auto.
      rewrite H1.
      apply subst_ee_expr; auto.
      apply subst_ee_value; auto.
    rewrite subst_ee_open_te; auto. apply red_tabs; auto.
      assert (exp_tabs K (subst_ee x e0 e1) = (subst_ee x e0 (exp_tabs K e1))). auto.
      rewrite H1.
      apply subst_ee_expr; auto.
Qed.
...
Lemma FrTyping__absvalue : forall L V E D e1 T1 K Q,
  wf_typ E V K ->
  (forall x : atom, x `notin` L -> typing ([(x, bind_typ V)] ++ E) D (open_ee e1 x) T1) ->
  value (exp_abs Q V e1).
Proof.
  intros.
  apply value_abs.
...
Lemma FrTyping__labsvalue : forall L V E D e1 T1 K Q,
  wf_typ E V K ->
...
Lemma FrTyping__tabsvalue : forall L E D e1 T1 K Q,
  (forall X : atom,  X `notin` L -> typing ([(X, bind_kn K)] ++ E) D (open_te e1 X) (open_tt T1 X)) ->
  value (exp_tabs Q e1).
Proof.
  intros.
  apply value_tabs.
...
Lemma rsubst_stronger : forall E E' rsubst rsubst' X K R,
...
Proof.
  intros E E' rsubst rsubst' X K R Hwf_rsubst HdomE HdomE' HX.
...
Lemma rsubst_weaken : forall E E' rsubst rsubst' X K R,
  wf_rho_subst (E'++E) (rsubst'++rsubst) ->
  ddom_env E [=] dom rsubst ->
  ddom_env E' [=] dom rsubst' ->
...
Lemma rsubst_weaken_head : forall E rsubst X K R,
  wf_rho_subst E rsubst ->
  ddom_env E [=] dom rsubst ->
  X `notin` (dom E)->
  wf_rho_subst ([(X,bind_kn K)]++E) ([(X,R)]++rsubst).
Proof.
  intros E rsubst X K R Hwf_rsubst HdomE HX.
    apply rsubst_weaken with (K:=K) (E:=E) (E':=(@nil (atom*binding))) (rsubst:=rsubst) (rsubst':=rho_nil) (R:=R) (X:=X); simpl_env; eauto.
Qed.  

(* ********************************************************************** *)
(** * Inversion *)

Lemma wft_arrow_inv : forall E T1 T2 K Q,
  wf_typ E (typ_arrow K T1 T2) Q ->
  wf_typ E T1 kn_lin /\ wf_typ E T2 kn_lin.
Proof.
  intros E T1 T2 K Q Wft.
  remember (typ_arrow K T1 T2) as T.
  ...
  induction Wft; intros; subst; try solve [inversion HeqT].
    ...
    apply IHWft with (K0:=K) ; auto.
Qed.

Lemma wft_all_inv : forall E T1 K Q,
  wf_typ E (typ_all K T1) Q ->
  uniq E ->
  (forall X,
    X `notin` dom E `union` fv_tt T1 ->
    wf_typ ([(X, bind_kn K)]++E) (open_tt T1 X) kn_lin).
Proof.
  intros E T1 K Q Wft.
  remember (typ_all K T1) as T.
  generalize dependent K.
  generalize dependent T1.
  induction Wft; intros; subst; try solve [inversion HeqT].
    inversion HeqT; subst.
...
Lemma wft_with_inv : forall E T1 T2 Q,
  wf_typ E (typ_with T1 T2) Q ->
  wf_typ E T1 kn_lin /\ wf_typ E T2 kn_lin.
Proof.
  intros E T1 T2 Q Wft.
...
Lemma wf_env_strengthening_typ : forall X Q E F,
  wf_env (F ++ [(X, bind_kn Q)] ++ E) ->
  X `notin` fv_env F ->
  wf_env (F ++ E).
...
Lemma lenv_split_strengthening_typ: forall E X Q G D1 D2 D3,
  lenv_split (E ++ [(X, bind_kn Q)] ++ G) D1 D2 D3 ->
  X `notin` fv_env E `union` fv_lenv D3 ->
  lenv_split (E ++ G) D1 D2 D3.
Proof.
  intros E X Q G D1 D2 D3 S.
  remember (E ++ [(X, bind_kn Q)] ++ G) as G0.
  generalize dependent E. generalize dependent G. generalize dependent X. generalize dependent Q.
  (lenv_split_cases (induction S) Case); intros Q X G1 E1 EQ NIN; subst.
...
Lemma lenv_split_strengthening_typ_tail: forall X Q G D1 D2 D3,
  lenv_split ([(X, bind_kn Q)] ++ G) D1 D2 D3 ->
  X `notin` fv_lenv D3 ->
  lenv_split G D1 D2 D3.
Proof.
  intros X Q G D1 D2 D3 S NIN.
  rewrite_env (nil++[(X, bind_kn Q)] ++ G) in S.
...
Lemma dnotin_fv_wf : forall E (X : atom) T K,
  wf_typ E T K ->
  X `notin` ddom_env E ->
  X `notin` fv_tt T.
Proof with auto.
  ...
  induction Wfle; intros; simpl.
    ...
    destruct (x==x0); subst.
      ...
      assert (x0 `notin` fv_tt T) as x0nT.
        apply dnotin_fv_wf with (E:=E) (K:=kn_lin); auto.
           eapply notin_ddom_env; eauto.
      apply IHWfle in Binds. auto.
Qed.

Lemma din_fv_wf : forall E (X : atom) T K,
  wf_typ E T K ->
  X `in` fv_tt T ->
  X `in` ddom_env E.
Proof with auto.
  intros E X T K Wf_typ XinT.
...
Proof.
  ...
  Case "typing_abs".
    ...
    destruct_notin. clear H H0 H1 H2 Fr NotInTac NotInTac1 NotInTac2 NotInTac3 NotInTac4 NotInTac5 NotInTac6 xnotinL.
    fsetdec.    
  Case "typing_labs".
    pick fresh x.
    assert (x `notin` L) as xnotinL. auto.
  ...
  Case "typing_tabs".
    ...
    destruct_notin. clear H H0 H1 Fr NotInTac NotInTac1 NotInTac2 NotInTac3 NotInTac4 NotInTac5 XnotinL.
    fsetdec.    
  Case "typing_apair".
    rewrite <- IHTyping1.
    fsetdec.
...
Proof.
  ...
  induction Red; intros; simpl; simpl in Fr; auto.
    apply value_through_subst_ee with (x:=x) (u:=y) in H; auto.

    apply value_through_subst_ee with (x:=x) (u:=y) in H0; auto.
...
Lemma wft_fv_tt_sub : forall E t K,
  wf_typ E t K ->
  fv_tt t [<=] dom E.
Proof.
  intros E t K Wft.
  induction Wft; simpl.
    ...
    assert (fv_tt T2 [<=] dom ([(X, bind_kn K1)]++E)) as sub'.
      fsetdec.
    assert (AtomSetImpl.diff (fv_tt T2) {{X}} [<=] AtomSetImpl.diff (dom ([(X, bind_kn K1)]++E)) {{X}}) as sub''.
      fsetdec.
    assert (AtomSetImpl.diff (fv_tt T2) {{X}}[=](fv_tt T2)) as eq.
      clear XnL sub sub' sub''. fsetdec.
    assert (AtomSetImpl.diff (dom ([(X, bind_kn K1)]++E)) {{X}}[=]dom E) as eq'.
