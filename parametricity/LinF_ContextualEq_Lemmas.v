...
Lemma wf_typ_permute: forall F E1 x t' E2 T K,
  wf_env (F++E1++[(x, bind_typ t')]++E2) ->
  wf_typ (F++E1++[(x, bind_typ t')]++E2) T K->
  wf_typ (F++[(x, bind_typ t')]++E1++E2) T K.
Proof.
  intros F E1 x t' E2 T K Wfe Wft.
...
Proof.
  ...
  Case "typing_abs".
    pick fresh y and apply typing_abs; auto.
      ...
      rewrite_env (([(y, bind_typ T1)]++F)++[(x, bind_typ t')]++E1++E2).
      apply H1; auto.
  Case "typing_labs".
    pick fresh y and apply typing_labs; auto.
      apply wf_typ_permute; auto.
        ...
        inversion H3; subst; auto.
  Case "typing_app".
    apply typing_app with (D1:=D1) (D2:=D2) (T1:=T1) (K:=K); auto.
      apply lenv_split_nonlin_permute; auto.
  Case "typing_tabs".
    pick fresh X and apply typing_tabs; eauto.
      rewrite_env (([(X, bind_kn K)]++F)++[(x, bind_typ t')]++E1++E2).
      apply H1; auto.
  Case "typing_tapp".
...
Lemma wf_typ_renaming_one: forall E1 x y t' E2 T K,
  wf_env (E1++[(x, bind_typ t')]++E2) ->
  wf_typ (E1++[(x, bind_typ t')]++E2) T K->
  y `notin` dom E1 `union` dom E2 ->
  wf_typ (E1++[(y, bind_typ t')]++E2) T K.
Proof.
  intros E1 x y t' E2 T K Wfe Wft yn.
  apply wf_typ_weakening; auto.
...
Proof.
  ...
  Case "typing_abs".
    pick fresh z and apply typing_abs; auto.
      ...
      rewrite_env (([(z, bind_typ T1)]++E1)++[(y, bind_typ t')]++E2).
  ...
  Case "typing_app".
    ...
    rewrite J in yn.
    apply typing_app with (D1:=D1) (D2:=D2) (T1:=T1) (K:=K); eauto.
  ...
  Case "typing_tabs".
    pick fresh X and apply typing_tabs; eauto.
      ...
      rewrite_env (([(X, bind_kn K)]++E1)++[(y, bind_typ t')]++E2).
      rewrite <- subst_ee_open_te; auto.
  Case "typing_tapp".
    apply typing_tapp with (K:=K); auto.
      apply wf_typ_renaming_one with (x:=x); auto.
...
Lemma _wf_typ_typ_permute: forall F E1 X K' E2 T K,
  wf_env (F++E1++[(X, bind_kn K')]++E2) ->
  wf_env (E1++E2) ->
  wf_typ (F++E1++[(X, bind_kn K')]++E2) T K->
  wf_typ (F++[(X, bind_kn K')]++E1++E2) T K.
Proof.
  intros F E1 X K' E2 T K Wfe Wfe' Wft.
  remember (F++E1++[(X, bind_kn K')]++E2) as E.
  generalize dependent F.
  generalize dependent E1.
  induction Wft; intros; subst.
    ...
    apply wf_typ_arrow with (K1:=K1) (K2:=K2); auto.

    apply wf_typ_all with (L:=L `union` {{X}} `union` dom (F++E1++E2)); auto.
      intros X0 X0nL.
      rewrite_env (([(X0, bind_kn K1)]++F)++[(X, bind_kn K')]++E1++E2).
      apply H0; simpl_env; auto.

    apply wf_typ_with with (K1:=K1) (K2:=K2); auto.

    apply wf_typ_sub; auto.
Qed.

Lemma wf_typ_typ_permute: forall E1 X K' E2 T K,
  wf_env (E1++[(X, bind_kn K')]++E2) ->
  wf_env (E1++E2) ->
  wf_typ (E1++[(X, bind_kn K')]++E2) T K->
  wf_typ ([(X, bind_kn K')]++E1++E2) T K.
Proof.
  intros E1 X K' E2 T K Wfe Wfe' Wft.
  rewrite_env (nil++E1++[(X, bind_kn K')]++E2) in Wft.
  apply _wf_typ_typ_permute in Wft; auto.
Qed.

Lemma _wf_env_typ_permute: forall F E1 X K' E2,
  wf_env (F++E1++[(X, bind_kn K')]++E2) ->
  wf_env (E1++E2) ->
  wf_env (F++[(X, bind_kn K')]++E1++E2).
Proof.
  intros F E1 X K' E2 Wfe.
  remember (F++E1++[(X, bind_kn K')]++E2) as E.
  generalize dependent F.
  generalize dependent E1.
  induction Wfe; intros; subst.
    symmetry in HeqE.
    contradict HeqE.
    simpl. rewrite_env ((F++E1)++(X, bind_kn K')::E2).
...
Lemma wf_env_typ_permute: forall E1 X K' E2,
  wf_env (E1++[(X, bind_kn K')]++E2) ->
  wf_env (E1++E2) ->
  wf_env ([(X, bind_kn K')]++E1++E2).
Proof.
  intros E1 X K' E2 Wfe Wfe'.
  rewrite_env (nil++E1++[(X, bind_kn K')]++E2) in Wfe.
  apply _wf_env_typ_permute with (X:=X) (K':=K') (F:=nil) (E1:=E1) (E2:=E2) in Wfe; auto.
Qed.

Lemma wf_typ_typ_renaming_one: forall E1 X Y K' E2 T K,
  wf_env (E1++[(X, bind_kn K')]++E2) ->
  wf_typ (E1++[(X, bind_kn K')]++E2) T K->
  Y `notin` dom E1 `union` dom E2 ->
  wf_typ (E1++[(Y, bind_kn K')]++E2) (subst_tt X Y T) K.
Proof.
  intros E1 X Y K' E2 T K Wfe Wft Yn.
  ...
  induction Wft; intros; subst; simpl; simpl_env.
    ...
    apply wf_typ_all with (L:=L `union` {{X}} `union` {{Y}} `union` dom E1 `union` dom E2); auto.
      intros X0 X0nL.
      rewrite_env (([(X0, bind_kn K1)]++E1)++[(Y, bind_kn K')]++E2).
...
Lemma wf_typ_typ_renaming_one': forall E1 X Y K' E2 T K,
  ...
  wf_typ (E1++[(X, bind_kn K')]++E2) T K->
  Y `notin` dom E1 `union` dom E2 ->
  wf_typ (map (subst_tb X Y) E1 ++[(Y, bind_kn K')]++E2) (subst_tt X Y T) K.
Proof.
  intros E1 X Y K' E2 T K Wfe Wft Yn.
  remember (E1++[(X, bind_kn K')]++E2) as E.
  generalize dependent E1.
  induction Wft; intros; subst; simpl; simpl_env.
    ...
    apply wf_typ_all with (L:=L `union` {{X}} `union` {{Y}} `union` dom E1 `union` dom E2); auto.
      intros X0 X0nL.
      rewrite_env ((map (subst_tb X Y) ([(X0, bind_kn K1)]++E1))++[(Y, bind_kn K')]++E2).    
      rewrite subst_tt_fresh with (T:=X0) (Z:=X) (U:=Y); auto.
      unfold open_tt.
      rewrite <- subst_tt_open_tt_rec; auto.
      apply H0; auto.
...
Lemma wf_env_typ_renaming_one: forall E1 X Y K E2,
  wf_env (E1++[(X, bind_kn K)]++E2) ->
  Y `notin` dom E1 `union` dom E2 ->
  wf_env (map (subst_tb X Y) E1 ++[(Y, bind_kn K)]++E2).
Proof.
  intros E1 X Y K E2 Wfe.
  remember (E1++[(X, bind_kn K)]++E2) as E.
  generalize dependent E1.
  induction Wfe; intros; subst.
    symmetry in HeqE.
...
Lemma wf_lenv_typ_renaming_one: forall E1 X Y K E2 D,
  wf_lenv (E1++[(X, bind_kn K)]++E2) D ->
  Y `notin` dom E1 `union` dom E2 `union` dom D ->
  wf_lenv (map (subst_tb X Y) E1 ++[(Y, bind_kn K)]++E2) (map (subst_tlb X Y) D).
Proof.
  intros E1 X Y K E2 D Wfle.
  remember (E1++[(X, bind_kn K)]++E2) as E.
  generalize dependent E1.
...
Lemma lenv_split_typ_renaming_one: forall E1 X (Y:atom) K' E2 D1 D2 D,
  lenv_split (E1++[(X, bind_kn K')]++E2) D1 D2 D ->
  Y `notin` dom E1 `union` dom E2 `union` dom D ->
  lenv_split (map (subst_tb X Y) E1++[(Y, bind_kn K')]++E2) (map (subst_tlb X Y) D1) (map (subst_tlb X Y) D2) (map (subst_tlb X Y) D).
Proof.
  intros E1 X Y K' E2 D1 D2 D Split yn.
  remember (E1++[(X, bind_kn K')]++E2) as E.
  generalize dependent E1.
  generalize dependent X.
...
Lemma typing_typ_renaming_one: forall E1 X Y K' E2 D e t,
  typing (E1++[(X, bind_kn K')]++E2) D e t ->
  Y `notin` dom E1 `union` dom E2 `union` dom D ->
  typing (map (subst_tb X Y) E1 ++[(Y, bind_kn K')]++E2) (map (subst_tlb X Y) D) (subst_te X Y e) (subst_tt X Y t).
Proof.
  intros E1 X Y K' E2 D e t TYP yn.
  remember (E1++[(X, bind_kn K')]++E2) as E.
  generalize dependent E1.
  generalize dependent X.
  generalize dependent K'.
  ...
  Case "typing_var".    
    ...
    apply typing_var; auto.      
      ...
      rewrite <- map_subst_tb_id in BindsTac0; auto.
        rewrite_env ((E1++[(X, bind_kn K')])++E2) in J.
  ...
  Case "typing_abs".
    pick fresh z and apply typing_abs; auto.
      ...
      rewrite_env ((map (subst_tb X Y) ([(z, bind_typ T1)]++E1))++[(Y, bind_kn K')]++E2).
  ...
  Case "typing_app".
    ...
    apply dom_lenv_split in J.
    rewrite J in yn.   
    apply typing_app with (D1:=map (subst_tlb X Y) D1) (D2:=map (subst_tlb X Y) D2) (T1:=subst_tt X Y T1) (K:=K); eauto.
  ...
  Case "typing_tabs".
    pick fresh Z and apply typing_tabs; eauto.
      rewrite subst_tt_fresh with (T:=Z) (Z:=X) (U:=Y); auto.      
      rewrite <- subst_te_open_te; auto.
      apply value_through_subst_te; auto.

      rewrite_env ((map (subst_tb X Y) ([(Z, bind_kn K)]++E1))++[(Y, bind_kn K')]++E2).
  ...
  Case "typing_tapp".
    rewrite subst_tt_open_tt; auto.
    apply typing_tapp with (K:=K); auto.
      assert (typ_all K (subst_tt X Y T2) =  subst_tt X Y (typ_all K T2)) as EQ. auto.
      rewrite EQ.
      apply IHTYP; auto.
...
Lemma _wf_lenv_typ_permute: forall F E1 X K' E2 D,
  wf_lenv (F++E1++[(X, bind_kn K')]++E2) D ->
  wf_env (E1++E2) ->
  wf_lenv (F++[(X, bind_kn K')]++E1++E2) D.
Proof.
  intros F E1 X K' E2 D Wfle.
  remember (F++E1++[(X, bind_kn K')]++E2) as E.
  generalize dependent F.
  generalize dependent E1.
...
Lemma wf_lenv_typ_permute: forall E1 X K' E2 D,
  wf_lenv (E1++[(X, bind_kn K')]++E2) D->
  wf_env (E1++E2) ->
  wf_lenv ([(X, bind_kn K')]++E1++E2) D.
Proof.
  intros E1 X K' E2 D Wfle Wfle'.
  rewrite_env (nil++E1++[(X, bind_kn K')]++E2) in Wfle.
  apply _wf_lenv_typ_permute with (X:=X) (K':=K') (F:=nil) (E1:=E1) (E2:=E2) in Wfle; auto.
Qed.

Lemma _lenv_split_typ_permute: forall F E1 X K E2 D1 D2 D,
  lenv_split (F++E1++[(X, bind_kn K)]++E2) D1 D2 D ->
  wf_env (E1++E2) ->
  lenv_split (F++ [(X, bind_kn K)]++E1++E2)D1 D2 D.
Proof.
  intros F E1 X K E2 D1 D2 D Split Wfe.
  remember (F++E1++[(X, bind_kn K)]++E2) as E.
  generalize dependent F.
  generalize dependent E1.
  generalize dependent X.
...
Lemma _typing_typ_permute: forall F E1 X K E2 D e t,
  typing (F++E1++[(X, bind_kn K)]++E2) D e t ->
  wf_env (E1++E2) ->
  typing (F++[(X, bind_kn K)]++E1++E2) D e t.
Proof.
  intros F E1 X K E2 D e t TYP.
  remember (F++E1++[(X, bind_kn K)]++E2) as E.
  generalize dependent F.
  generalize dependent E1.
  generalize dependent X.
  ...
  Case "typing_abs".
    pick fresh y and apply typing_abs; auto.
      ...
      rewrite_env (([(y, bind_typ T1)]++F)++[(X, bind_kn K0)]++E1++E2).
      apply H1; auto.
  Case "typing_labs".
    pick fresh y and apply typing_labs; auto.
      apply _wf_typ_typ_permute; auto.
  ...
  Case "typing_tabs".
    pick fresh Z and apply typing_tabs; eauto.
      rewrite_env (([(Z, bind_kn K)]++F)++[(X, bind_kn K0)]++E1++E2).
      apply H1; auto.
  Case "typing_tapp".
    apply typing_tapp with (K:=K); auto.
      apply _wf_typ_typ_permute; auto.
...
Lemma typing_typ_permute: forall E1 X K E2 D e t,
  typing (E1++[(X, bind_kn K)]++E2) D e t ->
  wf_env (E1++E2) ->
  typing ([(X, bind_kn K)]++E1++E2) D e t.
Proof.
  intros E1 X K E2 D e t TYP Wfe.
  rewrite_env (nil++E1++[(X, bind_kn K)]++E2) in TYP.
  apply _typing_typ_permute in TYP; auto.
Qed.

Lemma typing_typ_renaming_permute : forall E' E D e t (X Y:atom) K,
  typing (E'++[(X,bind_kn K)]++E) D e t ->
  wf_lenv (E'++E) D ->
  Y `notin` dom E `union` dom E' `union` dom D ->
  typing ([(Y,bind_kn K)]++E'++E) D (subst_te X Y e) (subst_tt X Y t).
Proof.
  intros E' E D e t X Y K Typ Wfle yndom.
...
Proof.
  ...
  induction Hrel_subst; intros; subst.
    ...
    destruct EQ as [v [lgsubst0 [v' [lgsubst0' [EQ1 [EQ2 [EQ3 [EQ4 [Wft [Typ [Typ' [Hrel Sub]]]]]]]]]]]]; subst.
    exists v. exists lgsubst0.
    exists v'. exists lgsubst0'.
    simpl_env.
    split; auto.
    ...
    split.
      ...
      apply Frel_weaken_head with (E:=E) (K:=K); auto.
    ...
    apply IHHrel_subst in EQ.
    destruct EQ as [v0 [lgsubst0 [v'0 [lgsubst0' [EQ1 [EQ2 [EQ3 [EQ4 [Wft [Typ [Typ' [Hrel Sub]]]]]]]]]]]]; subst.
...
Lemma Frel_stronger_head:  forall t E v v' rsubst dsubst dsubst' X R t2 t2' K,
  ...
  wf_delta_subst ([(X, bind_kn K)]++E) ([(X, t2)]++dsubst) ->
  wf_delta_subst ([(X, bind_kn K)]++E) ([(X, t2')]++dsubst') ->
  F_related_values t rsubst dsubst dsubst' v v'
  .
Proof.
  intros t E v v' rsubst dsubst dsubst' X R t2 t2' K Hrel HX Hwfr Rsubst Dsubst Dsubst'.
  rewrite_env (nil ++ [(X, R)] ++ rsubst) in Hrel.
  rewrite_env (nil ++ [(X, t2)] ++ dsubst) in Hrel.
  rewrite_env (nil ++ [(X, t2')] ++ dsubst') in Hrel.
  apply Frel_stronger with (E:=E) (E':=nil) (K:=K) in Hrel; auto.
  ...
  Case "wf_rho_subst_srel".
    ...
    apply one_eq_app in HeqrS. destruct HeqrS as [[rS0 [rEQ1 rEQ2]] | [rEQ1 rEQ2]]; subst.
      apply one_eq_app in H3. destruct H3 as [[dsubst1 [dEQ1 dEQ2]] | [dEQ1 dEQ2]]; subst.
        apply one_eq_app in H7. destruct H7 as [[dsubst1' [dEQ1' dEQ2']] | [dEQ1' dEQ2']]; subst.
        ...
        apply one_eq_app in H7. destruct H7 as [[dsubst1' [dEQ1' dEQ2']] | [dEQ1' dEQ2']]; subst.
          apply one_eq_app in HeqEE. destruct HeqEE as [[E0' [eEQ1' eEQ2']] | [eEQ1' eEQ2']]; subst.
            assert (X `notin` fv_env (E0'++E)) as XnE.
              ...
              apply wf_rho_subst__uniq in Hwfr.
              decompose [and] Hwfr; auto.
            apply Frel_weaken_head with (E:=E0'++E) (K:=k); auto.
               apply IHHwfr with (dsubst'0:=dsubst1') (dsubst0:=dsubst1) (rsubst':=rS0) (E':=E0'); auto.
                 apply ddom_dom__inv with (X:=X)(b:=T)(K:=k); auto.
                   destruct_notin.
                   apply dom_delta_subst in H4.
               ...
               apply dom_delta_subst in H4. auto.

               apply dom_delta_subst in H8. auto.
...
Lemma delta_subst_opt' :  forall E E' dsubst dsubst' X k t e,
...
Proof.
  intros E E' dsubst dsubst' X k t e Hwf_delta_subst Heq Heq' Typ.
...
Definition P_Frel_typ_permute_renaming_one (n:nat) := 
  ((forall t v v' E1 E2 E3 rsubst1 rsubst2 rsubst3 dsubst1 dsubst2 dsubst3 dsubst1' dsubst2' dsubst3' X R t2 t2' Y K,
  typ_size t = n ->
  ...
  wf_delta_subst (E1++E2++[(X, bind_kn K)]++E3) (dsubst1++dsubst2++[(X, t2)]++dsubst3) ->
  wf_delta_subst (E1++E2++[(X, bind_kn K)]++E3) (dsubst1'++dsubst2'++[(X, t2')]++dsubst3') ->
  wf_delta_subst (E1++[(Y, bind_kn K)]++E2++E3) (dsubst1++[(Y, t2)]++dsubst2++dsubst3) ->
  wf_delta_subst (E1++[(Y, bind_kn K)]++E2++E3) (dsubst1'++[(Y, t2')]++dsubst2'++dsubst3') ->
  ...
  *
  (forall t v v' E1 E2 E3 (rsubst1 rsubst2 rsubst3:rho_subst) dsubst1 dsubst2 dsubst3 dsubst1' dsubst2' dsubst3' (X:atom) (R:rel) t2 t2' (Y:atom) K,
  typ_size t = n ->
  (F_related_values (subst_tt X Y t) (rsubst1++[(Y, R)]++rsubst2++rsubst3) (dsubst1++[(Y, t2)]++dsubst2++dsubst3) (dsubst1'++[(Y, t2')]++dsubst2'++dsubst3') v v' ->
  ddom_env E1 [=] dom rsubst1 ->
  ddom_env E1 [=] dom dsubst1 ->
...
Proof.
  ...
  split.
  (* -> *)
  intros t v v' E1 E2 E3 rsubst1 rsubst2 rsubst3 dsubst1 dsubst2 dsubst3 dsubst1' dsubst2' dsubst3' X R t2 t2' Y K
  ...
  repeat(split; simpl_env; auto).
     ...
     apply Hrel_wft1'_wft1 with (E1:=E1) (E2:=E2) (E3:=E3) (K:=K) in Hrel_wft1'; auto; simpl_env.
       ...
       destruct (@Harrow x x') as [u [u' [Hnorm_vxu [Hnorm_v'x'u' Hrel_wft1]]]]; auto.
         ...
         rewrite delta_subst_opt with (E:=E2++E3) (E':=E1) (K:=K) in Htypingx; auto.
         ...
         assert (X `notin` dom (E1++E2) `union` dom E3) as Xn.
           ...
           decompose [and] Wfr.
           solve_uniq.
         rewrite delta_subst_opt with (E:=E3) (E':=E1++E2) (K:=K); auto.

         simpl_env.
         rewrite subst_tt_tt with (K:=K) in Htypingx; auto.
  ...
  repeat(split; simpl_env in *; auto).
     ...
     apply Hrel_wft1_wft1' with (X:=X)(Y:=Y)(R:=R)(t2:=t2)(t2':=t2')(E1:=E1)(E2:=E2)(E3:=E3)(K:=K) in Hrel_wft1; auto; simpl_env.
       ...
       destruct (@Harrow x x') as [u [u' [Hnorm_vxu [Hnorm_v'x'u' Hrel_wft1']]]]; auto.
         ...
         assert (wf_typ nil t2 K) as Wft2.
           apply wfr_left_inv in WfR; auto.
         rewrite delta_subst_opt with (E:=E2++E3) (E':=E1) (K:=K); auto.
         assert (ddom_env (E1++E2) [=] dom (dsubst1++dsubst2)) as EQ2.
           simpl_env. rewrite Heqd1. rewrite Heqd2. 
           clear. fsetdec.
         ...
         assert (X `notin` dom (E1++E2) `union` dom E3) as Xn.
           ...
           decompose [and] Wfr.
           solve_uniq.
         rewrite delta_subst_opt with (E:=E3) (E':=E1++E2) (K:=K) in Htypingx; auto.
         simpl_env  in Htypingx.
         rewrite subst_tt_tt with (K:=K); auto.
...
Lemma Frel_typ_permute_renaming_one': forall t v v' E1 E2 E3 rsubst1 rsubst2 rsubst3 dsubst1 dsubst2 dsubst3 dsubst1' dsubst2' dsubst3' X R t2 t2' Y K,
  F_related_values t (rsubst1++rsubst2++[(X, R)]++rsubst3) (dsubst1++dsubst2++[(X, t2)]++dsubst3) (dsubst1'++dsubst2'++[(X, t2')]++dsubst3') v v' ->
  ddom_env E1 [=] dom rsubst1 ->
  ddom_env E1 [=] dom dsubst1 ->
  ddom_env E1 [=] dom dsubst1' ->
...
Lemma Frel_typ_permute_renaming_one: forall t v v' E1 E2 rsubst1 rsubst2 dsubst1 dsubst2 dsubst1' dsubst2' X R t2 t2' Y K,
  ...
  wf_delta_subst (E1++[(X, bind_kn K)]++E2) (dsubst1++[(X, t2)]++dsubst2) ->
  wf_delta_subst (E1++[(X, bind_kn K)]++E2) (dsubst1'++[(X, t2')]++dsubst2') ->
  wf_delta_subst ([(Y, bind_kn K)]++E1++E2) ([(Y, t2)]++dsubst1++dsubst2) ->
  wf_delta_subst ([(Y, bind_kn K)]++E1++E2) ([(Y, t2')]++dsubst1'++dsubst2') ->
...
Proof.
  ...
  rewrite_env (nil++[(Y, t2')]++dsubst1'++dsubst2').
  apply Frel_typ_permute_renaming_one' with (E1:=nil) (E2:=E1) (E3:=E2) (K:=K); auto.
Qed.
...
Lemma F_related_subst_lgweaken : forall D1 E D2 gsubst gsubst' lgsubst1 lgsubst2 lgsubst1' lgsubst2' rsubst dsubst dsubst' x T e e',
  ...
  typing nil nil e (apply_delta_subst_typ dsubst T) ->
  typing nil nil e' (apply_delta_subst_typ dsubst' T) ->
  F_related_values T rsubst dsubst dsubst' e e' ->
...
Lemma F_related_subst_dweaken : forall E1 E2 D gsubst gsubst' lgsubst lgsubst' rsubst1 rsubst2 dsubst1 dsubst2 dsubst1' dsubst2' X t t' R K,
  ...
  wfr R t t' K ->
  F_related_subst (E1++[(X, bind_kn K)]++E2) D gsubst gsubst' lgsubst lgsubst' (rsubst1++[(X, R)]++rsubst2) (dsubst1++[(X, t)]++dsubst2) (dsubst1'++[(X, t')]++dsubst2').
Proof.
  intros E1 E2 D gsubst gsubst' lgsubst lgsubst' rsubst1 rsubst2 dsubst1 dsubst2 dsubst1' dsubst2' X t t' R K
  ...
  Case "F_related_subst_typ".
    ...
    apply one_eq_app in HeqE. destruct HeqE as [[E0' [eEQ1' eEQ2']] | [eEQ1' eEQ2']]; subst.
      ...
      simpl_env.
      apply F_related_subst_typ; auto.
        erewrite delta_subst_opt with (E:=E2) (E':=E0') (K:=K); eauto.
          rewrite <- subst_tt_fresh; auto.
        ...
        apply F_related_subst__inversion in Hrel_subst.
        decompose [prod] Hrel_subst.
        apply Frel_weaken with (E:=E2) (E':=E0') (K:=K); auto.
          assert (X `notin` fv_env (E0'++E2)) as XnE.
            apply wfe_dom_fv_env; auto.
  ...
  Case "F_related_subst_ltyp".
    simpl_env in *.
    apply F_related_subst_ltyp; auto.
      erewrite delta_subst_opt with (E:=E2) (E':=E1) (K:=K); eauto.
        rewrite <- subst_tt_fresh; auto.
          apply notin_fv_wf with (X:=X) in H3; auto.
      ...
      apply F_related_subst__inversion in Hrel_subst.
      decompose [prod] Hrel_subst.
      apply Frel_weaken with (E:=E2) (E':=E1) (K:=K); auto.
...
Lemma F_related_subst_gweaken : forall E1 E2 D gsubst1 gsubst2 gsubst1' gsubst2' lgsubst lgsubst' rsubst1 rsubst2 dsubst1 dsubst2 dsubst1' dsubst2' x T e e',
  ...
  typing nil nil e' (apply_delta_subst_typ dsubst2' T) ->
  F_related_values T rsubst2 dsubst2 dsubst2' e e' ->
...
Lemma F_Rsubst_dweaken : forall E1 E2 rsubst1 rsubst2 dsubst1 dsubst2 dsubst1' dsubst2' X R t2 t2' K,
  ...
  X `notin` (dom E1 `union` dom E2)->
  wfr R t2 t2' K ->
  F_Rsubst (E1++[(X, bind_kn K)]++E2) (rsubst1++[(X, R)]++rsubst2) (dsubst1++[(X, t2)]++dsubst2) (dsubst1'++[(X, t2')]++dsubst2').
Proof.
  intros E1 E2 rsubst1 rsubst2 dsubst1 dsubst2 dsubst1' dsubst2' X R t2 t2' K HRsubst 
