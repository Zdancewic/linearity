...
Fixpoint close_tt_rec (K : nat) (X : atom) (T : typ) {struct T} : typ :=
  match T with
    ...
    | typ_arrow k T1 T2 => typ_arrow k (close_tt_rec K X T1) (close_tt_rec K X T2)
    | typ_all k T1 => typ_all k (close_tt_rec (S K) X T1)
...
Fixpoint close_te_rec (K : nat) (X : atom) (e : exp) {struct e} : exp :=
  match e with
    ...
    | exp_abs k V e1 => exp_abs  k (close_tt_rec K X V)  (close_te_rec K X e1)
    | exp_app e1 e2 => exp_app  (close_te_rec K X e1) (close_te_rec K X e2)
    | exp_tabs k e1 => exp_tabs k (close_te_rec (S K) X e1)
...
Fixpoint close_ee_rec (k : nat) (x : atom) (e : exp) {struct e} : exp :=
  match e with
    ...
    | exp_abs K V e1 => exp_abs K V (close_ee_rec (S k) x e1)
    | exp_app e1 e2 => exp_app (close_ee_rec k x e1) (close_ee_rec k x e2)
    | exp_tabs K e1 => exp_tabs K (close_ee_rec k x e1)
...
Inductive ctx : Set :=
  | ctx_hole  : ctx
  | ctx_abs_free  : kn -> typ -> ctx ->  ctx 
  | ctx_abs_capture  : kn -> atom -> typ -> ctx ->  ctx 
  | ctx_app1  : ctx -> exp -> ctx
  | ctx_app2  : exp -> ctx ->  ctx
  | ctx_tabs_free  :  kn -> ctx ->  ctx 
  | ctx_tabs_capture  :  atom -> kn -> ctx ->  ctx 
...
Fixpoint open_tc_rec (K : nat) (U : typ) (C : ctx)  {struct C} : ctx :=
  match C with
  | ctx_hole  => ctx_hole
  | ctx_abs_free k V C1 => ctx_abs_free k (open_tt_rec K U V) (open_tc_rec K U C1) 
  | ctx_abs_capture k x V C1 => ctx_abs_capture k x (open_tt_rec K U V) (open_tc_rec K U C1) 
  ...
  | ctx_tabs_free k C1 => ctx_tabs_free k (open_tc_rec (S K) U C1)  
  | ctx_tabs_capture X k C1 => ctx_tabs_capture X k (open_tc_rec (S K) U C1)  
...
Fixpoint open_ec_rec (K : nat) (f : exp) (C : ctx)  {struct C} : ctx :=
  match C with
  | ctx_hole  => ctx_hole
  | ctx_abs_free k V C1 => ctx_abs_free k V (open_ec_rec (S K) f C1) 
  | ctx_abs_capture k x V C1 => ctx_abs_capture k x V (open_ec_rec (S K) f C1) 
  ...
  | ctx_tabs_free k C1 => ctx_tabs_free k (open_ec_rec K f C1)  
  | ctx_tabs_capture X k C1 => ctx_tabs_capture X k (open_ec_rec K f C1)  
...
Fixpoint close_tc_rec (K : nat)(X : atom) (C : ctx)  {struct C} : ctx :=
  match C with
  | ctx_hole  => ctx_hole
  | ctx_abs_free k V C1 => ctx_abs_free k (close_tt_rec K X V) (close_tc_rec K X C1) 
  | ctx_abs_capture k x V C1 => ctx_abs_capture k x (close_tt_rec K X V) (close_tc_rec K X C1) 
  ...
  | ctx_tabs_free k C1 => ctx_tabs_free k (close_tc_rec (S K) X C1)  
  | ctx_tabs_capture Y k C1 => ctx_tabs_capture Y k (close_tc_rec (S K) X C1)  
...
Fixpoint close_ec_rec (K : nat) (x : atom) (C : ctx)  {struct C} : ctx :=
  match C with
  | ctx_hole  => ctx_hole
  | ctx_abs_free k V C1 => ctx_abs_free k V (close_ec_rec (S K) x C1) 
  | ctx_abs_capture k y V C1 => ctx_abs_capture k y V (close_ec_rec (S K) x C1) 
  ...
  | ctx_tabs_free k C1 => ctx_tabs_free k (close_ec_rec K x C1)  
  | ctx_tabs_capture X k C1 => ctx_tabs_capture X k (close_ec_rec K x C1)  
...
Inductive context : ctx -> Prop :=
  ...
  | context_abs_free  :  forall L K T C1,
      type T ->
      (forall x : atom, x `notin` L -> context (open_ec C1 x)) ->
      context (ctx_abs_free K T C1) 
  | context_abs_capture  :  forall L K y T C1,
      type T ->
      (forall x : atom, x `notin` L -> context (open_ec C1 x)) ->
      context (ctx_abs_capture K y T C1) 
  ...
  | context_tabs_free  :  forall L K C1,
      (forall X : atom, X `notin` L -> context (open_tc C1 X)) ->
      context (ctx_tabs_free K C1) 
  | context_tabs_capture  :  forall L Y K C1,
      (forall X : atom, X `notin` L -> context (open_tc C1 X)) ->
      context (ctx_tabs_capture Y K C1) 
  ...
  | context_snf : forall C1, context C1 -> context (ctx_snd C1)
  .

Inductive vcontext : ctx -> Prop :=
  | vcontext_abs_free  : forall K T C,  context (ctx_abs_free K T C) -> vcontext (ctx_abs_free K T C)
  | vcontext_abs_capture  : forall K x T C,  context (ctx_abs_capture K x T C) -> vcontext (ctx_abs_capture K x T C)
  | vcontext_tabs_free  :  forall K C,  context (ctx_tabs_free K C) -> vcontext (ctx_tabs_free K C)
  | vcontext_tabs_capture  :  forall X K C,  context (ctx_tabs_capture X K C) -> vcontext (ctx_tabs_capture X K C)
...
Fixpoint shift_ee_rec (k : nat) (b : nat) (e : exp) {struct e} : exp :=
  match e with
    ...
    | exp_fvar x => exp_fvar x
    | exp_abs K V e1 => exp_abs K V (shift_ee_rec k (S b) e1)
    | exp_app e1 e2 => exp_app (shift_ee_rec k b e1) (shift_ee_rec k b e2)
    | exp_tabs K e1 => exp_tabs K (shift_ee_rec k b e1)
...
Fixpoint shift_tt_rec (k : nat) (b : nat) (t : typ) {struct t} : typ :=
  match t with
    ...
    | typ_arrow K t1 t2 => typ_arrow K (shift_tt_rec k b t1) (shift_tt_rec k b t2)
    | typ_all K t1 => typ_all K (shift_tt_rec k (S b) t1)
...
Fixpoint shift_te_rec (k : nat) (b : nat) (e : exp) {struct e} : exp :=
  match e with
    | exp_bvar i => exp_bvar i
    | exp_fvar x => exp_fvar x
    | exp_abs K V e1 => exp_abs K (shift_tt_rec k b V) (shift_te_rec k b e1)
    | exp_app e1 e2 => exp_app (shift_te_rec k b e1) (shift_te_rec k b e2)
    | exp_tabs K e1 => exp_tabs K (shift_te_rec k (S b) e1)
...
Fixpoint plug (C : ctx) (u : exp) {struct C} : exp :=
  match C  with 
  | ctx_hole  => u
  | ctx_abs_free K V C1   => exp_abs K V (plug C1 (shift_ee u ))
  | ctx_abs_capture K x V C1   => exp_abs K V (plug C1 (close_ee (shift_ee u )x))
  | ctx_app1 C1 e2   => exp_app (plug C1 u) e2
  | ctx_app2  v1 C2  => exp_app v1 (plug C2 u)
  | ctx_tabs_free K C1   => exp_tabs K (plug C1 (shift_te u)) 
  | ctx_tabs_capture X K C1   => exp_tabs K (plug C1 (close_te (shift_te u) X)) 
...
Fixpoint remove_gdom (E:env) {struct E } : env :=
  match E with
  | nil => nil
  | (X, bind_kn K)::E => (X, bind_kn K)::(remove_gdom E)
...
Fixpoint shift_ec_rec (k : nat) (b : nat) (C : ctx) {struct C} : ctx :=
  match C with
  | ctx_hole => ctx_hole
  | ctx_abs_free K V C1 => ctx_abs_free K V (shift_ec_rec k (S b) C1)
  | ctx_abs_capture K x V C1 => ctx_abs_capture K x V (shift_ec_rec k (S b) C1)
  ...
  | ctx_tabs_free K C1 => ctx_tabs_free K (shift_ec_rec k b C1)
  | ctx_tabs_capture x K C1 => ctx_tabs_capture x K (shift_ec_rec k b C1)
...
Fixpoint shift_tc_rec (k : nat) (b : nat) (C : ctx) {struct C} : ctx :=
  match C with
  | ctx_hole => ctx_hole
  | ctx_abs_free K V C1 => ctx_abs_free K (shift_tt_rec k b V) (shift_tc_rec k b C1)
  | ctx_abs_capture K x V C1 => ctx_abs_capture K x (shift_tt_rec k b V) (shift_tc_rec k b C1)
  ...
  | ctx_tabs_free K C1 => ctx_tabs_free K (shift_tc_rec k (S b) C1)
  | ctx_tabs_capture x K C1 => ctx_tabs_capture x K (shift_tc_rec k (S b) C1)
...
Fixpoint plugC (C0 C:ctx) {struct C0} : ctx :=
  match C0  with 
  | ctx_hole  => C
  | ctx_abs_free K V C1   => ctx_abs_free K V (plugC C1 (shift_ec C ))
  | ctx_abs_capture K x V C1   => ctx_abs_capture K x V (plugC C1 (close_ec (shift_ec C ) x))
  | ctx_app1 C1 e2   => ctx_app1 (plugC C1 C) e2
  | ctx_app2  v1 C2  => ctx_app2 v1 (plugC C2 C)
  | ctx_tabs_free K C1   => ctx_tabs_free K (plugC C1 (shift_tc C)) 
  | ctx_tabs_capture X K C1   => ctx_tabs_capture X K (plugC C1 (close_tc (shift_tc C) X)) 
...
Fixpoint cbv_ctx (C : ctx)  {struct C} : Prop :=
  match C with
  | ctx_hole  => True
  | ctx_app1 C1 e2 => cbv_ctx C1 /\ expr e2 
  | ctx_app2  v1 C2 => value v1 /\ cbv_ctx C2
  | ctx_tapp C1 V => cbv_ctx C1 /\ type V
  | ctx_fst C1 => cbv_ctx C1
  | ctx_snd C1 => cbv_ctx C1
  | _ => False
  end.
...
Fixpoint fv_tc (C : ctx) {struct C} : atoms :=
  match C with
  | ctx_hole  => {}
  | ctx_abs_free k V C1 => (fv_tt V) `union` fv_tc C1
  | ctx_abs_capture k x V C1 => (fv_tt V) `union` fv_tc C1
  | ctx_app1  C1 e2 => (fv_tc C1) `union` (fv_te e2)
  | ctx_app2  v1 C2 => (fv_te v1) `union` (fv_tc C2)
  | ctx_tabs_free k C1 => fv_tc C1
  | ctx_tabs_capture X k C1 => fv_tc C1
...
Fixpoint remove_tmvar (E:env) {struct E} : env :=
  match E with
  | nil => nil 
  | (X, bind_kn K)::E' => (X, bind_kn K)::(remove_tmvar E')
...
Fixpoint cv_ec (C : ctx) {struct C} : atoms :=
  match C with
  | ctx_hole  => {}
  | ctx_abs_free k V C1 => cv_ec C1
  | ctx_abs_capture k x V C1 => {{x}} `union` cv_ec C1
  | ctx_app1  C1 e2 => (cv_ec C1)
  | ctx_app2  v1 C2 => (cv_ec C2)
  | ctx_tabs_free k C1 => cv_ec C1
  | ctx_tabs_capture X k C1 => {{X}} `union`  cv_ec C1
...
Fixpoint cv_tc (C : ctx) : atoms :=
  ...
  | ctx_abs_free _  _ C1 => (cv_tc C1)
  | ctx_abs_capture _ x _ C1 => (cv_tc C1)
  | ctx_app1 C1 _ => cv_tc C1
  | ctx_app2 _ C2 => cv_tc C2
  | ctx_tabs_free _ C1 => (cv_tc C1)
  | ctx_tabs_capture X _ C1 => union (singleton X) (cv_tc C1)
...
Fixpoint bexp_eupper (e : exp) {struct e} : nat :=
  ...
  | exp_bvar i => i+1
  | exp_fvar x => 0
  | exp_abs K T e => bexp_eupper e - 1
  | exp_app e1 e2 => max (bexp_eupper e1) (bexp_eupper e2)
  | exp_tabs K e => bexp_eupper e
...
Fixpoint btyp_upper (t : typ) {struct t} : nat :=
  ...
  | typ_bvar i => i+1
  | typ_fvar x => 0
  | typ_arrow k t1 t2 => max (btyp_upper t1) (btyp_upper t2)
  | typ_all K t => btyp_upper t - 1
...
Fixpoint bexp_tupper (e : exp) {struct e} : nat :=
  ...
  | exp_bvar i => 0
  | exp_fvar x => 0
  | exp_abs K T e => max (btyp_upper T) (bexp_tupper e)
  | exp_app e1 e2 => max (bexp_tupper e1) (bexp_tupper e2)
  | exp_tabs K e => bexp_tupper e - 1
...
Fixpoint fv_ec (C : ctx) {struct C} : atoms :=
  match C with
  | ctx_hole  => {}
  | ctx_abs_free k V C1 => fv_ec C1
  | ctx_abs_capture k x V C1 => fv_ec C1
  | ctx_app1  C1 e2 => (fv_ec C1) `union` (fv_ee e2)
  | ctx_app2  v1 C2 => (fv_ee v1) `union` (fv_ec C2)
  | ctx_tabs_free k C1 => fv_ec C1
  | ctx_tabs_capture X k C1 => fv_ec C1
...
Fixpoint subst_tc (Z : atom) (U : typ) (C : ctx) {struct C} : ctx :=
  match C with
  | ctx_hole => ctx_hole
  | ctx_abs_free K V C1 => ctx_abs_free K (subst_tt Z U V)  (subst_tc Z U C1)
  | ctx_abs_capture K x V C1 => ctx_abs_capture K x (subst_tt Z U V)  (subst_tc Z U C1)
  | ctx_app1 C1 e2 => ctx_app1  (subst_tc Z U C1) (subst_te Z U e2)
  | ctx_app2 v1 C2 => ctx_app2  (subst_te Z U v1) (subst_tc Z U C2)
  | ctx_tabs_free K C1 => ctx_tabs_free K  (subst_tc Z U C1)
  | ctx_tabs_capture x K C1 => ctx_tabs_capture x K  (subst_tc Z U C1)
...
Fixpoint subst_ec (z : atom) (u : exp) (C : ctx) {struct C} : ctx :=
  match C with
  | ctx_hole => ctx_hole
  | ctx_abs_free K V C1 => ctx_abs_free K V (subst_ec z u C1)
  | ctx_abs_capture K x V C1 => ctx_abs_capture K x V (subst_ec z u C1)
  | ctx_app1 C1 e2 => ctx_app1 (subst_ec z u C1) (subst_ee z u e2)
  | ctx_app2 e1 C2 => ctx_app2 (subst_ee z u e1) (subst_ec z u C2)
  | ctx_tabs_free K C1 => ctx_tabs_free K (subst_ec z u C1)
  | ctx_tabs_capture x K C1 => ctx_tabs_capture x K (subst_ec z u C1)
...
Fixpoint bctx_eupper (C : ctx) {struct C} : nat :=
  match C with
  | ctx_hole  => 0
  | ctx_abs_free k V C1 => bctx_eupper C1 - 1
  | ctx_abs_capture k x V C1 => bctx_eupper C1 - 1
  | ctx_app1  C1 e2 => max (bctx_eupper C1) (bexp_eupper e2)
  | ctx_app2  v1 C2 => max (bexp_eupper v1) (bctx_eupper C2)
  | ctx_tabs_free k C1 =>  bctx_eupper C1
  | ctx_tabs_capture X k C1 =>  bctx_eupper C1
...
Fixpoint bctx_tupper (C : ctx) {struct C} : nat :=
  match C with
  | ctx_hole  => 0
  | ctx_abs_free k V C1 =>max (btyp_upper V) (bctx_tupper C1)
  | ctx_abs_capture k x V C1 =>max (btyp_upper V) (bctx_tupper C1)
  | ctx_app1  C1 e2 => max (bctx_tupper C1) (bexp_tupper e2)
  | ctx_app2  v1 C2 => max (bexp_tupper v1) (bctx_tupper C2)
  | ctx_tabs_free k C1 =>  bctx_tupper C1 -1
  | ctx_tabs_capture X k C1 =>  bctx_tupper C1 -1
...
Inductive contexting : env -> lenv -> typ -> ctx -> env -> lenv -> typ -> Prop :=
  | contexting_hole : forall E D T K,
      wf_lenv E D ->
      wf_typ E T K ->
      contexting E D T ctx_hole E D T
  | contexting_abs_free : forall L K E D T T1' C1 T2' E' D',
      wf_typ E' T1' kn_nonlin -> 
      (forall x : atom, x `notin` L->     
        contexting E D T (open_ec C1 x) ([(x, bind_typ T1')] ++ E') D' T2') ->
      (K = kn_nonlin -> D' = lempty) ->
      contexting E D T (ctx_abs_free K T1' C1) E' D' (typ_arrow K T1' T2')
  | contexting_labs_free : forall L K E D T T1' C1 T2' E' D',
      wf_typ E' T1' kn_lin -> 
      (forall x : atom, x `notin` L ->     
        contexting E D T (open_ec C1 x) E' ([(x, lbind_typ T1')] ++ D') T2') ->
      (K = kn_nonlin -> D' = lempty) ->
      contexting E D T (ctx_abs_free K T1' C1) E' D' (typ_arrow K T1' T2')
  | contexting_abs_capture : forall K E D T y T1' C1 T2' E' D',
      ...
      (K = kn_nonlin -> D' = lempty) ->
      contexting E D T (ctx_abs_capture K y T1' (close_ec C1 y)) (env_remove (y, (bind_typ T1')) E') D' (typ_arrow K T1' T2')
  | contexting_labs_capture : forall K E D T y T1' C1 T2' E' D',
      wf_typ E' T1' kn_lin -> 
      binds y (lbind_typ T1') D' ->
      y `notin` gdom_env E `union` cv_ec C1 ->
      contexting E D T C1 E' D' T2' ->
      (K = kn_nonlin -> lenv_remove (y, (lbind_typ T1')) D' = lempty) ->
      contexting E D T (ctx_abs_capture K y T1' (close_ec C1 y)) E' (lenv_remove (y, (lbind_typ T1')) D') (typ_arrow K T1' T2')
  | contexting_app1 : forall T1' K E D T E' D1' D2' D3' C1 e2 T2',
  ...
  | contexting_app2 : forall T1' K E D T E' D1' D2' D3' e1 C2 T2',
      typing E' D1' e1 (typ_arrow K T1' T2') ->
      contexting E D T C2 E' D2' T1' ->
      lenv_split E' D1' D2' D3' ->
      disjdom (fv_ee e1) (dom D) ->
      contexting E D T (ctx_app2 e1 C2) E' D3' T2'
  | contexting_tabs_free : forall L E D T K C1 T1' E' D',
      (forall X : atom, X `notin` L -> vcontext (open_tc C1 X)) ->
      (forall X : atom, X `notin` L ->   
        contexting E D T (open_tc C1 X) ([(X, bind_kn K)] ++ E') D' (open_tt T1' X)) ->
      contexting E D T (ctx_tabs_free K C1) E' D' (typ_all K T1')
  | contexting_tabs_capture : forall E D T Y K C1 T1' E' D',
      binds Y (bind_kn K) E' ->
      Y `notin` cv_ec C1 ->
      vcontext C1 ->
      contexting E D T C1 E' D' T1' ->
      wf_lenv (env_remove (Y, (bind_kn K)) E') D'  ->
      contexting E D T (ctx_tabs_capture Y K (close_tc C1 Y)) (env_remove (Y, (bind_kn K)) E') D' (typ_all K (close_tt T1' Y))
  | contexting_tapp : forall K E D T C1 T' T2' E' D',
...
Tactic Notation "contexting_cases" tactic(first) tactic(c) :=
  first;
  [ c "contexting_hole" |
     c "contexting_abs_free" | c "contexting_labs_free" |  
     c "contexting_abs_capture" | c "contexting_labs_capture" |  
  ...
  induction Wfe; intros; simpl.
    ...
    destruct b.
      analyze_binds H0.
        inversion BindsTacVal; subst.
          ...
          destruct (eq_binding_dec (y, bind_kn k) (X, bind_kn K)).
            inversion e; subst.
            exists nil. exists E.
            split; simpl; auto.
               apply env_remove_notin; auto.
      ...
      analyze_binds H0.
        apply IHWfe in BindsTac.
        destruct BindsTac as [E1 [E2 [EQ1 EQ2]]]; subst.
        destruct (eq_binding_dec (y, bind_typ t) (X, bind_kn K)).
          inversion e.
    ...
    destruct b.
      analyze_binds H1.
        apply IHWfe in BindsTac.
        destruct BindsTac as [E1 [E2 [EQ1 EQ2]]]; subst.
        destruct (eq_binding_dec (y, bind_kn k) (x, bind_typ T)).
          inversion e.
...
Proof.
  induction E; intros y b; simpl.
    ...
    destruct a.   
    destruct b0. 
      destruct (eq_binding_dec (y,b)  (a, bind_kn k)); simpl.
        inversion e; subst. 
        assert (J:=@IHE a (bind_kn k)). 
        fsetdec.
...
Proof.
  ...
  induction Wfe; intros t y Binds; simpl.
    ...
    destruct (eq_binding_dec (y, bind_typ t) (X, bind_kn K)); simpl.
      inversion e; subst.
...
Lemma wft_fv_tt_dsub : forall E t K,
  wf_typ E t K ->
  fv_tt t [<=] ddom_env E.
Proof.
  intros E t K Wft.
  induction Wft; simpl.
    ...
    apply H0 in XnL.
    assert (fv_tt T2 [<=] fv_tt (open_tt T2 X)) as sub.
      apply open_tt_fv_tt_lower; auto.
    assert (fv_tt T2 [<=] ddom_env ([(X, bind_kn K1)]++E)) as sub'.
      fsetdec.
    assert (AtomSetImpl.diff (fv_tt T2) {{X}} [<=] AtomSetImpl.diff (ddom_env ([(X, bind_kn K1)]++E)) {{X}}) as sub''.
      fsetdec.
    assert (AtomSetImpl.diff (fv_tt T2) {{X}}[=](fv_tt T2)) as eq.
      clear XnL sub sub' sub''. fsetdec.
    assert (X `notin` ddom_env E) as XnE.
      apply free_env__free_ddom; auto.
    assert (AtomSetImpl.diff (ddom_env ([(X, bind_kn K1)]++E)) {{X}}[=]ddom_env E) as eq'.
...
Lemma wft_strengthen_sub : forall E F T K,
  wf_env (F++E) ->
  wf_typ (F++E) T K -> 
  fv_tt T [<=] ddom_env E -> 
  wf_typ E T K.
Proof.
  intros E F T K Hwfe Hwft Hsub.
  induction F; auto.
    simpl_env in Hwfe.
    destruct a. destruct b; simpl_env in Hwft.
      ...
      rewrite_env (nil ++ [(a, bind_kn k)] ++ (F++E)) in Hwft.
...
Lemma wf_env_notin_fv_env : forall E1 X K E2,
  wf_env (E1++[(X,bind_kn K)]++E2) ->
  wf_env (E1++E2) ->
  X `notin` fv_env E1 `union` fv_env E2.
Proof.
  intros E1 X K E2 Wfe Wfe'.  
...
Lemma wf_lenv_notin_fv_env : forall E1 X K E2 D,
  wf_lenv (E1++[(X,bind_kn K)]++E2) D ->
  wf_lenv (E1++E2) D ->
  X `notin` fv_env E1 `union` fv_env E2 `union` fv_lenv D.
Proof.
  intros E1 X K E2 D Wfle Wfle'.  
  remember (E1++[(X, bind_kn K)]++E2) as E.
  generalize dependent E1.
  induction Wfle; intros; subst.
    apply wf_env_notin_fv_env with (E1:=E1) (E2:=E2) (X:=X) (K:=K) in H; auto.
...
Lemma bind_kn__in_ddom : forall E X K,
  binds X (bind_kn K) E ->
  X `in` ddom_env E.
Proof.
  induction E; intros X K Binds.
    inversion Binds.
...
Lemma in_fv_wf' : forall E (X : atom) T K,
  wf_typ E T K ->
  X `in` fv_tt T ->
  X `in` ddom_env E.
Proof.
...
Proof.
  induction E as [ | y b F IH ]; intros J.
    ...
    simpl_alist in J. destruct y.  destruct b0; simpl in J.
      ...
      apply F in J. destruct J as [k J].
      exists k. auto.
Qed.

Lemma empty_wft_disjdom : forall t k D,
  wf_typ nil t k ->
  disjdom (fv_tt t) D.
Proof.
  intros t0 k0 D Htyp.
...
Lemma subst_tt_tt : forall T (X Y:atom) t K,
  wf_typ nil t K ->
  Y `notin` fv_tt T ->
  subst_tt Y t (subst_tt X Y T) = subst_tt X t T.
Proof.
  induction T; intros X Y t K Wft Fv; simpl; auto.
    ...
    simpl in Fv.
    rewrite IHT1 with (K:=K); auto.
    rewrite IHT2 with (K:=K); auto.

    simpl in Fv.
    rewrite IHT with (K:=K); auto.
...
Proof.
  ...
  induction Wfe; intros; simpl.
    ...
    destruct yin as [yin | yin].
      destruct (y==X).
        inversion e; subst.
        exists nil. exists E. exists (bind_kn K).
        split; auto.
        apply env_remove2_notin; auto.
      ...
      destruct (y==X).
        ...
        exists ((X, bind_kn K)::E1). exists E2. exists b.
        split; simpl; auto.
          simpl_env. rewrite EQ1. auto.
