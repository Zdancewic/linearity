...
Require Import LinF_OParametricity_Pre.
Require Export LinF_OParametricity_Interface.
...
Lemma wfor_inv : forall E r T T' K,
  wfor E r T T' K->
  (wf_typ E T K) /\
  (wf_typ E T' K) /\
  admissible E r
...
Lemma wfor_left_inv : forall E t t' R K,
...
Lemma wfor_right_inv : forall E t t' R K,
  wfor E R t t' K -> wf_typ E t' K.
Proof.
  intros. 
...
Lemma wfor_renaming_inv : forall E r T T' K,
  wfor E r T T' K->
  admissible E r
  .
Proof.
...
Lemma F_Related_ovalues_arrow_eq: forall (t1 t2:typ) (rsubst:rho_subst)(dsubst dsubst':delta_subst)(v v':exp) K Env lEnv,
...
Proof. 
  intros.
  unfold_sub F_Related_ovalues (typ_arrow K t1 t2).
  split; intros H; decompose [and] H. auto. 
    split; auto.
Qed.

Lemma F_Related_ovalues_arrow_leq: forall (t1 t2:typ) (rsubst:rho_subst)(dsubst dsubst':delta_subst)(v v':exp) K Env lEnv,
  F_Related_ovalues (typ_arrow K t1 t2) rsubst dsubst dsubst' v v' Env lEnv
  ->
  (
    (value v) /\ (value v') /\
...
Proof. 
  intros.
  destruct (@F_Related_ovalues_arrow_eq t1 t2 rsubst dsubst dsubst' v v' K Env lEnv); auto.
Qed.

Lemma F_Related_ovalues_arrow_req: forall (t1 t2:typ) (rsubst:rho_subst)(dsubst dsubst':delta_subst)(v v':exp) K Env lEnv,
  (
    (value v) /\ (value v') /\
    (exists L,
      (forall lEnv1 x x',
...
Lemma F_Related_ovalues_all_eq: forall (t1:typ) (rsubst:rho_subst)(dsubst dsubst':delta_subst)(v v':exp) K Env lEnv,
...
Proof. 
  intros.
  unfold_sub F_Related_ovalues (typ_all K t1).
...
Lemma F_Related_ovalues_all_leq: forall (t1:typ) (rsubst:rho_subst)(dsubst dsubst':delta_subst)(v v':exp) K Env lEnv,
  F_Related_ovalues (typ_all K t1) rsubst dsubst dsubst' v v' Env lEnv
  ->
      (
      (value v) /\ (value v') /\
...
Proof. 
  intros.
  destruct (@F_Related_ovalues_all_eq t1 rsubst dsubst dsubst' v v' K Env lEnv); auto.
Qed.

Lemma F_Related_ovalues_all_req: forall (t1:typ) (rsubst:rho_subst)(dsubst dsubst':delta_subst)(v v':exp) K Env lEnv,
      (
      (value v) /\ (value v') /\
      (exists L,
         (forall (X:atom) t2 t2' R (Fr:X `notin` L),
...
Proof.
  ...
  Case "F_Related_osubst_kind".
    ...
    repeat (split; eauto using wfor_left_inv, wfor_right_inv).
      ...
      rewrite_env (nil ++ [(X, bind_kn K)] ++ E). apply wf_lenv_weakening; auto.
...
Proof.
 ...
 (typ_cases (destruct t) Case).
  ...
  repeat(split; auto).     
     ...
     repeat(split; auto).
       assert (typ_size t2 < typ_size (typ_arrow k t1 t2)) as G2. simpl. omega.
       simpl in Fvy0.
       apply H with (e:=u)(e':=u')(t:=t2)(rsubst:=rsubst)(dsubst:=dsubst)(dsubst':=dsubst')(Env:=Env)(lEnv:=lEnv)(lEnv':=lEnv1++lEnv')(x0:=x0)(y0:=y0)(b:=b) (E:=E)in G2; simpl_env in *; auto.
         apply preservation_normalization with (e:=exp_app e x); auto.
           ...
           apply typing_app with (D2:=lEnv1) (D1:=lEnv'++[(x0,b)]++lEnv)(T1:=apply_delta_subst_typ dsubst t1)(K:=k); auto.
  ...
  repeat(split; auto).
     ...
     apply H with (e:=w0)(e':=w'0)(t:=(open_tt t X1))(rsubst:=[(X1,R0)]++rsubst)(dsubst:=[(X1,t3)]++dsubst)(dsubst':=[(X1,t2'0)]++dsubst')(Env:=Env)(lEnv:=lEnv)(lEnv':=lEnv')(x0:=x0)(y0:=y0)(b:=b)(E:=[(X1,bind_kn k)]++E) in G; simpl_env; simpl; auto; simpl_env.
       ...
       apply preservation_normalization with (e:=exp_tapp e t3); auto.
         ...
         rewrite delta_osubst_closed_typ with (t:=t3); auto.
           apply typing_tapp with (K:=k); eauto using wfor_left_inv.
       ...
       apply preservation_normalization with (e:=exp_tapp e' t2'0); auto.
         ...
         rewrite delta_osubst_closed_typ with (t:=t2'0); auto.
           apply typing_tapp with (K:=k); eauto using wfor_right_inv.
...
Definition P_Forel_weaken_stronger (n:nat) := 
  ((forall t E E' v v' rsubst rsubst' dsubst dsubst' dsubst0 dsubst'0 Env lEnv X R t2 t2' K,
  typ_size t = n ->
...
Proof.
  ...
  split.
  (* -> *)
  intros t E E' v v' rsubst rsubst' dsubst dsubst' dsubst0 dsubst'0 Env lEnv X R t2 t2' K
  ...
  repeat(split; auto).
     ...
     simpl in Hfv.
     apply Hrel_wft1'_wft1 with (E:=E) (E':=E') (K:=K) in Hrel_wft1'; auto.
       SSCase "arrow".
       destruct (@Harrow lEnv1 x x') as [u [u' [Hnorm_vxu [Hnorm_v'x'u' Hrel_wft1]]]]; auto.
          apply typing_odsubst_stronger with (E:=E'++[(X,bind_kn K)]++E)(X:=X)(t:=t2); eauto.
  ...
  repeat(split; auto).
     ...
     apply H in G1. destruct G1 as [Hrel_wft1_wft1' Hrel_wft1'_wft1].
     simpl in Hfv.
     apply Hrel_wft1_wft1' with (X:=X)(R:=R)(t2:=t2)(t2':=t2')(K:=K) (E:=E) (E':=E') in Hrel_wft1; auto.
       SSCase "arrow".
       destruct (@Harrow lEnv1 x x') as [u [u' [Hnorm_vxu [Hnorm_v'x'u' Hrel_wft1']]]]; auto.
          apply typing_odsubst_weaken with (E:=E'++E); eauto using wfor_left_inv.
               apply odsubst_stronger with (t:=t2) (X:=X) (K:=K); eauto using wfor_left_inv.
          apply typing_odsubst_weaken with (E:=E'++E); eauto using wfor_right_inv.
               apply odsubst_stronger with (t:=t2') (X:=X) (K:=K); eauto using wfor_right_inv.
          destruct Disj as [J1 J2].
          split; intros x0 x0notin.
            apply J1; auto.
            apply J2 in x0notin; auto.
...
Lemma Forel_weaken: forall t E E' v v' rsubst rsubst' dsubst dsubst' dsubst0 dsubst'0 Env lEnv X R t2 t2' K,
  ((F_Related_ovalues t (rsubst'++rsubst) (dsubst0++dsubst) (dsubst'0++dsubst') v v' Env lEnv ->
  ddom_env E [=] dom dsubst ->
  ddom_env E [=] dom dsubst' ->
  ddom_env E' [=] dom dsubst0 ->
...
Lemma Forel_weaken_head:  forall t E v v' rsubst dsubst dsubst' Env lEnv X R t2 t2' K,
  ((F_Related_ovalues t rsubst dsubst dsubst' v v' Env lEnv ->
  ddom_env E [=] dom dsubst ->
  ddom_env E [=] dom dsubst' ->
  ddom_env E [=] dom rsubst ->
...
Lemma Forel_stronger:  forall t E E' v v' rsubst rsubst' dsubst dsubst' dsubst0 dsubst'0 Env lEnv X R t2 t2' K,
  F_Related_ovalues t (rsubst'++[(X,R)]++rsubst) (dsubst0++[(X,t2)]++dsubst) (dsubst'0++[(X,t2')]++dsubst') v v' Env lEnv ->
  ddom_env E [=] dom dsubst ->
  ddom_env E [=] dom dsubst' ->
  ddom_env E' [=] dom dsubst0 ->
...
Proof.
  ...
  Case "F_Related_osubst_kind".
    ...
    split; auto.
       ...
       split; simpl; auto.
         rewrite <- subst_tt_fresh; auto.
         apply Forel_weaken_head with (E:=E) (X:=X) (R:=R) (K:=K)
...
Proof.
  ...
  Case "F_Related_osubst_kind".
    ...
    destruct WFT' as [J1 [J2 [v [v' [[[[Hbinds Hbinds'] Htt] Htt'] Hrel]]]]].
    split; auto.
    split; auto.
       exists (v). exists (v').
       split; simpl; auto.
  ...
  Case "F_Related_osubst_typ".
    ...
    destruct WFT0' as [J1 [J2 [v0 [v'0 [[[[Hbinds1 Hbinds2] Htt] Htt'] Hrel]]]]].
    split. 
      rewrite dom_app.
      destruct (x0 == x); subst; auto.
...
Lemma F_FORel__R__wfor: forall (E:env) (t:typ) (rsubst:rho_subst) dsubst dsubst' Env (R:rel) K,
  F_FORel__R t E rsubst dsubst dsubst' Env R -> 
  wf_typ E t K ->
  wfor Env R (apply_delta_subst_typ dsubst t) (apply_delta_subst_typ dsubst' t) K.
Proof.
  intros E t rsubst dsubst dsubst' Env R K Hr_eq Hwft.
...
Definition P_oparametricity_subst_value (n:nat) := 
  forall t E E' e e' rsubst rsubst' dsubst dsubst' dsubst0 dsubst'0 Env lEnv X R t2 k K Q,
  typ_size t = n ->
...
Proof.
  ...
  intros t E E' e e' rsubst rsubst' dsubst dsubst' dsubst0 dsubst'0 Env lEnv X R t2 k K Q
         Htsize Hwft Hwft' Hwft2 Hr_eq Hfv H1dsubst  H1dsubst'  H1dsubst0  H1dsubst'0 H1rsubst  H1rsubst'.  
  assert (wfor Env R (apply_delta_subst_typ (dsubst0++dsubst) t2) (apply_delta_subst_typ (dsubst'0++dsubst') t2) Q) as wfor. 
  ...
  repeat(split; auto).
         ...
         destruct (@Harrow lEnv1 x x') as [u [u' [Hnorm_vxu [Hnorm_v'x'u' Hrel_wft1]]]]; auto.
           ...
           apply H with (E:=E) (E':=E') (e:=x) (e':=x') (t:=t1) (Env:=Env) (lEnv:=lEnv1)
              ...
              assert (X `notin` fv_te x). 
                eapply m_typing_onormalization_fv with (e:=x); eauto.
                  unfold normalize. 
                  split; auto.
                    apply F_Related_ovalues_inversion in Hrel_wft1'.
         ...
         repeat(split; auto).
           ...
           apply H with (E:=E) (E':=E') (e:=u) (e':=u') (t:=t3) (Env:=Env) (lEnv:=lEnv1++lEnv)
            ...
            apply Hrel_wft2_wft2'; auto.
              SSSSCase "typing".
              apply preservation_normalization with (e:=exp_app e x); auto.
                apply typing_app with (D1:=lEnv) (D2:=lEnv1) (T1:=apply_delta_subst_typ (dsubst0++[(X, apply_delta_subst_typ (dsubst0++dsubst) t2)]++dsubst) (open_tt_rec k X t1)) (K:=k0); auto.
              ...
              SSSSCase "typing".
              apply preservation_normalization with (e:=exp_app e' x'); auto.
                apply typing_app with (D1:=lEnv) (D2:=lEnv1) (T1:=apply_delta_subst_typ (dsubst'0++[(X, apply_delta_subst_typ (dsubst'0++dsubst') t2)]++dsubst') (open_tt_rec k X t1)) (K:=k0); auto.
                  simpl_commut_subst in Typinge'. auto.
       ...
       SCase "all".  
         ...
         apply Hrel_wft_wft'.
             ...
             apply preservation_normalization with (e:=exp_tapp e t3); auto.
               ...
               destruct (X == X4); subst.
                 ...
                 rewrite <- subst_tt_fresh with (T:=t); auto.
                 rewrite open_tt_rec_commute; auto.
                 rewrite commut_delta_osubst_open_tt_rec with (dE:=E'++[(X, bind_kn Q)]++E)(Env:=Env); auto.
                 rewrite delta_osubst_closed_typ with (t:=t3); auto.
                   apply typing_tapp with (T:=t3) (K:=k0); eauto using wfor_left_inv.
                   ...
                   split; intros x0 x0notin.
                     ...
                     clear x0notin Hwft Hwft' Hwft2 Hr_eq wfor Hwfor H1dsubst H1dsubst'  H1dsubst0  H1dsubst'0  H1rsubst  H1rsubst'.
                     solve_uniq.
                     ...
                     clear Hwft Hwft' Hwft2 Hr_eq wfor Hwfor H1dsubst H1dsubst'  H1dsubst0  H1dsubst'0  H1rsubst  H1rsubst'.
                     solve_uniq.
             ...
             apply preservation_normalization with (e:=exp_tapp e' t2'); auto.
               ...
               destruct (X == X4); subst.
                 ...
                 rewrite delta_osubst_closed_typ with (t:=t2'); auto.
                   apply typing_tapp with (T:=t2') (K:=k0); eauto using wfor_right_inv.
         ...
         SSCase "F_FORel_R".
             ...
             assert (X4 `notin` fv_lenv lEnv') as X4notinlEnv'.             
               assert (X4 `notin` dom Env `union` dom lEnv') as J.
                 assert (X4 `notin` dom lEnv').
                   apply ddom_ldoms_disjoint with (E:=[(X4, bind_kn k0)]); simpl; auto.
                 auto.
           ...
           SSSCase "R -> Rrel".
             ...
             split.
               simpl. rewrite <- subst_tt_fresh; auto.
             apply Forel_weaken_head with (E:=E'++E) (X:=X4) (R:=R0) (K:=k0)
  ...
  destruct (k==n0); subst.
    ...
    repeat(split; auto).
      apply wfor_renaming_inv in wfor; auto.
    SCase "bvar".
              ...
              simpl in Hrel_wft1_wft1'. apply Hrel_wft1_wft1' in Hrel_wft1; auto.
               SSSSCase "arrow_left". 
                 ...
                 simpl_env. apply odsubst_weaken; try solve [assumption].
                                           apply wfor_left_inv in wfor; auto.
                                           destruct_notin. auto using free_env__free_dom.
                 simpl_env. apply odsubst_weaken; try solve [assumption].
                                           apply wfor_right_inv in wfor; auto.
                                           destruct_notin. auto using free_env__free_dom.
              SSSCase "sizeeq".  auto.
              SSSCase "fv". 
               assert (X `notin` fv_te x). 
             ...
             apply Hrel_wft2'_wft2; auto.
              ...
              apply preservation_normalization with (e:=exp_app e x); auto.
                apply typing_app with (D1:=lEnv) (D2:=lEnv1) (T1:=apply_delta_subst_typ (dsubst0++[(X, apply_delta_subst_typ (dsubst0++dsubst) t2)]++dsubst) (open_tt_rec k X t1)) (K:=k0); auto.
                  ...
                  rewrite delta_osubst_opt with (dsubst':=dsubst0)(dsubst:=dsubst)(E:=E) (E':=E')(Env:=Env)(K:=Q)(X:=X)(t:=apply_delta_subst_typ (dsubst0++dsubst) t2); try solve [assumption].
                    assert (X `notin` fv_tt t2) as J.
                      destruct_notin.      
                    apply notin_fv_wf with (X:=X) in Hwft2.
                      apply Hwft2.
                      simpl_env. auto using free_env__free_dom. 
                    rewrite swap_subst_tt_odsubst with (E:=E'++E)(Env:=Env)(K:=Q); auto using free_env__free_dom.
                    simpl. 
                    destruct (X==X).
              ...
              apply preservation_normalization with (e:=exp_app e' x'); auto.
                apply typing_app with (D1:=lEnv) (D2:=lEnv1) (T1:=apply_delta_subst_typ (dsubst'0++[(X, apply_delta_subst_typ (dsubst'0++dsubst') t2)]++dsubst') (open_tt_rec k X t1)) (K:=k0); auto.
                  simpl in Typinge'. simpl_commut_subst in Typinge'. 
                  rewrite delta_osubst_opt with (dsubst':=dsubst'0)(dsubst:=dsubst')(E:=E) (E':=E')(Env:=Env)(K:=Q)(X:=X)(t:=apply_delta_subst_typ (dsubst'0++dsubst') t2); try solve [assumption].
                    assert (X `notin` fv_tt t2) as J.
                      destruct_notin.      
                    apply notin_fv_wf with (X:=X) in Hwft2.
                      apply Hwft2.
         ...
         unfold F_FORel__R in *. destruct Hr_eq as [ HHwfd [ HHwfd' [ HHwfor Hrel_rel]]].
           ...
           intro Hrel.
             ...
             assert (X2 `notin` fv_lenv lEnv') as X2notinlEnv'.
               assert (X2 `notin` dom Env `union` dom lEnv') as J.
                 assert (X2 `notin` dom lEnv').
                   apply ddom_ldoms_disjoint with (E:=[(X2, bind_kn k0)]); simpl; auto.
                 auto.
               apply typing_regular in Htypingv0.
               decompose [and] Htypingv0.
               apply wf_lenv_fv_lenv_sub in H2.
             ...
             apply Forel_stronger with (E:=E'++E) (E':=@nil (atom*binding)) (X:=X2) (R:=R0) 
                            ...
                            (dsubst:=dsubst0++dsubst) (dsubst0:=delta_nil)
                            (dsubst':=dsubst'0++dsubst') (dsubst'0:=delta_nil)
                            (t2:=t3) (t2':=t2') (K:=k0); simpl; simpl_env; auto.
                   eapply wfor_left_inv; eauto.
                   eapply wfor_right_inv; eauto.
...
Lemma oparametricity_subst_value : forall t E E' e e' rsubst rsubst' dsubst dsubst' dsubst0 dsubst'0 Env lEnv X R t2 k K Q,
  wf_typ (E'++[(X, bind_kn Q)]++E) (open_tt_rec k (typ_fvar X) t) K ->
  wf_typ (E'++E) (open_tt_rec k t2 t) K ->
  wf_typ (E'++E) t2 Q ->
  F_FORel__R t2 (E'++E) (rsubst'++rsubst) (dsubst0++dsubst) (dsubst'0++dsubst') Env R ->
...
Proof.
  ...
  decompose [prod] (@H t E E' e e' rsubst rsubst' dsubst dsubst' dsubst0 dsubst'0 Env lEnv X R t2 k K Q
...
Proof.
  ...
  Case "typing_var".
    ...
    rewrite m_delta_gamma_osubst_var with (E:=E) (lE:=[]) (x:=x) (gsubst:=gsubst') (t:=apply_delta_subst_typ dsubst' T) (e:=v') (Env:=Env) (lEnv:=[]); auto.
    exists v. exists v'.
      assert (J:=Hrel). apply F_Related_ovalues_inversion in J. destruct J as [JJ1 JJ2].
      repeat (split; auto).
  ...
  Case "typing_abs".
    ...
    split.
      ...
      apply typing_abs with (L:=L `union` dom (gsubst) `union` dom (lgsubst) `union` dom E `union` dom Env `union` dom lEnv); auto.
        ...
        apply H0 in Htyping. 
        apply m_typing_osubst_typ_closed with (E:=E) (dsubst:=dsubst) (gsubst:=gsubst) (lgsubst:=lgsubst) (Env:=Env) (lEnv:=lEnv) in Htyping; auto.

        intros J. apply H2 in J; subst.
        assert (dom lEnv [=] {}) as lEnvNil.
          apply wf_lgamma_osubst_empty_linctx in Hwflg; auto.
        apply empty_dom__empty_ctx in lEnvNil. subst. auto.
    split.
      SCase "typing".
      simpl_commut_subst.
      apply typing_abs with (L:=L `union` dom (gsubst') `union` dom (lgsubst') `union` dom E `union` dom Env `union` dom lEnv); auto.
        ...
        assert (dom lEnv [=] {}) as lEnvNil.
          apply wf_lgamma_osubst_empty_linctx in Hwflg; auto.
        apply empty_dom__empty_ctx in lEnvNil. subst. auto.
    split; try solve [split; try solve [apply delta_gamma_lgamma_osubst_value with (E:=E) (D:=D) (Env:=Env) (lEnv:=lEnv); eauto using FrTyping__absvalue | auto]].
    split; try solve [split; try solve [apply delta_gamma_lgamma_osubst_value with (E:=E) (D:=D) (Env:=Env) (lEnv:=lEnv); eauto using FrTyping__absvalue | auto]].
      SCase "Frel".
      apply F_Related_ovalues_arrow_req.
      split; try solve [apply delta_gamma_lgamma_osubst_value with (E:=E) (D:=D) (Env:=Env) (lEnv:=lEnv); eauto using FrTyping__absvalue].
      split; try solve [apply delta_gamma_lgamma_osubst_value with (E:=E) (D:=D) (Env:=Env) (lEnv:=lEnv); eauto using FrTyping__absvalue].
      SSCase "arrow".
        exists {}.
        ...
        assert (
           ...
           rewrite swap_subst_ee_olgsubst with (E:=E)(D:=D)(dsubst:=dsubst)(lgsubst:=lgsubst)(gsubst:=gsubst)(t:=apply_delta_subst_typ dsubst T1) (Env:=Env) (lEnv:=lEnv) (lEnv':=[]); auto.
             apply wf_lgamma_osubst__nfv with (x:=y) in Hwflg; auto.
         assert (
            apply_delta_subst dsubst' (apply_gamma_subst ([(y,x')]++gsubst') (apply_gamma_subst lgsubst' (open_ee e1 y))) =
            apply_delta_subst dsubst' (apply_gamma_subst gsubst' (apply_gamma_subst lgsubst' (subst_ee y x' (open_ee e1 y))))
                  ) as Heq2.  simpl.
  ...
  Case "typing_labs".
    ...
    split.
      SCase "typing".
      simpl_commut_subst.
      apply typing_labs with (L:=L `union` dom (gsubst) `union` dom (lgsubst) `union` dom E `union` dom Env `union` dom lEnv); auto.
        apply wft_osubst with (E:=E) (dsubst:=dsubst); auto.
  ...
  exists (apply_delta_subst dsubst' (apply_gamma_subst gsubst' (apply_gamma_subst lgsubst' (exp_tabs K e1)))).
      ...
      apply typing_tabs with (L:=L `union` dom (dsubst) `union` dom E `union` dom Env `union` dom lEnv).
        ...
        rewrite <- commut_gamma_osubst_open_te with (E:=E) (D:=D) (dsubst:=dsubst) (gsubst:=gsubst) (lgsubst:=lgsubst) (Env:=Env) (lEnv:=lEnv); auto.
        rewrite <- commut_lgamma_osubst_open_te with (E:=E) (D:=D) (dsubst:=dsubst) (gsubst:=gsubst) (lgsubst:=lgsubst) (Env:=Env) (lEnv:=lEnv); auto.
