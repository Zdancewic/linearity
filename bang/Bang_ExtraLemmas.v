...
Lemma wf_typ_rename : forall G T (x y:atom), 
  uniq G ->
  y `notin` (singleton x `union` fv_tt T `union` dom G)  ->
  x `notin` (fv_tt T `union` dom G) ->
  wf_typ ([(x, bind_kn)] ++ G) (open_tt T x) ->
  wf_typ ([(y, bind_kn)] ++ G) (open_tt T y).
Proof.
  intros. 
  rewrite (subst_tt_intro x).
  rewrite_env (nil ++ [(y, bind_kn)] ++ G).
  ...
  rewrite_env ([(y, bind_kn)] ++ G).
  apply binds_app_l. apply binds_one; auto. solve_notin.
Qed.

Lemma wf_all_exists : forall T (x:atom) G,
  uniq G ->
  x `notin` (fv_tt T `union` dom G) ->
  wf_typ ([(x, bind_kn)] ++ G) (open_tt T x) ->
  wf_typ G (typ_all T).
...
Lemma wf_typ_dec: forall G T,
  uniq G ->
  type T ->
  (wf_typ G T) \/ (~wf_typ G T).
Proof.
  intros G T Uniq Hyp. generalize dependent G.
  induction Hyp; intros G Uniq; auto.

  Case "fvar".
    destruct (binds_dec X (bind_kn) G).
    left. apply wf_typ_var; auto.
    right. unfold not; intros. inversion H. apply n; auto.

  Case "arrow".
    destruct (IHHyp1 G Uniq); 
    destruct (IHHyp2 G Uniq); 
  ...
  Case "all". 
    ...
    assert  (FR: y `notin` L).
    solve_notin.
    destruct (H0 y FR ([(y, bind_kn)] ++ G)). 
      simpl. apply uniq_cons. auto. solve_notin.
      left. apply (wf_all_exists T1 y). auto. solve_notin. auto. 
      right. unfold not. intros. inversion H2. subst. 
      pick fresh z. lapply (H5 z); auto. intros. 
      apply H1. apply (wf_typ_rename G T1 z y). auto. solve_notin. 
        solve_notin. auto.

  Case "bang".
    destruct (IHHyp G Uniq).
      left; auto.
      right. intro J. inversion J; subst; auto.

  Case "with".
    destruct (IHHyp1 G Uniq); destruct (IHHyp2 G Uniq); 
...
Lemma fv_tt_in_wf_typ: forall E T,
  wf_typ E T ->
...
Lemma uniq_from_wf_typ:  forall E T,
  wf_typ E T ->
  uniq E.
Proof.
  intros E T Hyp.
...
Lemma disjoint_dom: forall E1 E2 E3 T,
  wf_typ (E1 ++ E2 ++ E3) T ->
  wf_typ (E1 ++ E3) T ->
  (forall x, x `in` (dom E2) -> x `notin` fv_tt T).
Proof.
  intros E1 E2 E3 T H1 H2.
  intros.
  ...
  assert (x `in` dom (E1 ++ E3)). eapply fv_tt_in_wf_typ with (T:=T); auto.
...
Lemma wf_typ_strengthening_sub: forall F E1 E2 T,
  wf_typ (E2 ++ F ++ E1) T ->
...
Proof.
  ...
  apply IHF. destruct a. destruct b. 
    eapply (wf_typ_strengthening_typ (F ++ E1) E2 a). 
    simpl. rewrite_env (E2 ++ ((a, bind_kn) :: F) ++ E1). apply H.
...
Lemma wf_typ_strengthening_head:  forall E1 E2 T,
  wf_typ (E1 ++ E2) T ->
  wf_typ E1 T ->
  wf_typ E1 T.
Proof.
  intros E1 E2. generalize dependent E1.
  induction E2; intros E1 T H1 H2.
...
Lemma wf_typ_strengthening_sub2: forall E1 E2 E3 T,
  wf_typ (E1 ++ E2 ++ E3) T ->
  wf_typ (E1 ++ E3) T ->
  wf_typ (E1 ++ E3) T.
Proof.
...
Lemma notin_fv_tail: forall F X T E,
...
Proof.
  ...
  unfold not. intros.
  lapply (H1 X). intros. 
  assert (uniq (F ++ [(X, bind_kn)] ++ E)). auto. 
  assert (uniq (E ++ F ++ [(X, bind_kn)])). apply uniq_commute. simpl_env. auto.
  assert (X `notin` dom (F ++ [(X, bind_kn)])).
...
Proof.
  ...
  Case "bang".
    contradict EQ. apply nil_neq_one_mid; auto.
  ...
  Case "let".
    ...
    lapply (lenv_split_cases_app E DM D1R D2R DR); auto.
    intros EX.
    destruct EX as [D1RM [D1RR [D2RM [D2RR [S3 [S4 [EQ1 EQ2]]]]]]]; subst.
    lapply (IHTYP D1L D1RM D1RR); auto.
    intros TYPE1.
    assert (lenv_split E (D1L ++ D1RM ++ [(x, lbind_typ TX)] ++ D1RR)
                         (D2L ++ D2RM ++ D2RR)
                         (DL  ++ DM ++ [(x, lbind_typ TX)] ++ DR)).
      apply lenv_split_join; auto.
        apply lenv_split_weakening_left; subst; auto.
          apply wf_lenv_exchange.
          apply wf_lenv_from_lenv_split_right in H1; auto.
        apply wf_lenv_exchange_mid.
        eapply wf_lenv_split in H1; auto.
    apply typing_let with (L:=L)(D1:=(D1L ++ D1RM ++ [(x, lbind_typ TX)] ++ D1RR))(D2:=(D2L ++ D2RM ++ D2RR))(T1:=T1); auto.
    ...
    assert (lenv_split E (D1L ++ D1RM ++ D1RR)
      ...
      apply lenv_split_join; auto.
        apply lenv_split_weakening_right; subst; auto.
          apply wf_lenv_exchange.
          eapply wf_lenv_from_lenv_split_right in H1; auto.
       apply wf_lenv_exchange_mid.
       eapply wf_lenv_split in H1; auto.
    apply typing_let with (L:=L)(D1:=(D1L ++ D1RM ++ D1RR))(D2:=(D2L ++ D2RM ++ [(x, lbind_typ TX)] ++ D2RR))(T1:=T1); auto.
Qed.
...
Lemma open_kn: forall L E T,
  (forall (Y: atom), Y `notin` L -> wf_typ ([(Y, bind_kn)] ++ E) (open_tt T Y)) ->
  wf_typ E (typ_all T).
Proof.
  intros L E T H.
