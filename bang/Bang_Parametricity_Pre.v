...
Inductive wf_delta_subst : env -> delta_subst -> Prop :=
  ...
  | wf_delta_subst_styp : forall E SE X T,
      wf_delta_subst E SE -> X `notin` dom E -> wf_typ nil T ->
      wf_delta_subst ([(X, bind_kn)] ++ E) ([(X, T)] ++ SE)
  | wf_delta_subst_skip : forall E SE x T,
      wf_delta_subst E SE -> x `notin` dom E -> wf_typ E T ->
...
Inductive wf_gamma_subst : env -> delta_subst -> gamma_subst -> Prop :=
  ...
  | wf_gamma_subst_sval : forall x e T E dsE gsE,
      ...
      typing nil nil e (apply_delta_subst_typ dsE T) -> wf_typ E T ->
      wf_gamma_subst ([(x, bind_typ T)] ++ E) dsE ([(x, e)]++gsE)
  | wf_gamma_subst_skind : forall X T E dsE gsE,
      wf_gamma_subst E dsE gsE -> X `notin` dom E ->
      wf_typ nil T ->
      wf_gamma_subst ([(X, bind_kn)]++E) ([(X, T)]++dsE) gsE
  .
...
Inductive wf_lgamma_subst : env -> lenv -> delta_subst -> gamma_subst -> gamma_subst -> Prop :=
  ...
  | wf_lgamma_subst_skind : forall X T E lE dsE gsE lgsE,
      wf_lgamma_subst E lE dsE gsE lgsE -> 
      X `notin` dom E ->
      X `notin` dom lE ->
      wf_typ nil T ->
      wf_lgamma_subst ([(X, bind_kn)]++E) lE ([(X, T)]++dsE) gsE lgsE
  .
...
Lemma delta_subst_opt :  forall E E' dsubst X t t' dsubst',
...
Proof.
  intros E E' dsubst X t t' dsubst' Hwf_delta_subst Fr Heq He1' Hwft.
...
Lemma swap_subst_te_gsubst: forall t X E D dsubst gsubst lgsubst e,
  wf_lgamma_subst E D dsubst gsubst lgsubst ->
  wf_typ nil t ->
  subst_te X t (apply_gamma_subst gsubst e) =
  apply_gamma_subst gsubst (subst_te X t e).
Proof.
  intros t X E D dsubst gsubst lgsubst e Hwflg Hwft.
  generalize dependent e.
  generalize dependent t.
...
Lemma swap_subst_te_lgsubst: forall t X E D dsubst gsubst lgsubst e,
  wf_lgamma_subst E D dsubst gsubst lgsubst ->
  wf_typ nil t ->
...
Lemma delta_subst_permut : forall dE dsubst X T t,
...
Proof.
  intros dE dsubst X T t Hwfd Fr Hwft.
  generalize dependent t.
...
Lemma commut_subst_tt_dsubst: forall t X E dsubst T,
  wf_delta_subst E dsubst ->
  wf_typ nil t ->
  X `notin` dom E ->
...
Lemma swap_subst_tt_dsubst: forall t X E dsubst T,
  wf_delta_subst E dsubst ->
  wf_typ E t ->
  ...
  apply_delta_subst_typ dsubst (subst_tt X t T).
Proof.
  intros t X E dsubst T Hwfd Hwft Fr1 Fr2.
  generalize dependent t.
  generalize dependent T.
  induction Hwfd; intros; simpl; auto.
    simpl_env in*. destruct_notin.
...
Lemma red_abs_preserved_under_delta_subst: forall dE dsubst T e1 e2,
   wf_delta_subst dE dsubst ->
...
Proof.
  intros dE dsubst T e1 e2 Hwfd Hrc.
  generalize dependent T.
...
Lemma red_tabs_preserved_under_delta_subst: forall dE dsubst e1 t2,
   wf_delta_subst dE dsubst ->
...
Lemma red_abs_preserved_under_gamma_subst: forall E D dsubst gsubst lgsubst T e1 e2,
   wf_lgamma_subst E D dsubst gsubst lgsubst ->
   red (exp_app (exp_abs T e1) e2) (open_ee e1 e2) ->
...
Lemma red_tabs_preserved_under_gamma_subst: forall E D dsubst gsubst lgsubst e1 t2,
   wf_lgamma_subst E D dsubst gsubst lgsubst ->
   red (exp_tapp (exp_tabs e1) t2) (open_te e1 t2) ->
...
Lemma red_abs_preserved_under_lgamma_subst: forall E D dsubst gsubst lgsubst T e1 e2,
   wf_lgamma_subst E D dsubst gsubst lgsubst ->
   red (exp_app (exp_abs T e1) e2) (open_ee e1 e2) ->
...
Lemma red_tabs_preserved_under_lgamma_subst: forall E D dsubst gsubst lgsubst e1 t2,
   wf_lgamma_subst E D dsubst gsubst lgsubst ->
   red (exp_tapp (exp_tabs e1) t2) (open_te e1 t2) ->
...
Lemma red_bang_preserved_under_delta_subst: forall dE dsubst e1 e2,
   wf_delta_subst dE dsubst ->
...
Proof.
  intros dE dsubst e1 e2 Hwfd Hrc.
  generalize dependent e1.
  generalize dependent e2.
  induction Hwfd; intros; simpl; auto.
    rewrite subst_te_open_ee.
...
Lemma red_bang_preserved_under_gamma_subst: forall E D dsubst gsubst lgsubst e1 e2,
   wf_lgamma_subst E D dsubst gsubst lgsubst ->
   red (exp_let (exp_bang e1) e2) (open_ee e2 e1) ->
...
Lemma red_bang_preserved_under_lgamma_subst: forall E D dsubst gsubst lgsubst e1 e2,
   wf_lgamma_subst E D dsubst gsubst lgsubst ->
   red (exp_let (exp_bang e1) e2) (open_ee e2 e1) ->
...
(* ********************************************************************** *)
(* weaken stronger *)
Lemma dsubst_stronger : forall E E' dsubst dsubst' X t,
  wf_delta_subst (E'++[(X,bind_kn)]++E) (dsubst'++[(X,t)]++dsubst) ->
  wf_typ nil t ->
  ddom_env E [=] dom dsubst ->
  ddom_env E' [=] dom dsubst' ->
...
Proof.
  intros E E' dsubst dsubst' X t Hwf_dsubst HT HdomE HdomE' HX.
  remember (E'++[(X,bind_kn)]++E) as G.
...
Lemma dsubst_weaken : forall E E' dsubst dsubst' X t,
  wf_delta_subst (E'++E) (dsubst'++dsubst) ->
  wf_typ nil t ->
  ddom_env E [=] dom dsubst ->
  ddom_env E' [=] dom dsubst' ->
...
Lemma dsubst_weaken_head : forall E dsubst X t,
  wf_delta_subst E dsubst ->
  wf_typ nil t ->
  ddom_env E [=] dom dsubst ->
  X `notin` (dom E)->
  wf_delta_subst ([(X,bind_kn)]++E) ([(X,t)]++dsubst).
Proof.
  intros E dsubst X t Hwf_dsubst Ht HdomE HX.
    apply dsubst_weaken with (t:=t) (E:=E) (E':=(@nil (atom*binding))) (dsubst:=dsubst) (dsubst':=delta_nil) (X:=X); simpl_env; eauto.
Qed.
      ...
      apply one_eq_app in HeqdsG. destruct HeqdsG as [[dsubst'' [dsEQ1 dsEQ2]] | [dsEQ1 dsEQ2]]; subst.
        ...
        apply IHwf_delta_subst with (dsubst':=dsubst''); auto.
          assert (X `notin` fv_tt T).
            apply notin_fv_wf with (E:=(@nil (atom*binding))); auto.
...
intros E D dsubst gsubst lgsubst x t Hwfg Hbinds.
  ...
  induction Hwfg; intros; analyze_binds Hbinds.
    ...
    split; auto.
      simpl. rewrite -> delta_subst_permut with (dE:=E); auto.
        rewrite <-subst_tt_fresh; auto.
          exists (e'). split;auto.
...
Lemma bindsdE__bindsT : forall E dsubst X,
  wf_delta_subst E dsubst ->
  binds X (bind_kn) E ->
  exists T, binds X (T) dsubst /\ wf_typ nil T.
intros.
...
Lemma lgamma_subst_split_nonlin_weakening_typ : forall E G lE dsubst dsubst' gsubst lgsubst1 lgsubst2 lgsubst X t, 
  lgamma_subst_split (E++G) lE (dsubst'++dsubst) gsubst lgsubst1 lgsubst2 lgsubst ->
  X `notin` fv_lenv lE ->
  wf_lgamma_subst (E++[(X, bind_kn)]++G) lE (dsubst'++[(X,t)]++dsubst) gsubst lgsubst ->
  lgamma_subst_split (E++[(X, bind_kn)]++G) lE (dsubst'++[(X,t)]++dsubst) gsubst lgsubst1 lgsubst2 lgsubst.
Proof.
  intros E G lE dsubst dsubst' gsubst lgsubst1 lgsubst2 lgsubst X t H1 H2 H3.
  remember (E++G) as GG.
...
Lemma lgamma_subst_split_nonlin_weakening_typ_tail : forall G lE dsubst gsubst lgsubst1 lgsubst2 lgsubst X t, 
  lgamma_subst_split (G) lE dsubst gsubst lgsubst1 lgsubst2 lgsubst ->
  X `notin` fv_lenv lE ->
  wf_lgamma_subst ([(X, bind_kn)]++G) lE ([(X,t)]++dsubst) gsubst lgsubst ->
  lgamma_subst_split ([(X, bind_kn)]++G) lE ([(X,t)]++dsubst) gsubst lgsubst1 lgsubst2 lgsubst.
Proof.
  intros G lE dsubst gsubst lgsubst1 lgsubst2 lgsubst X t H1 H2 H3.
...
Lemma swap_subst_te_dsubst: forall t X E dsubst e,
  wf_delta_subst E dsubst ->
  wf_typ nil t ->
  X `notin` dom dsubst ->
...
(* ********************************************************************** *)
(* notin *)
Lemma dsubst__preserves__notin_fv_ee : forall E dsubst e x,
  wf_delta_subst E dsubst ->
  x `notin` fv_ee e ->
  x `notin` fv_ee (apply_delta_subst dsubst e).
Proof.
  intros E dsubst e x H.
  induction H; intros; simpl; auto.
    rewrite <- swap_subst_te_dsubst with (E:=E); auto.
      assert (J:=@subst_te_fv_ee_eq (apply_delta_subst SE e) X T).
      apply IHwf_delta_subst in H2. fsetdec.
...
Lemma gsubst__preserves__notin_fv_ee : forall E D dsubst gsubst lgsubst e x,
  wf_lgamma_subst E D dsubst gsubst lgsubst ->
  x `notin` fv_ee e ->
  x `notin` fv_ee (apply_gamma_subst gsubst e).
Proof.
  ...
  induction H; intros; simpl; auto.
    rewrite <- swap_subst_ee_gsubst with (E:=E)(D:=lE)(lgsubst:=lgsE)(dsubst:=dsE)(t:=apply_delta_subst_typ dsE T); auto.
      assert (J:=@subst_ee_fv_ee_sub (apply_gamma_subst gsE e) x0 e0).
      apply IHwf_lgamma_subst in H4.
      apply typing_fv_ee_upper in H2. simpl in H2.
      fsetdec.

      apply wf_lgamma_subst__nfv with (x:=x0) in H; auto.
Qed.

Lemma lgsubst__preserves__notin_fv_ee : forall E D dsubst gsubst lgsubst e x,
  wf_lgamma_subst E D dsubst gsubst lgsubst ->
  x `notin` fv_ee e ->
  x `notin` fv_ee (apply_gamma_subst lgsubst e).
Proof.
  intros E D dsubst gsubst lgsubst e x H.
  induction H; intros; simpl; auto.
    rewrite <- swap_subst_ee_lgsubst with (E:=E)(D:=lE)(gsubst:=gsE)(dsubst:=dsE)(t:=apply_delta_subst_typ dsE T); auto.
      assert (J:=@subst_ee_fv_ee_sub (apply_gamma_subst lgsE e) x0 e0).
      apply IHwf_lgamma_subst in H4.
      apply typing_fv_ee_upper in H2. simpl in H2.
      fsetdec.
...
(* ********************)
(*** closed after subst *)
Lemma wft_subst_closed : forall E E' t dsubst,
  wf_delta_subst E dsubst ->
  wf_typ (E'++E) t ->
  wf_env (E'++E) ->
...
Proof.
  intros E E' t dsubst Hwfd Hwft Hwfe Hwfe'.
  remember (E'++E) as EE.
  generalize dependent E'.
  generalize dependent E.
  generalize dependent dsubst.
...
Proof.
  ...
  Case "typing_lvar".
     ...
     destruct HeqDD as [[D'' [EQ1 EQ2]] | [EQ1 EQ2]]; subst.
       ...
       apply typing_weakening with (F:=apply_delta_subst_env dsubst E') (E:=@nil (atom*binding)) (G:=@nil (atom*binding)) in H3; simpl_env in *; auto.
  Case "typing_abs".
     assert (wf_typ (apply_delta_subst_env dsubst E') (apply_delta_subst_typ dsubst T1)) as Wft.
  ...
  Case "typing_tabs".
     ...
     apply typing_tabs with (L:=L `union` dom gsubst `union` dom lgsubst  
       ...
       intros.
       assert (X `notin` L) as FrxL. auto.
       apply H0 with (D:=D0) (D'0:=D')(E:=E0)(gsubst:=gsubst)(lgsubst:=lgsubst)(dsubst:=dsubst)(E'0:=[(X,bind_kn)]++E') in FrxL; clear H0; auto.
         ...
         simpl. rewrite_env (nil++[(X, bind_kn)]++apply_delta_subst_env dsubst E').
         apply wf_lenv_weakening; auto.
  ...
  Case "typing_bang".
    ...
    apply typing_bang; auto.
      rewrite_env (apply_delta_subst_lenv dsubst lempty).
      apply IHtyping with (D:=lempty)(E:=E0); auto.
  Case "typing_let".
     assert (HH:=H2).
     apply lenv_split_cases_app in H2.
     destruct H2 as [D1L [D1R [D2L [D2R [Hsplit' [Hsplit [J1 J2]]]]]]]; subst.
     assert (J:=H4).
     apply wf_lgamma_subst_split with (lE1:=D1R) (lE2:=D2R) (E':=E') in H4; auto.
     destruct H4 as [lgsubst1 [lgsubst2 [Hgsplit [Hwflg1 Hwflg2]]]].
     assert (wf_lenv (E'++E0) D1L) as Wfe1.
       apply wf_lenv_split_left in Hsplit'; auto.
     assert (wf_lenv (E'++E0) D2L) as Wfe2.
       apply wf_lenv_split_right in Hsplit'; auto.
     assert (exp_let
       ...
       assert (forall x, x `in` dom lgsubst2 -> x `notin` fv_ee e1) as FV1.
         ...
         assert (x `notin` dom D1L `union` dom D2L) as xnotinD12L.
           apply dom_lenv_split in Hsplit'.
           rewrite Hsplit' in xnotinD'. auto.
         apply typing_fv with (x:=x) in H0; auto.
       rewrite gamma_subst_disjoint_exp with (gsubst:=lgsubst2)(e:=e1); auto.
       assert (forall x, x `in` dom lgsubst1 -> x `notin` fv_ee e2) as FV2.
         intros x FV.
         apply wf_lgamma_subst__fv with (x:=x) in Hwflg1; auto.
     ...
     assert (wf_delta_subst E0 dsubst) as Wfd.
       apply wf_lgamma_subst__wf_subst in J.
       destruct J; auto.
     apply typing_let with (L:=L `union` dom gsubst `union` dom lgsubst2 `union` dom (E'++E0) `union` dom (apply_delta_subst_env dsubst E') `union` dom (apply_delta_subst_lenv dsubst D2L))(T1:=apply_delta_subst_typ dsubst T1)
                           (D1:=apply_delta_subst_lenv dsubst D1L)
                           (D2:=apply_delta_subst_lenv dsubst D2L).     
       rewrite <- commut_delta_subst_tbang.
       apply IHtyping with (E:=E0)(D:=D1R)(D':=D1L); auto.
         eapply wfle_subst_closed; eauto.

       intros x xn.
       assert (wf_typ (E'++E0) T1) as WFT1.
         apply typing_regular in H0. destruct H0 as [_ [_ [_ H0]]].
         inversion H0; subst; auto.
       assert (x `notin` L) as FrxL. auto.
       apply H1 with (E:=E0) (gsubst:=gsubst) (dsubst:=dsubst) (E'0:=[(x, bind_typ T1)]++E') (D':=D2L) (D:=D2R) (lgsubst:=lgsubst2) in FrxL; clear H1;auto.
         rewrite commut_lgamma_subst_open_ee with (E:=E0) (dsubst:=dsubst) (D:=D2R) (gsubst:=gsubst) in FrxL; auto.
         rewrite commut_gamma_subst_open_ee with (E:=E0) (dsubst:=dsubst) (D:=D) (lgsubst:=lgsubst) in FrxL; auto.
         rewrite commut_delta_subst_open_ee with (dE:=E0) in FrxL; auto.
         simpl in FrxL. simpl_env in *.
         rewrite apply_gamma_subst_nfv in FrxL; auto.
         rewrite apply_gamma_subst_nfv in FrxL; auto.
...
Lemma wft_subst: forall t dsubst E,
  wf_delta_subst E dsubst ->
  wf_typ E t ->
  wf_typ nil (apply_delta_subst_typ dsubst t).
intros.
