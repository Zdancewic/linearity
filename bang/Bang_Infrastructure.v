(** Infrastructure lemmas and tactic definitions for Bang.
...
Require Export Bang_Definitions.
...
Fixpoint fv_te (e : exp) {struct e} : atoms :=
  ...
  | exp_tabs e1 => (fv_te e1)
  | exp_tapp e1 V => (fv_tt V) `union` (fv_te e1)
  | exp_bang e1 => (fv_te e1)
...
Fixpoint fv_ee (e : exp) {struct e} : atoms :=
  ...
  | exp_tabs e1 => (fv_ee e1)
  | exp_tapp e1 V => (fv_ee e1)
  | exp_bang e1 => (fv_ee e1)
...
Fixpoint subst_tt (Z : atom) (U : typ) (T : typ) {struct T} : typ :=
  ...
  | typ_bang T1 => typ_bang (subst_tt Z U T1)
...
Fixpoint subst_te (Z : atom) (U : typ) (e : exp) {struct e} : exp :=
  ...
  | exp_tabs e1 => exp_tabs (subst_te Z U e1)
  | exp_tapp e1 V => exp_tapp (subst_te Z U e1) (subst_tt Z U V)
  | exp_bang e1 => exp_bang  (subst_te Z U e1)
...
Fixpoint subst_ee (z : atom) (u : exp) (e : exp) {struct e} : exp :=
  ...
  | exp_tabs e1 => exp_tabs (subst_ee z u e1)
  | exp_tapp e1 V => exp_tapp (subst_ee z u e1) V
  | exp_bang e1 => exp_bang  (subst_ee z u e1)
...
Proof with try solve [congruence | intuition].
  ...
  Case "typ_all".
    unfold open_tt in *.
    pick fresh X.
    apply (open_tt_rec_type_aux T1 0 (typ_fvar X))...
Qed.
...
Proof.
  ...
  destruct H as [H | H].
    assert (x `in` fv_ee (open_ee_rec k1 e2 e1_1)).
      apply IHe1_1; auto. 
    fsetdec.
    assert (x `in` fv_ee (open_ee_rec (S k1) e2 e1_2)).
      apply IHe1_2; auto. 
    fsetdec.
