...
Proof.
  ...
  induction H; intros; simpl; try solve [
    ...
    apply H1 with (b:=S b) (k:=k) in Heq.
    unfold open_ee in Heq.
    assert (J := @shift_ee_rec__open_ee_rec e2 x k (S b) 0).
    destruct (le_lt_dec (S b) 0).
       contradict l; omega.
       rewrite J in Heq.
       apply open_ee_rec_inv in Heq; auto.
...
Proof.
  induction e; intros e0 m b k0 He0; simpl; subst;
    ...
    destruct (k0 === n); subst; simpl.
      ...
      assert (J1 := IHe1 e0 m b k0 He0); auto.
      assert (J2 := IHe2 e0 m (S b) (S k0) He0); auto.
      destruct (le_lt_dec b k0).
        destruct (le_lt_dec (S b) (S k0)).
          rewrite J1. rewrite J2. auto.
          contradict l; omega.
...
Proof.
  induction e; intros x kx ey ky xny xney; simpl; 
    try solve [ auto |  rewrite IHe; auto | rewrite IHe1; auto; rewrite IHe2; auto ].
...
Proof.
  ...
  induction H; intros; simpl; try solve [
    ...
    assert (J := @shift_tt_rec__open_tt_rec T1 X k (S b) 0).
    rewrite J in Heq.
...
Proof.
  ...
  induction H; intros; simpl; try solve [
    ...
    apply H1 with (b:=b) (k:=k) in Heq.
    unfold open_ee in Heq.
    assert (J := @shift_te_rec__open_ee_rec e2 x k b 0).
    rewrite J in Heq.
    apply open_ee_rec_inv in Heq; auto.
       rewrite <- Heq; auto.
...
Proof.
  induction e; intros e0 m b k0 He0; simpl; subst; try solve [
    ...
    assert (J1 := IHe1 e0 m b k0 He0).
    assert (J2 := IHe2 e0 m b (S k0) He0).
     rewrite J1. rewrite J2. auto.
Qed.
...
Proof.
  induction e; intros X kX ey ky Xney; simpl; 
    try solve [auto | rewrite IHe; auto | rewrite IHe1; auto; rewrite IHe2; auto].
    destruct (ky==n); subst; simpl; auto.
...
Proof.
  induction e; intros x kx ey ky xnty; simpl;
    try solve [auto | rewrite IHe; auto | rewrite IHe1; auto; rewrite IHe2; auto].
    destruct (x==a); subst; simpl; auto.
Qed.  
...
Proof.
  induction t; intros X kX tY kY XnY XntY; simpl; 
    try solve [
      auto | 
...
Proof.
  induction e; intros X kX ty ky Xny Xnty; simpl; 
    try solve [auto | rewrite IHe; auto | rewrite IHe1; auto; rewrite IHe2; auto].
    rewrite IHe; auto.
...
Proof.
  induction e; intros x kx y ky xny; simpl;
    try solve [auto | rewrite IHe; auto | rewrite IHe1; auto; rewrite IHe2; auto].
    destruct (y==a); subst.
      destruct (x==a); subst.
        contradict xny; auto.
...
Proof.
  induction e; intros k0 x; simpl.
    ...
    assert (J1:=@IHe1 k0 x). 
    assert (J2:=@IHe2 (S k0) x). 
    apply max_injective; omega.
...
Proof.
  induction e; intros kk x i EQ; simpl in *; auto.
    ...
    Case "let".
    destruct (@max_dec (bexp_eupper (open_ee_rec kk x e1)) (bexp_eupper (open_ee_rec(S  kk) x e2) - 1)) as [J | J].
      SCase "left".
      rewrite J in EQ.
      assert ((bexp_eupper (open_ee_rec (S kk) x e2) - 1)<= (bexp_eupper (open_ee_rec kk x e1))) as J'.
        rewrite <- J.
        apply le_max_r.
      remember (bexp_eupper (open_ee_rec (S kk) x e2) - 1) as i'.
      assert (i' <= i) as J''.
        omega.
      apply IHe1 in EQ.
      symmetry in Heqi'.
      assert (bexp_eupper (open_ee_rec (S kk) x e2) = 0 \/ bexp_eupper (open_ee_rec (S kk) x e2) = i' + 1) as EQ2.
        clear - Heqi'. omega.
      destruct EQ2 as [EQ2 | EQ2].
      SSCase "bexp_eupper (open_ee_rec (S kk) x e2) = 0".
      assert (i' = 0) as i'_is_0.
        clear - EQ2 Heqi'. omega.
      subst.
      apply IHe2 in EQ2.
      destruct EQ as [EQ1 | EQ1].
        destruct EQ2 as [EQ2 | EQ2].
        ...
        destruct EQ2 as [EQ2 | EQ2].
          rewrite EQ1. rewrite EQ2.
          assert (S kk + 1 - 1 = kk + 1) as EQ. omega.
          rewrite EQ in *.
          destruct (le_lt_dec i (kk+1)).
             left. apply max_r. auto.
             right. apply max_l. omega.
      ...
      SSCase "bexp_eupper (open_ee_rec (S kk) x e2) = i'+1".
      apply IHe2 in EQ2.
      destruct EQ as [EQ1 | EQ1].
        destruct EQ2 as [EQ2 | EQ2].
          rewrite EQ1. rewrite EQ2.
      ...
      SCase "right".
      rewrite J in EQ.
      assert ((bexp_eupper (open_ee_rec kk x e1))<= (bexp_eupper (open_ee_rec (S kk) x e2) -1)) as J'.
        rewrite <- J.
        apply le_max_l.
      remember (bexp_eupper (open_ee_rec kk x e1)) as i'.
      assert (i' <= i) as J''.
      ...
      destruct EQ2 as [EQ2 | EQ2].
        ...
        destruct Heqi' as [EQ1 | EQ1].
          ...
          destruct (le_lt_dec i (kk+1)).
             left. apply max_l. omega.
             subst i. contradict l; omega.

          rewrite EQ1. rewrite EQ2.
          right. 
            assert (i'=0) as i'_is_0. omega. 
            rewrite i_is_0. rewrite i'_is_0. apply max_l. omega.
      ...
      destruct EQ2 as [EQ2 | EQ2].
        destruct Heqi' as [EQ1 | EQ1].
          ...
          rewrite EQ1. rewrite EQ2.
          assert (S kk + 1 - 1 = kk + 1) as EQk. omega.
          rewrite EQk in *.
          left. apply max_r. 
            assert (J''':=@open_ee_bexp_eupper e2 (S kk) x).
            omega.
        destruct Heqi' as [EQ1 | EQ1].
          rewrite EQ1. rewrite EQ2.
          destruct (le_lt_dec i (kk+1)).
             left. apply max_l. omega.
             right.
               assert (i+1-1=i) as EQi. omega.
               rewrite EQi. apply max_r. omega.

          rewrite EQ1. rewrite EQ2.
          right. 
            assert (i+1-1=i) as EQi. omega.
            rewrite EQi. apply max_r. auto.
...
Proof.
  ...
  induction He; simpl; auto.
   ...
   apply max_l. auto.
    ...
    apply bexp_eupper_open_ee_rec_inv in xnL.
    destruct xnL as [J | J].
      rewrite J. rewrite IHHe. apply max_l. auto.
      rewrite J. rewrite IHHe. apply max_l. auto.
...
Proof.
  induction e; intros x y k0 Hup; simpl; simpl in Hup; auto.
    ...
    rewrite IHe1; auto.
    rewrite IHe2; auto.
    assert (J:=@le_max_r (bexp_eupper e1) ((bexp_eupper e2)-1)).  omega.
    assert (J:=@le_max_l (bexp_eupper e1) ((bexp_eupper e2)-1)).  omega.
...
Proof.
  induction C; intros k0 x; simpl; 
    try solve [
     ...
     apply max_injective; auto |
     assert (J1:=@open_ee_bexp_eupper e (S k0) x);
     assert (J2:=@IHC k0 x);
     apply max_injective; omega |
     assert (J1:=@open_ee_bexp_eupper e k0 x);
     assert (J2:=@IHC (S k0) x);
     apply max_injective; omega
    ].
Qed.
...
Proof.
  induction C; intros kk x i EQ; simpl in *; auto.
    ...
    Case "let1".
    destruct (@max_dec (bctx_eupper (open_ec_rec kk x C)) (bexp_eupper (open_ee_rec(S  kk) x e) - 1)) as [J | J].
      SCase "left".
      rewrite J in EQ.
      assert ((bexp_eupper (open_ee_rec (S kk) x e) - 1)<= (bctx_eupper (open_ec_rec kk x C))) as J'.
        rewrite <- J.
        apply le_max_r.
      remember (bexp_eupper (open_ee_rec (S kk) x e) - 1) as i'.
      assert (i' <= i) as J''.
        omega.
      apply IHC in EQ.
      symmetry in Heqi'.
      assert (bexp_eupper (open_ee_rec (S kk) x e) = 0 \/ bexp_eupper (open_ee_rec (S kk) x e) = i' + 1) as EQ2.
        clear - Heqi'. omega.
      destruct EQ2 as [EQ2 | EQ2].
      SSCase "bexp_eupper (open_ee_rec (S kk) x e) = 0".
      assert (i' = 0) as i'_is_0.
        clear - EQ2 Heqi'. omega.
      subst.
      apply bexp_eupper_open_ee_rec_inv in EQ2.
      destruct EQ as [EQ1 | EQ1].
        destruct EQ2 as [EQ2 | EQ2].
          rewrite EQ1. rewrite EQ2.
          left. apply max_l. omega.
      ...
      SSCase "bexp_eupper (open_ee_rec (S kk) x e) = i'+1".
      apply bexp_eupper_open_ee_rec_inv in EQ2.
      destruct EQ as [EQ1 | EQ1].
        destruct EQ2 as [EQ2 | EQ2].
          rewrite EQ1. rewrite EQ2.
      ...
      SCase "right".
      rewrite J in EQ.
      assert ((bctx_eupper (open_ec_rec kk x C))<= (bexp_eupper (open_ee_rec (S kk) x e) -1)) as J'.
        rewrite <- J.
        apply le_max_l.
      remember (bctx_eupper (open_ec_rec kk x C)) as i'.
      assert (i' <= i) as J''.
      ...
      destruct EQ2 as [EQ2 | EQ2].
        destruct Heqi' as [EQ1 | EQ1].
          ...
          rewrite EQk in *.
          left. apply max_r. 
            assert (J''':=@open_ee_bexp_eupper e (S kk) x).
            omega.
        destruct Heqi' as [EQ1 | EQ1].
          rewrite EQ1. rewrite EQ2.
          destruct (le_lt_dec i (kk+1)).
    ...
    Case "let2".
    destruct (@max_dec (bexp_eupper (open_ee_rec kk x e)) (bctx_eupper (open_ec_rec(S kk) x C) - 1)) as [J | J].
      SCase "left".
      rewrite J in EQ.
      assert ((bctx_eupper (open_ec_rec (S kk) x C) - 1)<= (bexp_eupper (open_ee_rec kk x e))) as J'.
        rewrite <- J.
        apply le_max_r.
      remember (bctx_eupper (open_ec_rec (S kk) x C) - 1) as i'.
      assert (i' <= i) as J''.
        omega.
      apply bexp_eupper_open_ee_rec_inv in EQ.
      symmetry in Heqi'.
      assert (bctx_eupper (open_ec_rec (S kk) x C) = 0 \/ bctx_eupper (open_ec_rec (S kk) x C) = i' + 1) as EQ2.
        clear - Heqi'. omega.
      destruct EQ2 as [EQ2 | EQ2].
      SSCase "bctx_eupper (open_ec_rec (S kk) x C) = 0".
      assert (i' = 0) as i'_is_0.
        clear - EQ2 Heqi'. omega.
      subst.
      apply IHC in EQ2.
      ...
      SSCase "bctx_eupper (open_ec_rec (S kk) x C) = i'+1".
      apply IHC in EQ2.
      destruct EQ as [EQ1 | EQ1].
        destruct EQ2 as [EQ2 | EQ2].
          rewrite EQ1. rewrite EQ2.
      ...
      SCase "right".
      rewrite J in EQ.
      assert ((bexp_eupper (open_ee_rec kk x e))<= (bctx_eupper (open_ec_rec (S kk) x C) -1)) as J'.
        rewrite <- J.
        apply le_max_l.
      remember (bexp_eupper (open_ee_rec kk x e)) as i'.
      assert (i' <= i) as J''.
      ...
      destruct EQ2 as [EQ2 | EQ2].
        destruct Heqi' as [EQ1 | EQ1].
          ...
          rewrite EQk in *.
          left. apply max_r. 
            assert (J''':=@open_ec_bctx_eupper C (S kk) x).
            omega.
        destruct Heqi' as [EQ1 | EQ1].
          rewrite EQ1. rewrite EQ2.
          destruct (le_lt_dec i (kk+1)).
...
Proof.
  induction e; intros kk X i EQ; simpl in *; auto.
    ...
    Case "let".
    destruct (@max_dec (bexp_eupper (open_te_rec kk X e1)) (bexp_eupper (open_te_rec kk X e2) - 1)) as [J | J].
      SCase "left".
      rewrite J in EQ.
      assert ((bexp_eupper (open_te_rec kk X e2) - 1)<= (bexp_eupper (open_te_rec kk X e1))) as J'.
        rewrite <- J.
        apply le_max_r.
      remember (bexp_eupper (open_te_rec kk X e2) - 1) as i'.
      assert (i' <= i) as J''.
        omega.
      apply IHe1 in EQ.
      symmetry in Heqi'.
      assert (bexp_eupper (open_te_rec kk X e2) = 0 \/ bexp_eupper (open_te_rec kk X e2) = i' + 1) as EQ2.
        clear - Heqi'. omega.
      destruct EQ2 as [EQ2 | EQ2].
      SSCase "bexp_eupper (open_te_rec kk X e2) = 0".
      assert (i' = 0) as i'_is_0.
        clear - EQ2 Heqi'. omega.
      subst.
      apply IHe2 in EQ2.
      ...
      SSCase "bexp_eupper (open_te_rec kk X e2) = i'+1".
      apply IHe2 in EQ2.
      ...
      SCase "right".
      rewrite J in EQ.
      assert ((bexp_eupper (open_te_rec kk X e1))<= (bexp_eupper (open_te_rec kk X e2) -1)) as J'.
        rewrite <- J.
        apply le_max_l.
      remember (bexp_eupper (open_te_rec kk X e1)) as i'.
      assert (i' <= i) as J''.
      ...
      apply IHe2 in EQ2.
      apply IHe1 in Heqi'.
      rewrite Heqi'. rewrite EQ2.
      subst. rewrite i_is_0 in *. 
      assert (0-1=0) as EQ. omega. rewrite EQ.
      apply max_r. omega.
      ...
      apply IHe1 in Heqi'.
      rewrite Heqi'. rewrite EQ2.
      assert (i+1-1=i) as EQi. omega. rewrite EQi.
      apply max_r. omega.
...
Proof.
  induction C; intros kk X i EQ; simpl in *; eauto.
    ...
    Case "let1".
    destruct (@max_dec (bctx_eupper (open_tc_rec kk X C)) (bexp_eupper (open_te_rec kk X e) - 1)) as [J | J].
      SCase "left".
      rewrite J in EQ.
      assert ((bexp_eupper (open_te_rec kk X e) - 1)<= (bctx_eupper (open_tc_rec kk X C))) as J'.
        rewrite <- J.
        apply le_max_r.
      remember (bexp_eupper (open_te_rec kk X e) - 1) as i'.
      assert (i' <= i) as J''.
        omega.
      apply IHC in EQ.
      symmetry in Heqi'.
      assert (bexp_eupper (open_te_rec kk X e) = 0 \/ bexp_eupper (open_te_rec kk X e) = i' + 1) as EQ2.
        clear - Heqi'. omega.
      destruct EQ2 as [EQ2 | EQ2].
      SSCase "bexp_eupper (open_te_rec kk X e) = 0".
      assert (i' = 0) as i'_is_0.
        clear - EQ2 Heqi'. omega.
      subst.
      apply bexp_eupper_open_te_rec_inv in EQ2.
      rewrite EQ2.
      apply max_l. omega.

      SSCase "bexp_eupper (open_te_rec kk X e) = i'+1".
      apply bexp_eupper_open_te_rec_inv in EQ2.
      rewrite EQ. rewrite EQ2.
      assert (i'+1-1=i') as EQi. omega.
      rewrite EQi. apply max_l. omega.

      SCase "right".
      rewrite J in EQ.
      assert ((bctx_eupper (open_tc_rec kk X C))<= (bexp_eupper (open_te_rec kk X e) -1)) as J'.
        rewrite <- J.
        apply le_max_l.
      remember (bctx_eupper (open_tc_rec kk X C)) as i'.
      assert (i' <= i) as J''.
      ...
      apply IHC in Heqi'.
      rewrite Heqi'. rewrite EQ2.
      assert (0-1=0) as EQ0. omega. rewrite EQ0. rewrite i_is_0 in *.
      apply max_r. omega.
    ...
    Case "let2".
    destruct (@max_dec (bexp_eupper (open_te_rec kk X e)) (bctx_eupper (open_tc_rec kk X C) - 1)) as [J | J].
      SCase "left".
      rewrite J in EQ.
      assert ((bctx_eupper (open_tc_rec kk X C) - 1)<= (bexp_eupper (open_te_rec kk X e))) as J'.
        rewrite <- J.
        apply le_max_r.
      remember (bctx_eupper (open_tc_rec kk X C) - 1) as i'.
      assert (i' <= i) as J''.
        omega.
      apply bexp_eupper_open_te_rec_inv in EQ.
      symmetry in Heqi'.
      assert (bctx_eupper (open_tc_rec kk X C) = 0 \/ bctx_eupper (open_tc_rec kk X C) = i' + 1) as EQ2.
        clear - Heqi'. omega.
      destruct EQ2 as [EQ2 | EQ2].
      SSCase "bctx_eupper (open_tc_rec kk X C) = 0".
      assert (i' = 0) as i'_is_0.
        clear - EQ2 Heqi'. omega.
      subst.
      apply IHC in EQ2.
      ...
      SSCase "bctx_eupper (open_tc_rec kk X C) = i'+1".
      apply IHC in EQ2.
      rewrite EQ. rewrite EQ2.
      assert (i'+1-1=i') as EQi. omega.
      rewrite EQi. apply max_l. omega.
      ...
      SCase "right".
      rewrite J in EQ.
      assert ((bexp_eupper (open_te_rec kk X e))<= (bctx_eupper (open_tc_rec kk X C) -1)) as J'.
        rewrite <- J.
        apply le_max_l.
      remember (bexp_eupper (open_te_rec kk X e)) as i'.
      assert (i' <= i) as J''.
...
Proof.
  ...
  induction HC; simpl; auto.
   ...
   rewrite IHHC. rewrite expr_bexp_eupper; auto.
    ...
    apply H in xnL; auto.
    rewrite IHHC.
    apply expr_bexp_eupper in xnL.
    apply bexp_eupper_open_ee_rec_inv in xnL.
    destruct xnL as [J | J].
      rewrite J. assert (0+1-1=0) as EQ. omega. rewrite EQ. apply max_l; auto.
      rewrite J. assert (0-1=0) as EQ. omega. rewrite EQ. apply max_l; auto.

    apply expr_bexp_eupper in H.
    rewrite H.
    pick fresh x.
    assert (x `notin` L) as xnL. auto.
    apply H1 in xnL; auto.
...
Proof.
  induction e; intros x kx ey y xney xny; simpl; 
    try solve [auto | rewrite IHe; auto | rewrite IHe1; auto; rewrite IHe2; auto ].
...
Proof.
  induction e; intros x kx tY Y xntY xnY; simpl;
    try solve [auto | rewrite IHe; auto | rewrite IHe1; auto; rewrite IHe2; auto ].
    destruct (x==a); subst; simpl; auto.
Qed.  
...
Proof.
  induction t; intros X kX tY Y XntY XnY; simpl; 
    try solve [auto | rewrite IHt; auto | rewrite IHt1; auto; rewrite IHt2; auto ].
    destruct (a==Y); subst; simpl; auto.
      destruct (X==Y); subst; simpl; auto.
         contradict XnY; auto.
         destruct (Y==Y); subst; simpl; auto.
...
Proof.
  induction C; intros x y k0 Hup; simpl; simpl in Hup; 
    try solve [
      auto |
      rewrite IHC; try solve [auto | omega] |
     rewrite IHC; auto
    ].
    ...
    rewrite IHC; auto.
    rewrite close_open_ee_rec__subst_ee; auto.
    assert (J:=@le_max_r (bctx_eupper C) (bexp_eupper e-1)).  omega.
    assert (J:=@le_max_l (bctx_eupper C) (bexp_eupper e-1)).  omega.

    rewrite IHC; auto.
    rewrite close_open_ee_rec__subst_ee; auto.
    assert (J:=@le_max_l (bexp_eupper e) (bctx_eupper C-1)).  omega.
    assert (J:=@le_max_r (bexp_eupper e) (bctx_eupper C-1)).  omega.
...
Proof.
  ...
  induction HC; intro; simpl; auto.
    ...
    assert (x `notin` L) as xnL. auto.
    apply H in xnL.
    unfold open_ee in xnL.
    apply open_ee_rec_expr with (k:= S k)(u:=u) in xnL.
    rewrite <- open_ee_rec_expr_aux with (j:=0)(v:=x); auto.
...
Proof.
  ...
  induction HC; intro; simpl; auto.
    ...
    apply H in xnL.
    unfold open_te in xnL.
    apply open_te_rec_expr with (k:= k)(U:=U) in xnL.
    rewrite <- open_te_rec_expr_aux with (j:=0)(u:=x); auto.
...
Proof with auto.
  ...
  induction HC; simpl; auto.
    ...
    apply context_tabs_capture with (L:=L `union` singleton z); auto.
      intros X XnL.
      rewrite subst_ec_open_tc_var; auto.
    apply context_let1 with (L:=L `union` singleton z); auto.
      intros x xnL.
      rewrite subst_ee_open_ee_var; auto.
    apply context_let2_free with (L:=L `union` singleton z); auto.
      intros x xnL.
      rewrite subst_ec_open_ec_var; auto.
    apply context_let2_capture with (L:=L `union` singleton z); auto.
      intros x xnL.
      rewrite subst_ec_open_ec_var; auto.
Qed.
...
Proof.
  induction e; intros kk x i EQ; simpl in *; auto.
    ...
    destruct (@max_dec (bexp_tupper (open_ee_rec kk x e1)) (bexp_tupper (open_ee_rec (S kk) x e2))) as [J | J].
      rewrite J in EQ.
      assert ((bexp_tupper (open_ee_rec (S kk) x e2))<= (bexp_tupper (open_ee_rec kk x e1))) as J'.
        rewrite <- J.
        apply le_max_r.
      remember (bexp_tupper (open_ee_rec (S kk) x e2)) as i'.
      assert (i' <= i) as J''.
        omega.
      apply IHe1 in EQ.
      symmetry in Heqi'.
      ...
      rewrite J in EQ.
      assert ((bexp_tupper (open_ee_rec kk x e1))<= (bexp_tupper (open_ee_rec (S kk) x e2))) as J'.
        rewrite <- J.
        apply le_max_l.
      remember (bexp_tupper (open_ee_rec kk x e1)) as i'.
      assert (i' <= i) as J''.
...
Proof.
  induction C; intros kk x i EQ; simpl in *; eauto.
    ...
    destruct (@max_dec (bctx_tupper (open_ec_rec kk x C)) (bexp_tupper (open_ee_rec (S kk) x e))) as [J | J].
      rewrite J in EQ.
      assert ((bexp_tupper (open_ee_rec (S kk) x e)) <= (bctx_tupper (open_ec_rec kk x C))) as J'.
        rewrite <- J.
        apply le_max_r.
      remember (bexp_tupper (open_ee_rec (S kk) x e)) as i'.
      assert (i' <= i) as J''.
      ...
      rewrite J in EQ.
      assert ((bctx_tupper (open_ec_rec kk x C)) <= (bexp_tupper (open_ee_rec (S kk) x e))) as J'.
        rewrite <- J.
        apply le_max_l.
      remember (bctx_tupper (open_ec_rec kk x C)) as i'.
      assert (i' <= i) as J''.
    ...
    destruct (@max_dec (bexp_tupper (open_ee_rec kk x e)) (bctx_tupper (open_ec_rec (S kk) x C))) as [J | J].
      rewrite J in EQ.
      assert ((bctx_tupper (open_ec_rec (S kk) x C)) <= (bexp_tupper (open_ee_rec kk x e))) as J'.
        rewrite <- J.
        apply le_max_r.
      remember (bctx_tupper (open_ec_rec (S kk) x C)) as i'.
      assert (i' <= i) as J''.
      ...
      rewrite J in EQ.
      assert ((bexp_tupper (open_ee_rec kk x e)) <= (bctx_tupper (open_ec_rec (S kk) x C))) as J'.
        rewrite <- J.
        apply le_max_l.
      remember (bexp_tupper (open_ee_rec kk x e)) as i'.
      assert (i' <= i) as J''.
...
Proof.
  induction C1; intros e2 kk; simpl; try solve [eauto | fsetdec].
    ...
    assert (J:=@open_ee_rec_fv_ee_upper e e2 (S kk)).
    assert (J':=@IHC1 e2 kk).
    fsetdec.

    assert (J:=@open_ee_rec_fv_ee_upper e e2 kk).
    assert (J':=@IHC1 e2 (S kk)).
    fsetdec.
...
Proof.
  induction C1; intros e2 kk; simpl; try solve [eauto | fsetdec].
    ...
    assert (J:=@open_ee_rec_fv_ee_lower e e2 (S kk)).
    assert (J':=@IHC1 e2 kk).
    fsetdec.
...
Proof.
  induction C1; intros x kk; simpl; try solve [eauto | fsetdec].
    ...
    assert (J:=@close_ee_rec_fv_ee_upper e x (S kk)).
    assert (J':=@IHC1 x kk).
    fsetdec.

    assert (J:=@close_ee_rec_fv_ee_upper e x kk).
    assert (J':=@IHC1 x (S kk)).
    fsetdec.
...
Proof.
  induction e; intros X; simpl; 
    try solve [
      auto |
      rewrite IHe; rewrite subst_tt_id; auto |
      rewrite IHe1; rewrite IHe2; auto |
      rewrite IHe; auto
   ].
Qed.
...
Proof.
  induction C; intros X; simpl; 
    try solve [
      auto |
      rewrite IHC; rewrite subst_tt_id; auto |
      rewrite IHC; rewrite subst_te_id; auto |
      rewrite IHC; auto
    ].
Qed.
...
Proof.
  induction e; intros kx ky x y; intros; simpl; 
    try solve [
      auto |
      rewrite IHe; auto |
...
Proof.
  induction t; intros kx ky x y; intros; simpl; 
    try solve [
      auto |
      rewrite IHt1; auto; rewrite IHt2; auto |
      rewrite IHt; auto
...
Proof.
  ...
  induction He; intros; simpl; auto.
    ...
    apply expr_let with (L:=L `union` {{y}}); auto.
      intros x0 x0nL.
      assert (x0 `notin` L) as FV. auto.
...
Proof.
  induction C; intros e0 m b k0 He0; simpl; subst;
    destruct (le_lt_dec b k0); try solve[
      ...
      assert (J := IHC e0 m b k0 He0); auto.
      assert (J' := aux_shift_ee_rec__open_ee_rec e e0 m (S b) (S k0) He0); auto.
      destruct (le_lt_dec b k0).
        destruct (le_lt_dec (S b) (S k0)).
      ...
      assert (J := IHC e0 m (S b) (S k0) He0); auto.
      assert (J' := aux_shift_ee_rec__open_ee_rec e e0 m b k0 He0); auto.
      destruct (le_lt_dec b k0).
        destruct (le_lt_dec (S b) (S k0)).
          rewrite J. rewrite J'. auto.
          contradict l0; omega.
...
Proof.
  induction C; intros e0 m b k0 He0; simpl; subst; try solve [
    ...
    destruct (le_lt_dec b k0); try solve [
      ...
      assert (J := IHC e0 m b k0 He0); auto.
      assert (J' := aux_shift_te_rec__open_ee_rec e e0 m b (S k0) He0); auto.
       rewrite J. rewrite J'. auto.

      assert (J := IHC e0 m b (S k0) He0); auto.
      assert (J' := aux_shift_te_rec__open_ee_rec e e0 m b k0 He0); auto.
       rewrite J. rewrite J'. auto.
...
Proof.
  induction C; intros x m b k0; simpl; subst;
    destruct (le_lt_dec b k0); try solve[
      ...
      assert (J' := aux_shift_ee_rec__open_ee_rec e x m (S b) (S k0)); auto.
      destruct (le_lt_dec b k0).
        destruct (le_lt_dec (S b) (S k0)).
          rewrite J. rewrite J'; auto.
          contradict l0; omega.
      ...
      assert (J' := aux_shift_ee_rec__open_ee_rec e x m b k0); auto.
      destruct (le_lt_dec b k0).
        destruct (le_lt_dec (S b) (S k0)).
          rewrite J. rewrite J'; auto.
          contradict l0; omega.
...
Proof.
  induction C; intros x i b H; simpl in *; auto.
    ...
    destruct_notin.
    apply open_shift_ee__notin_fv with (n:=i) (b:=S b) in NotInTac; auto.
...
Proof.
  induction C; intros x i b H; simpl in *; auto.
    ...
    destruct_notin.
    apply open_te_shift_ee__notin_fv with (n:=i) (b:=S b) in NotInTac; auto.
...
Proof.
  ...
  induction H; intros; simpl; try solve [
    ...
    apply H0 in Heq.
    unfold open_ee in Heq.
    apply shift_ee_rec_expr with (b:=S b) (k:=k) in Heq.
    assert (J := @shift_ee_rec__open_ee_rec e2 x k (S b) 0).
    destruct (le_lt_dec (S b) 0).
       contradict l; omega.
    ...
    apply H1 with (b:=S b) (k:=k) in Heq.
    unfold open_ec in Heq.
    assert (J := @shift_ec_rec__open_ec_rec C2 x k (S b) 0).
    destruct (le_lt_dec (S b) 0).
       contradict l; omega.
       ...
       apply open_ec_rec_inv in Heq; auto.
         rewrite <- Heq. 
         apply shift_ee_rec_expr with (b:=b)(k:=k) in H.
         rewrite <- H; auto.
...
Proof.
  induction C1; intros T2 kk; simpl; 
    try solve [eauto | fsetdec |
...
Proof.
  induction C; intros x kk; simpl; try solve [eauto | fsetdec].
    ...
    assert (J:=@close_ee_rec_fv_ee_lower e x (S kk)).
    assert (J':=@IHC x kk).
    fsetdec.
...
Proof.
  induction e; intros x m b k0; simpl; subst; try solve [
    ...
    assert (J1 := IHe1 x m b k0); auto.
    assert (J2 := IHe2 x m (S b) (S k0)); auto.
    destruct (le_lt_dec b k0).
      destruct (le_lt_dec (S b) (S k0)).
         rewrite J1. rewrite J2. auto.
         contradict l. omega. 
...
Proof.
  induction e; intros x kx y ky; simpl; 
    try solve [
      auto |
      rewrite IHe; auto |
      rewrite IHe1; auto; rewrite IHe2; auto].
...
Proof.
  induction C; intros x m b k0; simpl; subst; try solve [
    ...
    assert (J1 := IHC x m b k0); auto.
    assert (J2 := @aux_shift_ee_rec__close_ee_rec e x m (S b) (S k0)); auto.
    destruct (le_lt_dec b k0).      
      destruct (le_lt_dec (S b) (S k0)).
         rewrite J1. rewrite J2; auto.
         contradict l. omega. 
    ...
    assert (J1 := IHC x m (S b) (S k0)); auto.
    assert (J2 := @aux_shift_ee_rec__close_ee_rec e x m b k0); auto.
    destruct (le_lt_dec b k0).      
      destruct (le_lt_dec (S b) (S k0)).
         rewrite J1. rewrite J2; auto.
...
Proof.
  ...
  induction H; intros; simpl; try solve [
    ...
    apply H0 in Heq.
    unfold open_ee in Heq.
    apply shift_te_rec_expr with (b:=b)(k:=k) in Heq.
    assert (J := @shift_te_rec__open_ee_rec e2 x k b 0).
    rewrite J in Heq.
    apply open_ee_rec_inv in Heq; auto.
       rewrite <- Heq; auto.
    ...
    apply H1 with (b:=b)(k:=k) in Heq.
    unfold open_ec in Heq.
    assert (J := @shift_tc_rec__open_ec_rec C2 x k b 0).
    rewrite J in Heq.
    apply open_ec_rec_inv in Heq; auto.
      rewrite <- Heq.
      rewrite <- shift_te_rec_expr; auto.
...
Proof with auto*.
  intros X C k0 Hfv. revert k0.
  induction C; intro k0; simpl in *; f_equal; 
    auto using close_tt_rec_typ, close_te_rec_expr.
Qed.
...
Proof.
  induction C; intros X kX ty ky Xny Xnty; simpl; 
    try solve [auto |
      rewrite IHC; auto |
      rewrite IHC; auto ; rewrite close_open_tt_rec_commut; auto |
      rewrite IHC; auto ; rewrite close_open_te_rec_commut; auto].
Qed.  
...
Proof with auto.
  ...
  Case "exp_let".
    assert (J1 := IHe1 k1 b1 k2 b2).
    assert (J2 := IHe2 k1 (S b1) k2 (S b2)).
    destruct (le_lt_dec b1 b2); subst; simpl.
      destruct (le_lt_dec (S b1) (S b2)); subst; simpl.
        rewrite J1. rewrite J2. auto.
      ...
      destruct (le_lt_dec (S b1) (S b2)); subst; simpl.
        ...
        rewrite J1. rewrite J2. rewrite max_SS.
        destruct (le_lt_dec (S b1) k2).
          assert (S b1 - k2 = 0) as EQ. omega.
          assert (S (b1 - k2) = 1) as EQ'. omega.
          rewrite EQ. rewrite EQ'. clear EQ EQ'.
...
Proof.
  induction e; intros k1 b1 k2 b2; simpl; 
    try solve [auto | rewrite IHe; auto | rewrite IHe1; auto; rewrite IHe2; auto].
...
Proof.
  induction C1; intros x2 kk; simpl; try solve [eauto | fsetdec].
    ...
    assert (J:=@open_ee_rec_fv_te_eq e x2 (S kk)).
    assert (J':=@IHC1 x2 kk).
    fsetdec.
...
Proof.
  induction C; intros kk x; simpl; 
    try solve [
     auto |
     rewrite <- IHC; fsetdec |
     rewrite <- IHC; rewrite <- fv_te_close_ee_rec; fsetdec
   ].
Qed.
...
Proof.
  induction C1; intros X kk; simpl; 
    try solve [eauto | fsetdec |
...
Proof.
  induction e; intros x k0 Hnotin; simpl; simpl in Hnotin;
    try solve [auto | rewrite IHe; auto | rewrite IHe1; auto; rewrite IHe2; auto].
    destruct (k0==n); subst; simpl; auto.
      destruct (x==x); auto.
        contradict n0; auto.
...
Proof.
  induction t; intros X k0 Hnotin; simpl; simpl in Hnotin; 
    try solve [auto | rewrite IHt; auto | rewrite IHt1; auto; rewrite IHt2; auto].
    destruct (k0==n); subst; simpl; auto.
      destruct (X==X); auto.
        contradict n0; auto.
...
Lemma subst_tt_tt' : forall T (X Y:atom) t Env,
  wf_typ Env t ->
  Y `notin` fv_tt T `union` dom Env ->
  subst_tt Y t (subst_tt X Y T) = subst_tt X t T.
Proof.
  induction T; intros X Y t Env Wft Fv; simpl; auto.
    ...
    simpl in Fv.
    rewrite IHT1 with (Env:=Env); auto.
    rewrite IHT2 with (Env:=Env); auto.

    simpl in Fv.
    rewrite IHT with (Env:=Env); auto.
...
Proof.
  induction C1; intros x kk; simpl; try solve [eauto | fsetdec].
    ...
    assert (J:=@close_ee_rec_fv_ee_eq' e x (S kk)).
    assert (J':=@IHC1 x kk).
    fsetdec.
...
Lemma open_ee_var_plug : forall C x e,
  expr e -> 
  x `notin` (cv_ec C) ->
  open_ee (plug C e) x  =  plug (open_ec C x) e.
Proof.
  intros C x e He Hfv.
...
Lemma open_te_var_plug : forall C X e,
  expr e -> 
  X `notin` (cv_ec C) ->
  open_te (plug C e) X  =  plug (open_tc C X) e.
Proof.
  intros C X e He Hfv.
...
Lemma shift_ee_rec_fv_ee_eq : forall e1 b kk,
  fv_ee (shift_ee_rec b kk e1) [=] (fv_ee e1).
Proof.
  induction e1; intros b kk; simpl; try solve [eauto | fsetdec].
     destruct (le_lt_dec kk n); subst; simpl; fsetdec.
Qed.

Lemma shift_ee_fv_ee_eq : forall e1,
  fv_ee (shift_ee e1) [=] (fv_ee e1).
Proof.
  intros. apply shift_ee_rec_fv_ee_eq.
Qed.

Lemma shift_te_rec_fv_ee_eq : forall e1 b kk,
  fv_ee (shift_te_rec b kk e1) [=] (fv_ee e1).
Proof.
  induction e1; intros b kk; simpl; try solve [eauto | fsetdec].
Qed.

Lemma shift_te_fv_ee_eq : forall e1,
  fv_ee (shift_te e1) [=] (fv_ee e1).
Proof.
  intros. apply shift_te_rec_fv_ee_eq.
Qed.

Lemma fv_ee_plug : forall C e,
  fv_ee (plug C e) [<=] fv_ec C `union` fv_ee e.
Proof.
  intros C.
  induction C; intros; simpl; 
    try solve [auto |
                         fsetdec |
                         assert (J:=@IHC e0); fsetdec].

    assert (J:=@IHC (shift_ee e)).
    rewrite shift_ee_fv_ee_eq in J.
    fsetdec.

    assert (J:=@IHC (close_ee (shift_ee e) a)).
    rewrite close_ee_fv_ee_eq in J.
    rewrite shift_ee_fv_ee_eq in J.
    fsetdec.
    
    assert (J:=@IHC (shift_te e)).
    rewrite shift_te_fv_ee_eq in J.
    fsetdec.

    assert (J:=@IHC (close_te (shift_te e) a)).
    rewrite <- close_te_fv_ee_eq in J.
    rewrite shift_te_fv_ee_eq in J.
    fsetdec.

    assert (J:=@IHC (shift_ee e0)).
    rewrite shift_ee_fv_ee_eq in J.
    fsetdec.

    assert (J:=@IHC (close_ee (shift_ee e0) a)).
    rewrite close_ee_fv_ee_eq in J.
    rewrite shift_ee_fv_ee_eq in J.
    fsetdec.
Qed.  

Lemma close_ee_plug : forall C x e,
  x `notin`  (cv_ec C) ->
  close_ee (plug C e) x  =  plug (close_ec C x) (close_ee e x).
Proof.
  intros C x e Hfv.
  unfold close_ee. unfold close_ec.
  rewrite <- close_ee_rec_plug; auto.
Qed.
 
Definition P_open_ee_rec_swap (n:nat) := forall e (x y:atom) k,
  exp_size e = n ->
  expr (open_ee_rec k x e) ->
  expr (open_ee_rec k y e).

Lemma open_te_rec_exp_size_eq : forall (X : atom) e k,
  exp_size (open_te_rec k X e) = exp_size e.
Proof.
  (exp_cases (induction e) Case); intros k0; simpl; auto.
Qed.
...
Lemma _open_ee_rec_swap : forall n, P_open_ee_rec_swap n.
Proof.
  intro n.
  apply lt_wf_ind. clear n.
  intros n H.
  unfold P_open_ee_rec_swap in *.
  intros e x y k H1 H2; (exp_cases (destruct e) Case); subst; simpl in *; auto.
  Case "exp_bvar".
    destruct (k==n0); auto.
  Case "exp_abs".
    inversion H2; subst.
    apply expr_abs with (L:=L); auto.
      intros x0 x0n.
      apply H4 in x0n; auto.
      unfold open_ee in *.
      rewrite open_ee_open_ee_rec__commut; auto.
      rewrite open_ee_open_ee_rec__commut in x0n; auto.
      apply H with (x:=x)(m:=exp_size (open_ee_rec 0 x0 e)); auto.
        rewrite open_ee_rec_exp_size_eq. auto.
  Case "exp_app".
    inversion H2; subst.
    apply expr_app.
      apply H with (x:=x)(m:=exp_size e1); simpl; try solve [auto|omega].
      apply H with (x:=x)(m:=exp_size e2); simpl; try solve [auto|omega].
  Case "exp_tabs".
    inversion H2; subst.
    apply expr_tabs with (L:=L); auto.
      ...
      unfold open_te in *.
      rewrite <- open_ee_open_te_rec__commut; auto.
      rewrite <- open_ee_open_te_rec__commut in Xn; auto.
      apply H with (x:=x)(m:=exp_size (open_te_rec 0 X e)); auto.
        rewrite open_te_rec_exp_size_eq. auto.
  Case "exp_tapp".
    inversion H2; subst.
    apply expr_tapp; auto.
      apply H with (x:=x)(m:=exp_size e); simpl; try solve [auto|omega].
  Case "exp_bang".
    inversion H2; subst.
    apply expr_bang; auto.
      apply H with (x:=x)(m:=exp_size e); simpl; try solve [auto|omega].
  Case "exp_let".
    inversion H2; subst.
    apply expr_let with (L:=L); auto.
      ...
      rewrite open_ee_open_ee_rec__commut; auto.
      rewrite open_ee_open_ee_rec__commut in x0n; auto.
      apply H with (x:=x)(m:=exp_size (open_ee_rec 0 x0 e2)); auto.
        rewrite open_ee_rec_exp_size_eq. simpl. omega.
  Case "exp_apair".
    inversion H2; subst.
    apply expr_apair.
      apply H with (x:=x)(m:=exp_size e1); simpl; try solve [auto|omega].
...
Lemma open_ee_rec_swap : forall e (x y:atom) k,
  expr (open_ee_rec k x e) ->
  expr (open_ee_rec k y e).
Proof.
  intros e x y k H1.
  assert (P_open_ee_rec_swap (exp_size e)) as J.
    apply (@_open_ee_rec_swap (exp_size e)).
  unfold P_open_ee_rec_swap in J.
  apply J with (x:=x); auto.
Qed.

Lemma open_ec_rec_ctx_size_eq : forall (x : atom) C k,
  ctx_size (open_ec_rec k (exp_fvar x) C) = ctx_size C.
Proof.
  induction C; intros k0; simpl; auto.
Qed.

Lemma open_ec_ctx_size_eq : forall (x : atom) C,
  ctx_size (open_ec C x) = ctx_size C.
Proof.
  unfold open_ec.
  auto using open_ec_rec_ctx_size_eq.
Qed.

Lemma open_ec_open_ec_rec__commut : forall C kx ky (x y:atom),
  kx <> ky ->
  (open_ec_rec kx x (open_ec_rec ky y C)) = 
    (open_ec_rec ky y (open_ec_rec kx x C)).
Proof.
  induction C; intros kx ky x y; intros; simpl; 
    try solve [
      auto | 
      rewrite IHC; auto | 
      rewrite IHC1; auto; rewrite IHC2; auto |
      rewrite IHC; auto; rewrite open_ee_open_ee_rec__commut; auto
    ].
Qed.

Lemma open_tc_rec_ctx_size_eq : forall (X : atom) C k,
  ctx_size (open_tc_rec k X C) = ctx_size C.
Proof.
  induction C; intros k0; simpl; auto.
Qed.

Lemma open_tc_ctx_size_eq : forall (X : atom) C,
  ctx_size (open_tc C X) = ctx_size C.
Proof.
  unfold open_tc.
  auto using open_tc_rec_ctx_size_eq.
Qed.

Lemma open_ec_open_tc_rec__commut : forall C kx kY (x Y:atom),
  (open_ec_rec kx x (open_tc_rec kY Y C)) = 
    (open_tc_rec kY Y (open_ec_rec kx x C)).
Proof.
  induction C; intros kx kY x Y; intros; simpl;
    try solve [
      auto | 
      rewrite IHC; auto | 
      rewrite IHC1; auto; rewrite IHC2; auto |
      rewrite IHC; auto; rewrite open_ee_open_te_rec__commut; auto
    ].
Qed.

Definition P_open_ec_rec_swap (n:nat) := forall C (x y:atom) k,
  ctx_size C = n ->
  context (open_ec_rec k x C) ->
  context (open_ec_rec k y C).

Lemma _open_ec_rec_swap : forall n, P_open_ec_rec_swap n.
Proof.
  intro n.
  apply lt_wf_ind. clear n.
  intros n H.
  unfold P_open_ec_rec_swap in *.
  intros C x y k H1 H2; (ctx_cases (destruct C) Case); subst; simpl in *; auto.
  Case "ctx_abs_free".
    inversion H2; subst.
    apply context_abs_free with (L:=L); auto.
      ...
      rewrite open_ec_open_ec_rec__commut in x0n; auto.
      apply H with (x:=x)(m:=ctx_size (open_ec_rec 0 x0 C)); auto.
        rewrite open_ec_rec_ctx_size_eq. auto.
  Case "ctx_abs_capture".
    inversion H2; subst.
    apply context_abs_capture with (L:=L); auto.
      intros x0 x0n.
      apply H5 in x0n; auto.
      unfold open_ec in *.
      rewrite open_ec_open_ec_rec__commut; auto.
      rewrite open_ec_open_ec_rec__commut in x0n; auto.
      apply H with (x:=x)(m:=ctx_size (open_ec_rec 0 x0 C)); auto.
  ...
  Case "ctx_app1".
    inversion H2; subst.
    apply context_app1.
      apply H with (x:=x)(m:=ctx_size C); simpl; try solve [auto|omega].
      apply open_ee_rec_swap with (x:=x); auto.
  Case "ctx_app2".
    inversion H2; subst.
    apply context_app2.
      apply open_ee_rec_swap with (x:=x); auto.
  ...
  Case "ctx_tabs_free".
    ...
    apply context_tabs_free with (L:=L); auto.
      ...
      apply H1 in Xn; auto.
      unfold open_tc in *.
      rewrite <- open_ec_open_tc_rec__commut; auto.
      rewrite <- open_ec_open_tc_rec__commut in Xn; auto.
      apply H with (x:=x)(m:=ctx_size (open_tc_rec 0 X C)); auto.
        rewrite open_tc_rec_ctx_size_eq. auto.
  Case "ctx_tabs_capture".
    inversion H2; subst.
    apply context_tabs_capture with (L:=L); auto.
      intros X Xn.
      apply H1 in Xn; auto.
      unfold open_tc in *.
      rewrite <- open_ec_open_tc_rec__commut; auto.
  ...
  Case "ctx_let2_free".
    ...
    apply context_let2_free with (L:=L); auto.
      ...
      rewrite open_ec_open_ec_rec__commut in x0n; auto.
      apply H with (x:=x)(m:=ctx_size (open_ec_rec 0 x0 C)); auto.
        rewrite open_ec_rec_ctx_size_eq. simpl. omega.
  Case "ctx_let2_capture".
    inversion H2; subst.
    apply context_let2_capture with (L:=L); auto.
      apply open_ee_rec_swap with (x:=x); auto.
  ...
  Case "ctx_apair1".
    inversion H2; subst.
    apply context_apair1.
      apply H with (x:=x)(m:=ctx_size C); simpl; try solve [auto|omega].
      apply open_ee_rec_swap with (x:=x); auto.
  Case "ctx_apair2".
    inversion H2; subst.
    apply context_apair2.
      apply open_ee_rec_swap with (x:=x); auto.
      apply H with (x:=x)(m:=ctx_size C); simpl; try solve [auto|omega].
  Case "ctx_fst".
    inversion H2; subst.
    apply context_fst; auto.
      apply H with (x:=x)(m:=ctx_size C); simpl; try solve [auto|omega].
  Case "ctx_snd".
    inversion H2; subst.
    apply context_snd; auto.
      apply H with (x:=x)(m:=ctx_size C); simpl; try solve [auto|omega].
Qed.

Lemma open_ec_rec_swap : forall C (x y:atom) k,
  context (open_ec_rec k x C) ->
  context (open_ec_rec k y C).
Proof.
  intros C x y k H1.
  assert (P_open_ec_rec_swap (ctx_size C)) as J.
    apply (@_open_ec_rec_swap (ctx_size C)).
  unfold P_open_ec_rec_swap in J.
  apply J with (x:=x); auto.
Qed.

Lemma open_ee_rec_id_bexp_eupper : forall e e' k,
  bexp_eupper e <=k ->
  open_ee_rec k e' e = e.
Proof.
  induction e; intros e' k H1; simpl in *; 
    try solve [auto | rewrite IHe; try solve [auto|omega]].
    destruct (k==n); subst; auto.
      contradict H1; omega.

    rewrite IHe1. rewrite IHe2; auto.
      assert (J2:=@le_max_r (bexp_eupper e1) (bexp_eupper e2)). omega.
      assert (J1:=@le_max_l (bexp_eupper e1) (bexp_eupper e2)). omega.

    rewrite IHe1. rewrite IHe2; auto.
      assert (J2:=@le_max_r (bexp_eupper e1) (bexp_eupper e2-1)). omega.
      assert (J1:=@le_max_l (bexp_eupper e1) (bexp_eupper e2-1)). omega.
...
Lemma close_ee_bexp_eupper : forall e k (x:atom),
  bexp_eupper (close_ee_rec k x e) <= max (bexp_eupper e) (k+1).
Proof.
  induction e; intros k x; simpl in *; auto.
    Case "bvar".
    apply le_max_l; auto.

    Case "fvar".
    destruct (x==a); simpl; omega.

    Case "abs".
    assert (J:=@IHe (S k) x).
    destruct (le_lt_dec (bexp_eupper e) (S k+1)).
      rewrite max_r in J; auto.
      rewrite max_r; try solve [omega].

      rewrite max_l in J; try solve [omega].
      rewrite max_l; try solve [omega].

    Case "app".
    assert (J1:=@IHe1 k x).
    assert (J2:=@IHe2 k x).
    destruct (le_lt_dec (bexp_eupper e1) (k+1)).
      rewrite max_r in J1; try solve [omega].
      destruct (le_lt_dec (bexp_eupper e2) (k+1)).
        rewrite max_r in J2; try solve [omega].
        destruct (le_lt_dec (bexp_eupper e1) (bexp_eupper e2)).
          rewrite max_r with (n:=bexp_eupper e1)(m:=bexp_eupper e2); try solve [omega].
          rewrite max_r with (n:=bexp_eupper e2) (m:=k+1); try solve [omega].
          destruct (le_lt_dec (bexp_eupper (close_ee_rec k x e1)) (bexp_eupper (close_ee_rec k x e2))).
            rewrite max_r; try solve [omega].
            rewrite max_l; try solve [omega].
          
          rewrite max_l with (n:=bexp_eupper e1)(m:=bexp_eupper e2); try solve [omega].
          rewrite max_r with (n:=bexp_eupper e1) (m:=k+1); try solve [omega].
          destruct (le_lt_dec (bexp_eupper (close_ee_rec k x e1)) (bexp_eupper (close_ee_rec k x e2))).
            rewrite max_r; try solve [omega].
            rewrite max_l; try solve [omega].

        rewrite max_l in J2; try solve [omega].
        destruct (le_lt_dec (bexp_eupper e1) (bexp_eupper e2)).
          rewrite max_r with (n:=bexp_eupper e1)(m:=bexp_eupper e2); try solve [omega].
          rewrite max_l with (n:=bexp_eupper e2) (m:=k+1); try solve [omega].
          destruct (le_lt_dec (bexp_eupper (close_ee_rec k x e1)) (bexp_eupper (close_ee_rec k x e2))).
            rewrite max_r; try solve [omega].
            rewrite max_l; try solve [omega].
      ...
      rewrite max_l in J1; try solve [omega].
      destruct (le_lt_dec (bexp_eupper e2) (k+1)).
        rewrite max_r in J2; try solve [omega].
        rewrite max_l with (n:=bexp_eupper e1)(m:=bexp_eupper e2); try solve [omega].
        rewrite max_l with (n:=bexp_eupper e1) (m:=k+1); try solve [omega].
        destruct (le_lt_dec (bexp_eupper (close_ee_rec k x e1)) (bexp_eupper (close_ee_rec k x e2))).
          rewrite max_r; try solve [omega].
          rewrite max_l; try solve [omega].
    ...
    Case "let".
    assert (J1:=@IHe1 k x).
    assert (J2:=@IHe2 (S k) x).
    destruct (le_lt_dec (bexp_eupper e1) (k+1)).
      rewrite max_r in J1; try solve [omega].
      destruct (le_lt_dec (bexp_eupper e2) (S k+1)).
        rewrite max_r in J2; try solve [omega].
        destruct (le_lt_dec (bexp_eupper e1) (bexp_eupper e2-1)).
          rewrite max_r with (n:=bexp_eupper e1)(m:=bexp_eupper e2 -1); try solve [omega].
          rewrite max_r with (n:=bexp_eupper e2-1) (m:=k+1); try solve [omega].
          destruct (le_lt_dec (bexp_eupper (close_ee_rec k x e1)) (bexp_eupper (close_ee_rec (S k) x e2)-1)).
            rewrite max_r; try solve [omega].
            rewrite max_l; try solve [omega].
          
          rewrite max_l with (n:=bexp_eupper e1)(m:=bexp_eupper e2-1); try solve [omega].
          rewrite max_r with (n:=bexp_eupper e1) (m:=k+1); try solve [omega].
          destruct (le_lt_dec (bexp_eupper (close_ee_rec k x e1)) (bexp_eupper (close_ee_rec (S k) x e2)-1)).
            rewrite max_r; try solve [omega].
            rewrite max_l; try solve [omega].
        ...
        destruct (le_lt_dec (bexp_eupper e1) (bexp_eupper e2-1)).
          rewrite max_r with (n:=bexp_eupper e1)(m:=bexp_eupper e2-1); try solve [omega].
          rewrite max_l with (n:=bexp_eupper e2-1) (m:=k+1); try solve [omega].
          destruct (le_lt_dec (bexp_eupper (close_ee_rec k x e1)) (bexp_eupper (close_ee_rec (S k) x e2)-1)).
            rewrite max_r; try solve [omega].
            rewrite max_l; try solve [omega].
...
Lemma open_ec_rec_id_bctx_eupper : forall C e k,
  bctx_eupper C <=k ->
  open_ec_rec k e C = C.
Proof.
  induction C; intros e' k H1; simpl in *; 
    try solve [auto | rewrite IHC; try solve [auto|omega]].
    rewrite IHC. rewrite open_ee_rec_id_bexp_eupper; auto.
      assert (J2:=@le_max_r (bctx_eupper C) (bexp_eupper e)). omega.
      assert (J1:=@le_max_l (bctx_eupper C) (bexp_eupper e)). omega.

    rewrite IHC. rewrite open_ee_rec_id_bexp_eupper; auto.
      assert (J1:=@le_max_l (bexp_eupper e) (bctx_eupper C)). omega.
      assert (J2:=@le_max_r (bexp_eupper e) (bctx_eupper C)). omega.

    rewrite IHC. rewrite open_ee_rec_id_bexp_eupper; auto.
      assert (J2:=@le_max_r (bctx_eupper C) (bexp_eupper e-1)). omega.
      assert (J1:=@le_max_l (bctx_eupper C) (bexp_eupper e-1)). omega.

    rewrite IHC. rewrite open_ee_rec_id_bexp_eupper; auto.
      assert (J1:=@le_max_l (bexp_eupper e) (bctx_eupper C-1)). omega.
      assert (J2:=@le_max_r (bexp_eupper e) (bctx_eupper C-1)). omega.
...
Lemma close_ec_bctx_eupper : forall C k (x:atom),
  bctx_eupper (close_ec_rec k x C) <= max (bctx_eupper C) (k+1).
Proof.
  induction C; intros k x; simpl in *; auto.
    Case "hole".
      omega.

    Case "abs_free".
    assert (J:=@IHC (S k) x).
    destruct (le_lt_dec (bctx_eupper C) (S k+1)).
      rewrite max_r in J; auto.
      rewrite max_r; try solve [omega].
    ...
    Case "app1".
    assert (J1:=@IHC k x).
    assert (J2:=@close_ee_bexp_eupper e k x).
    destruct (le_lt_dec (bctx_eupper C) (k+1)).
      rewrite max_r in J1; try solve [omega].
      destruct (le_lt_dec (bexp_eupper e) (k+1)).
        rewrite max_r in J2; try solve [omega].
        destruct (le_lt_dec (bctx_eupper C) (bexp_eupper e)).
          rewrite max_r with (n:=bctx_eupper C)(m:=bexp_eupper e); try solve [omega].
          rewrite max_r with (n:=bexp_eupper e) (m:=k+1); try solve [omega].
          destruct (le_lt_dec (bctx_eupper (close_ec_rec k x C)) (bexp_eupper (close_ee_rec k x e))).
            rewrite max_r; try solve [omega].
            rewrite max_l; try solve [omega].
          
          rewrite max_l with (n:=bctx_eupper C)(m:=bexp_eupper e); try solve [omega].
          rewrite max_r with (n:=bctx_eupper C) (m:=k+1); try solve [omega].
          destruct (le_lt_dec (bctx_eupper (close_ec_rec k x C)) (bexp_eupper (close_ee_rec k x e))).
            rewrite max_r; try solve [omega].
            rewrite max_l; try solve [omega].
        ...
        destruct (le_lt_dec (bctx_eupper C) (bexp_eupper e)).
          rewrite max_r with (n:=bctx_eupper C)(m:=bexp_eupper e); try solve [omega].
          rewrite max_l with (n:=bexp_eupper e) (m:=k+1); try solve [omega].
          destruct (le_lt_dec (bctx_eupper (close_ec_rec k x C)) (bexp_eupper (close_ee_rec k x e))).
            rewrite max_r; try solve [omega].
            rewrite max_l; try solve [omega].
      ...
      destruct (le_lt_dec (bexp_eupper e) (k+1)).
        rewrite max_r in J2; try solve [omega].
        rewrite max_l with (n:=bctx_eupper C)(m:=bexp_eupper e); try solve [omega].
        rewrite max_l with (n:=bctx_eupper C) (m:=k+1); try solve [omega].
        destruct (le_lt_dec (bctx_eupper (close_ec_rec k x C)) (bexp_eupper (close_ee_rec k x e))).
          rewrite max_r; try solve [omega].
          rewrite max_l; try solve [omega].
    ...
    Case "app2".
    assert (J1:=@close_ee_bexp_eupper e k x).
    assert (J2:=@IHC k x).
    destruct (le_lt_dec (bexp_eupper e) (k+1)).
      rewrite max_r in J1; try solve [omega].
      destruct (le_lt_dec (bctx_eupper C) (k+1)).
        rewrite max_r in J2; try solve [omega].
        destruct (le_lt_dec (bexp_eupper e) (bctx_eupper C)).
          rewrite max_r with (n:=bexp_eupper e)(m:=bctx_eupper C); try solve [omega].
          rewrite max_r with (n:=bctx_eupper C) (m:=k+1); try solve [omega].
          destruct (le_lt_dec (bexp_eupper (close_ee_rec k x e)) (bctx_eupper (close_ec_rec k x C))).
            rewrite max_r; try solve [omega].
            rewrite max_l; try solve [omega].
          
          rewrite max_l with (n:=bexp_eupper e)(m:=bctx_eupper C); try solve [omega].
          rewrite max_r with (n:=bexp_eupper e) (m:=k+1); try solve [omega].
          destruct (le_lt_dec (bexp_eupper (close_ee_rec k x e)) (bctx_eupper (close_ec_rec k x C))).
            rewrite max_r; try solve [omega].
            rewrite max_l; try solve [omega].
    ...
    Case "let1".
    assert (J1:=@IHC k x).
    assert (J2:=@close_ee_bexp_eupper e (S k) x).
    destruct (le_lt_dec (bctx_eupper C) (k+1)).
      rewrite max_r in J1; try solve [omega].
      destruct (le_lt_dec (bexp_eupper e) (S k+1)).
        rewrite max_r in J2; try solve [omega].
        destruct (le_lt_dec (bctx_eupper C) (bexp_eupper e -1)).
          rewrite max_r with (n:=bctx_eupper C)(m:=bexp_eupper e-1); try solve [omega].
          rewrite max_r with (n:=bexp_eupper e-1) (m:=k+1); try solve [omega].
          destruct (le_lt_dec (bctx_eupper (close_ec_rec k x C)) (bexp_eupper (close_ee_rec (S k) x e)-1)).
            rewrite max_r; try solve [omega].
            rewrite max_l; try solve [omega].
          
          rewrite max_l with (n:=bctx_eupper C)(m:=bexp_eupper e-1); try solve [omega].
          rewrite max_r with (n:=bctx_eupper C) (m:=k+1); try solve [omega].
          destruct (le_lt_dec (bctx_eupper (close_ec_rec k x C)) (bexp_eupper (close_ee_rec (S k) x e)-1)).
            rewrite max_r; try solve [omega].
            rewrite max_l; try solve [omega].
        ...
        destruct (le_lt_dec (bctx_eupper C) (bexp_eupper e-1)).
          rewrite max_r with (n:=bctx_eupper C)(m:=bexp_eupper e-1); try solve [omega].
          rewrite max_l with (n:=bexp_eupper e-1) (m:=k+1); try solve [omega].
          destruct (le_lt_dec (bctx_eupper (close_ec_rec k x C)) (bexp_eupper (close_ee_rec (S k) x e)-1)).
            rewrite max_r; try solve [omega].
            rewrite max_l; try solve [omega].
    ...
    destruct (le_lt_dec (bexp_eupper e) (k+1)).
      ...
      destruct (le_lt_dec (bctx_eupper C) (S k+1)).
        rewrite max_r in J2; try solve [omega].
        destruct (le_lt_dec (bexp_eupper e) (bctx_eupper C-1)).
          rewrite max_r with (n:=bexp_eupper e)(m:=bctx_eupper C-1); try solve [omega].
          rewrite max_r with (n:=bctx_eupper C-1) (m:=k+1); try solve [omega].
          destruct (le_lt_dec (bexp_eupper (close_ee_rec k x e)) (bctx_eupper (close_ec_rec (S k) x C)-1)).
            rewrite max_r; try solve [omega].
            rewrite max_l; try solve [omega].
          
          rewrite max_l with (n:=bexp_eupper e)(m:=bctx_eupper C-1); try solve [omega].
          rewrite max_r with (n:=bexp_eupper e) (m:=k+1); try solve [omega].
          destruct (le_lt_dec (bexp_eupper (close_ee_rec k x e)) (bctx_eupper (close_ec_rec (S k) x C)-1)).
            rewrite max_r; try solve [omega].
            rewrite max_l; try solve [omega].
        ...
        destruct (le_lt_dec (bexp_eupper e) (bctx_eupper C-1)).
          rewrite max_r with (n:=bexp_eupper e)(m:=bctx_eupper C-1); try solve [omega].
          rewrite max_l with (n:=bctx_eupper C-1) (m:=k+1); try solve [omega].
          destruct (le_lt_dec (bexp_eupper (close_ee_rec k x e)) (bctx_eupper (close_ec_rec (S k) x C)-1)).
            rewrite max_r; try solve [omega].
            rewrite max_l; try solve [omega].
      ...
      rewrite max_l in J1; try solve [omega].
      destruct (le_lt_dec (bctx_eupper C-1) (k+1)).
        rewrite max_r in J2; try solve [omega].
        rewrite max_l with (n:=bexp_eupper e)(m:=bctx_eupper C-1); try solve [omega].
        rewrite max_l with (n:=bexp_eupper e) (m:=k+1); try solve [omega].
        destruct (le_lt_dec (bexp_eupper (close_ee_rec k x e)) (bctx_eupper (close_ec_rec (S k) x C)-1)).
